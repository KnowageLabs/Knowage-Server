...
Window.prototype.handleEvent = function(evt) {
    ...
    if (evt.type == "mousedown") {
    	var elId = evt.target.getAttributeNS(null,"id");
    	if (elId == "titleBar"+this.id) {
    		//put it to the front
    		this.parentGroup.appendChild(this.windowGroup);
    		if (this.moveable) {
...
Window.prototype.resize = function(width,height,fireFunction) {
	...
	this.windowGroup.setAttributeNS(null,"transform","translate("+this.transX+","+this.transY+")");
	if (fireFunction) {
		this.timer.setTimeout("fireFunction",this.timerMs,"resized");
	}
}
...
Window.prototype.appendContent = function(node,inheritDisplay) {
	...
	this.windowMainGroup.appendChild(node);
}
...
Window.prototype.removeContent = function(node) {
	...
	this.windowMainGroup.removeChild(node);
}
...
Window.prototype.insertContentBefore = function(node,referenceNode,inheritDisplay) {
	...
	this.windowMainGroup.insertBefore(node,referenceNode);
}

//hide content of Window
Window.prototype.hideContents = function() {
        if (this.windowMainGroup.hasChildNodes()) {
                var windowContentChild = this.windowMainGroup.firstChild;
                while (windowContentChild) {
                        if (windowContentChild.nodeType == 1) { //ELEMENT_NODE
                                windowContentChild.setAttributeNS(null,"display","none");
                        }
                        windowContentChild = windowContentChild.nextSibling;
                }
        }
}
...
Window.prototype.showContents = function() {
        if (this.windowMainGroup.hasChildNodes()) {
                ...
                while (windowContentChild) {
                        if (windowContentChild.nodeType == 1) { //ELEMENT_NODE
                                windowContentChild.setAttributeNS(null,"display","inherit");
                        }
                        windowContentChild = windowContentChild.nextSibling;
                }
        }
