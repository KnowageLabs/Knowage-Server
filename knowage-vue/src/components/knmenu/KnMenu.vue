    ...
    <div class="menu-scroll-content">
      ...
      <div>
        <ul class="layout-menu">
          <KnAdminMenu
            :model="technicalUserFunctionalities"
            v-if="technicalUserFunctionalities && technicalUserFunctionalities.length > 0"
            @click="itemClick"
          ></KnAdminMenu>
          <template v-for="(item, i) of allowedUserFunctionalities" :key="i">
            <KnMenuItem
              :item="item"
              @click="itemClick"
              v-if="!item.conditionedView || (item.conditionedView == 'download' && download) || (item.conditionedView == 'news' && news)"
            ></KnMenuItem>
          </template>
...
import DownloadsDialog from '../downloadsDialog/DownloadsDialog.vue'
