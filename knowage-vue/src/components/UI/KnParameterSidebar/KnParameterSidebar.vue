  ...
  <div id="kn-parameter-sidebar" :class="positionClass">
    <Toolbar
      v-if="
        mode !== 'workspaceView' &&
          mode !== 'qbeView' &&
          mode !== 'datasetManagement'
      "
      id="kn-parameter-sidebar-toolbar"
      class="kn-toolbar kn-toolbar--secondary"
    >
      <template #start>
        <div
          id="kn-parameter-sidebar-toolbar-icons-container"
          class="p-d-flex p-flex-row p-jc-around"
        >
          <Button
            icon="fa fa-eraser"
            class="p-button-text p-button-rounded p-button-plain p-mx-2"
            v-tooltip.top="$t('documentExecution.main.resetParametersTooltip')"
            @click="resetAllParameters"
          ></Button>
          <Button
            icon="pi pi-pencil"
            class="p-button-text p-button-rounded p-button-plain p-mx-2"
            v-tooltip.top="$t('documentExecution.main.savedParametersTooltip')"
            @click="openSavedParametersDialog"
          ></Button>
          <Button
            icon="fas fa-save"
            class="p-button-text p-button-rounded p-button-plain p-mx-2"
            v-tooltip.top="
              $t('documentExecution.main.saveParametersFromStateTooltip')
            "
            @click="openSaveParameterDialog"
          ></Button>
        </div>
      </template>
    </Toolbar>
    ...
    <div class="p-fluid kn-parameter-sidebar-content kn-alternated-rows">
      <div
        ...
        v-if="
          user &&
            (!sessionRole ||
              sessionRole === $t('role.defaultRolePlaceholder')) &&
            (mode === 'execution' ||
              mode === 'qbeView' ||
              (mode === 'workspaceView' && dataset?.drivers?.length > 0))
        "
      >
        <div class="p-d-flex">
        ...
        <div v-for="(qbeParameter, index) in qbeParameters" :key="index">
          <div class="p-field p-m-4">
            <div class="p-d-flex">
              ...
              <i
                class="fa fa-eraser parameter-clear-icon kn-cursor-pointer"
                v-tooltip.left="
                  $t('documentExecution.main.parameterClearTooltip')
                "
                @click="qbeParameter.value = qbeParameter.defaultValue"
              ></i>
            </div>
            <Chips
              v-if="qbeParameter.multiValue"
            ...
            <InputText
              ...
              :class="{
                'p-invalid':
                  !qbeParameter.value && mode !== 'datasetManagement',
              }"
            />
          </div>
        </div>
      ...
      <template
        v-for="(parameter, index) in parameters.filterStatus"
        :key="index"
      >
        <div
          class="p-field p-my-1 p-p-2"
          v-if="
            (parameter.type === 'STRING' || parameter.type === 'NUM') &&
              !parameter.selectionType &&
              parameter.valueSelection === 'man_in' &&
              parameter.showOnPanel === 'true' &&
              parameter.visible
          "
        >
          <div class="p-d-flex">
            <label
              ...
              :class="{
                'p-text-italic':
                  ...
                  parameter.lovDependsOnParameters,
              }"
            ...
            <i
              ...
              v-tooltip.left="
                $t('documentExecution.main.parameterClearTooltip')
              "
              @click="resetParameterValue(parameter)"
              :data-test="'parameter-input-clear-' + parameter.id"
            ></i>
          </div>
          <InputText
        ...
        >
          <div class="p-d-flex">
            ...
            <i
              ...
              "
              @click="resetParameterValue(parameter)"
              :data-test="'parameter-date-clear-' + parameter.id"
            ></i>
          </div>
          <Calendar
            v-if="parameter.parameterValue"
        ...
        >
          <div class="p-d-flex">
            ...
            <i
              ...
              "
              @click="resetParameterValue(parameter)"
              :data-test="'parameter-checkbox-clear-' + parameter.id"
            ></i>
          </div>
          <ScrollPanel class="inputScrollPanel">
            <div
              class="p-field-radiobutton"
              v-for="(option, index) in parameter.data"
              :key="index"
              :data-test="'parameter-list-' + parameter.id"
            >
              <RadioButton
                v-if="!parameter.multivalue && parameter.parameterValue"
                :value="option.value"
                v-model="parameter.parameterValue[0].value"
                @change="setRadioButtonValue(parameter)"
              />
              <Checkbox
                v-if="parameter.multivalue && parameter.parameterValue"
                :value="option.value"
                v-model="selectedParameterCheckbox[parameter.id]"
                @change="setCheckboxValue(parameter)"
              />
        ...
        >
          ...
          <Dropdown
            ...
            @change="onDropdownChange(parameter)"
          />
        ...
        >
          ...
          <div class="p-d-flex p-flex-row">
            <i
              class="pi pi-external-link kn-cursor-pointer p-mr-2"
              @click="openPopupDialog(parameter)"
            ></i>
            <ScrollPanel class="lookupScrollPanel">
              <Chip
                class="parameterValueChip"
                v-for="(parameterValue, index) in parameter.parameterValue"
                :key="index"
                >{{ parameterValue.description ?? parameterValue.value }}</Chip
              >
            </ScrollPanel>
          </div>
        </div>
        ...
        >
          ...
          <div class="p-d-flex p-flex-row">
            <i
              class="pi pi-external-link kn-cursor-pointer p-mr-2"
              @click="openTreeDialog(parameter)"
            ></i>
            <div>
              <Chip
                v-for="(parameterValue, index) in parameter.parameterValue"
    ...
    <div
      v-if="
        (parameters && parameters.filterStatus.length > 0) ||
          mode === 'qbeView' ||
          mode === 'workspaceView' ||
          mode === 'datasetManagement'
      "
      class="p-fluid p-d-flex p-flex-row p-m-2 kn-parameter-sidebar-buttons"
    >
      <Button
        class="kn-button kn-button--primary"
        :disabled="buttonsDisabled"
        @click="$emit('execute', qbeParameters, parameters)"
      >
      ...
      <Button
        v-if="
          mode !== 'qbeView' &&
            ...
            mode !== 'datasetManagement'
        "
        class="kn-button kn-button--primary p-ml-1"
    ...
    <KnParameterPopupDialog
      ...
      :propLoading="loading"
      :parameterPopUpData="parameterPopUpData"
    ...
    <KnParameterTreeDialog
      ...
      :selectedParameter="selectedParameter"
      :formatedParameterValues="formatedParameterValues"
      :document="document"
      :mode="mode"
      :selectedRole="role"
      @close="onTreeClose"
    ...
    <KnParameterSaveDialog
      :visible="parameterSaveDialogVisible"
      :propLoading="loading"
      @close="parameterSaveDialogVisible = false"
      @saveViewpoint="saveViewpoint"
    ></KnParameterSaveDialog>
    <KnParameterSavedParametersDialog
      :visible="savedParametersDialogVisible"
      :propViewpoints="viewpoints"
      @close="savedParametersDialogVisible = false"
      @fillForm="fillParameterForm"
      @executeViewpoint="executeViewpoint"
      @deleteViewpoint="deleteViewpoint"
    ></KnParameterSavedParametersDialog>
  </div>
</template>
<script lang="ts">
...
} from "./KnParameterSidebarVisualDependency";
import {
  setDataDependency,
  updateDataDependency,
} from "./KnParameterSidebarDataDependency";
import {
  setLovsDependency,
  updateLovDependency,
} from "./KnParameterSidebarLovsDependency";
...
export default defineComponent({
  ...
  data() {
    return {
      ...
      parameters: { isReadyForExecution: false, filterStatus: [] } as {
  ...
  computed: {
    ...
    positionClass(): string {
      return this.document?.parametersRegion
        ? "kn-parameter-sidebar-" + this.document.parametersRegion
  ...
  created() {
    ...
    if (
      this.mode === "qbeView" ||
      this.mode === "workspaceView" ||
      this.mode === "datasetManagement"
    )
      this.loadQBEParameters();
  ...
  methods: {
    applyFieldClass(cssClass: string): string {
      let cssCompleteClass = this.primary
        ? cssClass + " fieldBackgroundColorPrimary"
        : cssClass + " fieldBackgroundColorSecondary";
    ...
    loadParameters() {
      ...
      this.filtersData?.filterStatus?.forEach((el: iParameter) => {
        if (
          el.selectionType == "LIST" &&
          el.showOnPanel == "true" &&
          el.visible &&
          el.multivalue
        ) {
          this.selectedParameterCheckbox[el.id] = el.parameterValue?.map(
            (parameterValue: any) => parameterValue.value
          );
        }
        this.parameters.filterStatus.push(el);
      });
      this.parameters?.filterStatus.forEach((el: any) =>
        setVisualDependency(this.parameters, el)
      );
      this.parameters?.filterStatus.forEach((el: any) =>
        setDataDependency(this.parameters, el)
      );
      this.parameters?.filterStatus.forEach((el: any) =>
        setLovsDependency(this.parameters, el)
      );
      this.parameters?.filterStatus.forEach((el: any) =>
        this.updateVisualDependency(el)
      );
    },
      ...
      if (parameter.dependencies.data.length !== 0) {
        parameter.dependencies.data.forEach((dependency: any) => {
          ...
          if (index !== -1) {
            ...
            parameter.dataDependsOnParameters
              ? parameter.dataDependsOnParameters.push(tempParameter)
              : (parameter.dataDependsOnParameters = [tempParameter]);
            tempParameter.dataDependentParameters
              ? tempParameter.dataDependentParameters.push(parameter)
              : (tempParameter.dataDependentParameters = [parameter]);
          }
        });
      }
    },
    resetParameterValue(parameter: any) {
      ...
      if (parameter.metadata.colsMap) {
        valueIndex = Object.keys(parameter.metadata.colsMap).find(
          (key: string) => parameter.metadata.colsMap[key] === valueColumn
        );
      }
      let descriptionIndex = null as any;
      if (parameter.metadata.colsMap) {
        descriptionIndex = Object.keys(parameter.metadata.colsMap).find(
          (key: string) => parameter.metadata.colsMap[key] === descriptionColumn
        );
      }
      if (
        (parameter.selectionType === "LIST" ||
        ...
        for (let i = 0; i < parameter.driverDefaultValue.length; i++) {
          ...
          parameter.parameterValue.push({
            value: valueIndex ? temp[valueIndex] : "",
            description: descriptionIndex ? temp[descriptionIndex] : "",
          });
          if (valueIndex) {
        ...
        parameter.parameterValue = parameter.driverDefaultValue?.map(
          (el: { value: string; desc: string }) => {
        ...
        parameter.parameterValue = parameter.driverDefaultValue.map(
          (el: any) => {
            return {
              value: valueIndex ? el[valueIndex] : "",
              description: descriptionIndex ? el[descriptionIndex] : "",
            };
          }
        );
      } else if (
        ...
        if (parameter.driverDefaultValue[0].desc?.split("#")[0]) {
          parameter.parameterValue[0].value = this.getFormattedDate(
            parameter.driverDefaultValue[0].desc?.split("#")[0],
            undefined
          );
        }
      } else {
        ...
        parameter.parameterValue[0].value =
          parameter.driverDefaultValue[0].value ??
          parameter.driverDefaultValue[0][valueIndex];
      }
      this.parameters.filterStatus.forEach((el: any) =>
        this.updateDependency(el)
      );
    ...
    createMenuItems() {
      ...
      this.executeMenuItems.push({
        label: this.$t("common.exportCSV"),
        command: () => this.$emit("exportCSV"),
      });
    },
    ...
    setRadioButtonValue(parameter: iParameter) {
      const index = parameter.data?.findIndex(
        (el: any) => el.value === parameter.parameterValue[0].value
      );
      if (index !== -1)
        parameter.parameterValue[0].description =
          parameter.data[index].description;
      this.updateDependency(parameter);
    },
    setCheckboxValue(parameter: iParameter) {
      parameter.parameterValue = this.selectedParameterCheckbox[
        parameter.id
      ].map((el: any) => {
    ...
    async getParameterPopupInfo(parameter: iParameter) {
      ...
      const role =
        this.sessionRole &&
        this.sessionRole !== this.$t("role.defaultRolePlaceholder")
          ? this.sessionRole
          : this.role;
      const postData = {
        label: this.document?.label,
        parameters: this.getFormattedParameters(),
        paramId: parameter.urlName,
        role: role,
      };
      ...
      if (this.mode !== "execution" && this.document) {
        url =
          this.document.type === "businessModel"
            ? `1.0/businessmodel/${this.document.name}/admissibleValues`
            : `/3.0/datasets/${this.document.label}/admissibleValues`;
      }
      ...
      await this.$http
        ...
        .then(
          (response: AxiosResponse<any>) =>
            (this.parameterPopUpData = response.data)
        )
    ...
    getParameterValues() {
      ...
      Object.keys(this.parameters.filterStatus).forEach((key: any) => {
        ...
        if (parameter.type === "DATE") {
          ...
          parameters[
            parameter.urlName + "_field_visible_description"
          ] = tempString;
        } else {
          parameters[parameter.urlName] = parameter.parameterValue[0].value;
          parameters[parameter.urlName + "_field_visible_description"] =
    ...
    async saveViewpoint(viewpoint: any) {
      ...
      const postData = {
        ...viewpoint,
        OBJECT_LABEL: this.document?.label,
        ROLE: role,
        VIEWPOINT: this.getParameterValues(),
      };
      this.loading = true;
      await this.$http
        .post(
          process.env.VUE_APP_RESTFUL_SERVICES_PATH +
            `1.0/documentviewpoint/addViewpoint`,
          postData
        )
        .then(() => {
    ...
    async openSavedParametersDialog() {
      ...
      await this.$http
        .get(
          process.env.VUE_APP_RESTFUL_SERVICES_PATH +
            `1.0/documentviewpoint/getViewpoints?label=${this.document?.label}&role=${role}`
        )
    ...
    fillParameterForm(viewpoint: any) {
      const tempParameters = this.decodeViewpointPrameterValues(
        viewpoint.vpValueParams
      );

      Object.keys(tempParameters)?.forEach((key: any) => {
        const index = this.parameters.filterStatus.findIndex(
          (el: any) => el.urlName === key
        );
        if (index !== -1) {
          ...
          if (parameter.type === "DATE") {
            ...
            parameter.parameterValue[0].value =
              temp instanceof Date && !isNaN(temp as any)
                ? temp
                : this.getFormattedDate(tempParameters[key], "DD/MM/YYYY");
            ...
            parameter.parameterValue[0].description =
              tempParameters[key + "_field_visible_description"];
          } else if (
            parameter.selectionType === "TREE" ||
            parameter.selectionType === "LOOKUP" ||
            parameter.multivalue
            ...
            const tempArrayDescriptions = tempParameters[
              key + "_field_visible_description"
            ].split(";");
            parameter.parameterValue = [];
            for (let i = 0; i < tempArrayValues.length; i++) {
              parameter.parameterValue[i] = {
                value: tempArrayValues[i],
                description: tempArrayDescriptions[i] ?? "",
              };
            }
            if (parameter.selectionType === "LIST") {
              this.selectedParameterCheckbox[
                parameter.id
              ] = parameter.parameterValue?.map(
                (parameterValue: any) => parameterValue.value
              );
            }
          } else {
    ...
    async deleteViewpoint(viewpoint: any) {
      ...
      await this.$http
        .post(
          process.env.VUE_APP_RESTFUL_SERVICES_PATH +
            `1.0/documentviewpoint/deleteViewpoint`,
          { VIEWPOINT: "" + viewpoint.vpId }
        )
        .then(async () => {
    ...
    removeViewpoint(viewpoint: any) {
      const index = this.viewpoints.findIndex(
        (el: any) => el.vpId === viewpoint.vpId
      );
    ...
    updateParameterDescriptionOnDropdownChange(parameter: any) {
      ...
      const index = parameter.data?.findIndex(
        (el: { value: string; description: string }) =>
          el.value === parameter.parameterValue[0].value
      );
      if (index !== -1)
        parameter.parameterValue[0].description =
          parameter.data[index].description;
