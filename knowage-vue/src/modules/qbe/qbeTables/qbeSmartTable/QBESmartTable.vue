    ...
    >
        ...
        <Column v-for="(col, index) of filteredVisibleFields" class="kn-truncated" :hidden="!col.visible" :field="`column_${index + 1}`" :key="index" :style="qbeSimpleTableDescriptor.style.column">
            ...
            <template #body="slotProps">
                <span v-if="typeof slotProps.data[`column_${index + 1}`] === 'number' && slotProps.data[`column_${index + 1}`]"> {{ getFormattedNumber(col, slotProps.data[`column_${index + 1}`]) }}</span>
...
export default defineComponent({
    ...
    methods: {
        ...
        getFormattedNumber(column: any, number: number) {
            ...
            console.log('GOOOOOOOOOOOOOOOOOOOOOOOOOOOT HEEEEEEEEEEEEEEEEEEEEERE', formattedNumber)
