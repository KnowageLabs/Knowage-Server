<template>
  <Dialog class="full-screen-dialog" :visible="true" :modal="false" :closable="false" position="right" :baseZIndex="1" :autoZIndex="true" @click="closeMainMenu">
    <template #header>
    ...
    <KnCalculatedField v-if="calcFieldDialogVisible" v-model:template="selectedCalcField" v-model:visibility="calcFieldDialogVisible" :fields="calcFieldColumns" :descriptor="calcFieldDescriptor" :propCalcFieldFunctions="calcFieldFunctionsToShow" :readOnly="false" :valid="true" source="QBE" @save="onCalcFieldSave" @cancel="calcFieldDialogVisible = false">
...
export default defineComponent({
  ...
  methods: {
    ...
    async loadDataset() {
      ...
      if (dataset) {
        await this.$http.get(process.env.VUE_APP_RESTFUL_SERVICES_PATH + `1.0/datasets/${dataset.label}`).then((response: AxiosResponse<any>) => {
    ...
    closeMainMenu() {
      if ((this.$store as any).menuOpened) {
    ...
    async initializeQBE() {
      if (this.dataset) {
        ...
        await this.$http
          .get(process.env.VUE_APP_QBE_PATH + `start-qbe?sourceDatasetLabel=${sourceDatasetLabel}&user_id=${this.user?.userUniqueIdentifier}&SBI_EXECUTION_ID=${this.uniqueID}`)
          .then(() => {
            this.qbeLoaded = true;
          })
          .catch(() => {});
      ...
      if (this.returnQueryMode) {
        ...
        if (this.sourceDataset && this.qbe?.dsTypeCd) {
          if (this.qbe?.dsTypeCd === "File" && !this.qbe.persistTableName) {
            await this.$http
              .get(process.env.VUE_APP_QBE_PATH + `qbequery/persistTableName?sourceDatasetName=${this.qbe.label}&SBI_EXECUTION_ID=${this.uniqueID}`)
              .then((response: AxiosResponse<any>) => {
