<template>
  ...
  <Toolbar class="kn-toolbar p-mb-2">
    <template #right>
      <FabButton
        icon="fas fa-plus"
        :style="licenseDialogDescriptor.fabButton.style"
        v-tooltip.top="$t('licenseDialog.dataRequired')"
        @click="setUploadType('', false)"
        data-test="new-button"
      />
      <KnInputFile
        label=""
        v-if="!uploading"
        :changeFunction="uploadLicense"
        accept=".lic"
  ...
  <Listbox
    ...
    :style="licenseDialogDescriptor.list.style"
  ...
  >
    ...
    <template #option="slotProps">
      <div class="kn-list-item" data-test="list-item">
        <Avatar
          :image="
            require(`@/assets/images/licenseImages/${slotProps.option.product}.png`)
          "
          size="medium"
        />
        <div class="kn-list-item-text">
          ...
          <span
            class="kn-list-item-text-secondary"
            :class="setLicenseClass(slotProps.option.status)"
            >{{ licenseText(slotProps.option.status) }}</span
          >
        </div>
        <div class="kn-list-item-text">
          <span class="kn-list-item-text-secondary"
            >{{ $t("licenseDialog.licenseId") }}:</span
          >
        ...
        <Button
          icon="pi pi-download"
          class="p-button-link"
          v-tooltip.top="$t('licenseDialog.downloadLicense')"
          @click="downloadLicence(slotProps.option.product)"
          data-test="download-button"
        />
        <Button
          icon="pi pi-pencil"
          class="p-button-link"
          v-tooltip.top="$t('licenseDialog.changeLicense')"
          @click="setUploadType(slotProps.option.product, true)"
          data-test="edit-button"
        />
        <Button
          icon="pi pi-trash"
          class="p-button-link"
          v-tooltip.top="$t('licenseDialog.deleteLicense')"
          @click="showDeleteDialog(slotProps.option.product)"
...
export default defineComponent({
  ...
  methods: {
    ...
    licenseText(status: string) {
      return status === "LICENSE_VALID"
        ? this.$t("licenseDialog.validLicense")
        : this.$t("licenseDialog.invalidLicense");
    },
    async downloadLicence(productName) {
      await axios
        .get(
          process.env.VUE_APP_RESTFUL_SERVICES_PATH +
            `1.0/license/download` +
          ...
          (response) => {
            if (response.data.errors) {
              ...
              if (response.headers) {
                ...
                var fileAndExtension = contentDisposition.match(
                  /filename[^;\n=]*=((['"]).*?\2|[^;\n]*)/i
                )[1];
