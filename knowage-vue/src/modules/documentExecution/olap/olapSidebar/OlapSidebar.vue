    ...
    <div v-if="olap" class="p-d-flex p-flex-column kn-flex p-m-3">
      <div>
        <label class="kn-material-input-label">{{
          $t("documentExecution.olap.sidebar.drillOnDimension")
        }}</label>
        <SelectButton
          class="p-mt-2"
          v-model="drillOn"
          :options="olapSidebarDescriptor.drillOnOptions"
          @click="$emit('drillTypeChanged', drillOn)"
        ></SelectButton>
      </div>
      <div v-if="!olapDesignerMode" class="kn-flex-0">
        <div
          v-if="isButtonVisible('BUTTON_DRILL_THROUGH')"
          class="p-d-flex p-flex-column p-my-3"
        >
          <label class="kn-material-input-label">{{
            $t("documentExecution.olap.sidebar.drillOnData")
          }}</label>
          <Button
        ...
        <div>
          <label
            v-if="
              isButtonVisible('BUTTON_MDX') ||
                isButtonVisible('BUTTON_FLUSH_CACHE') ||
                olap.modelConfig.crossNavigation
            "
            class="kn-material-input-label"
            >{{ $t("documentExecution.olap.sidebar.olapFunctions") }}</label
          >
          <div class="p-grid p-mt-1">
            <div class="p-col-4">
              <Button
                ...
                v-if="isButtonVisible('BUTTON_MDX')"
            ...
            <div class="p-col-4">
              <Button
                ...
                class="p-button-plain kn-button--secondary"
                v-tooltip.top="
                  $t('documentExecution.olap.sidebar.reloadSchema')
                "
                v-if="isButtonVisible('BUTTON_FLUSH_CACHE')"
                @click="$emit('reloadSchema')"
              />
            </div>
            <div class="p-col-4">
              <Button
                v-if="olap.modelConfig.crossNavigation"
                icon="fas fa-arrow-right"
                class="p-button-plain kn-button--secondary"
                v-tooltip.top="
                  $t('documentExecution.olap.sidebar.enableCrossNavigation')
                "
        ...
        <div class="p-my-3">
          <label v-if="tableFunctionsVisible" class="kn-material-input-label">{{
            $t("documentExecution.olap.sidebar.tableFunctions")
          }}</label>
          <div class="p-grid p-mt-1">
            <div class="p-col-4">
              <Button
                ...
                v-tooltip.top="
                  $t('documentExecution.olap.sidebar.showParentMembers')
                "
            ...
            <div class="p-col-4">
              <Button
                ...
                class="p-button-plain kn-button--secondary"
                :class="{ 'olap-sidebar-button-active': hideSpans }"
                v-tooltip.top="$t('documentExecution.olap.sidebar.hideSpans')"
                v-if="isButtonVisible('BUTTON_HIDE_SPANS')"
            ...
            <div class="p-col-4">
              <Button
                ...
                "
                v-if="isButtonVisible('BUTTON_SORTING_SETTINGS')"
            ...
            <div class="p-col-4">
              <Button
                ...
                v-tooltip.top="
                  $t('documentExecution.olap.sidebar.showProperties')
                "
            ...
            <div class="p-col-4">
              <Button
                ...
                v-tooltip.top="
                  $t('documentExecution.olap.sidebar.suppressEmptyRowsColumns')
                "
            ...
            <div class="p-col-4">
              <Button
                ...
                class="p-button-plain kn-button--secondary"
                v-tooltip.top="
                  $t('documentExecution.olap.sidebar.saveCustomizedView')
                "
                v-if="isButtonVisible('BUTTON_SAVE_SUBOBJECT')"
      ...
      <div
        v-if="olapHasScenario && !olapDesignerMode"
        id="whatif-container"
        class="kn-flex"
      >
        <label class="kn-material-input-label">{{
          $t("documentExecution.olap.sidebar.whatIfTitle")
        }}</label>
        <div class="p-grid p-mt-1">
          <div v-if="olapLocked" class="p-col-4">
            <Button
              :icon="olapLocked ? 'fas fa-lock-open' : 'fas fa-lock'"
              class="p-button-plain kn-button--secondary"
              v-tooltip.top="
                olapLocked
                  ? $t('documentExecution.olap.sidebar.unlockSchema')
                  : $t('documentExecution.olap.sidebar.lockSchema')
              "
              @click="changeLock"
            />
          </div>
          <div v-if="olapLocked" class="p-col-4">
            <Button
              ...
              class="p-button-plain kn-button--secondary"
              v-tooltip.top="
                $t('documentExecution.olap.sidebar.saveAsNewVersion')
              "
              v-if="isButtonVisible('BUTTON_SAVE_SUBOBJECT')"
              @click="$emit('showSaveAsNewVersion')"
            />
          </div>
          <div v-if="olapLocked" class="p-col-4">
            <Button
              ...
              class="p-button-plain kn-button--secondary"
              v-tooltip.top="$t('documentExecution.olap.sidebar.undo')"
              v-if="isButtonVisible('BUTTON_UNDO')"
              @click="$emit('undo')"
            />
          </div>
          <div v-if="olapLocked" class="p-col-4">
            <Button
              ...
              class="p-button-plain kn-button--secondary"
              v-tooltip.top="
                $t('documentExecution.olap.sidebar.deleteVersions')
              "
              v-if="isButtonVisible('BUTTON_VERSION_MANAGER')"
              @click="$emit('showDeleteVersions')"
            />
          </div>
          <div class="p-col-4">
            <Button
              ...
              class="p-button-plain kn-button--secondary"
              v-tooltip.top="$t('documentExecution.olap.sidebar.outputWizard')"
              v-if="isButtonVisible('BUTTON_EXPORT_OUTPUT')"
              @click="$emit('showOutputWizard')"
            />
          </div>
          <div v-if="olapLocked" class="p-col-4">
            <Button
              ...
              class="p-button-plain kn-button--secondary"
              v-tooltip.top="$t('documentExecution.olap.sidebar.alg')"
              v-if="isButtonVisible('BUTTON_ALGORITHMS')"
              @click="$emit('showAlgorithmDialog')"
            />
          </div>
        </div>
      </div>
      ...
      <div v-if="olapDesignerMode" class="kn-flex p-mt-3">
        <label class="kn-material-input-label">{{
          $t("documentExecution.olap.sidebar.templateEditing")
        }}</label>
        <div class="p-grid p-mt-1">
          <div class="p-col-4">
            <Button
          ...
          <div v-if="whatIfMode" class="p-col-4">
            <Button
              ...
              class="p-button-plain kn-button--secondary"
              v-tooltip.top="$t('documentExecution.olap.sidebar.scenario')"
              @click="$emit('showScenarioWizard')"
            />
          </div>
          <div class="p-col-4">
            <Button
              icon="fas fa-arrow-right"
              class="p-button-plain kn-button--secondary"
              :class="{ 'olap-sidebar-button-active': crossNavigation }"
              v-tooltip.top="
                $t('documentExecution.olap.sidebar.defineCrossNavigation')
              "
              @click="$emit('openCrossNavigationDefinitionDialog')"
            />
          </div>
          <div class="p-col-4">
            <Button
              ...
              class="p-button-plain kn-button--secondary"
              v-tooltip.top="
                $t('documentExecution.olap.sidebar.configureButtonsVisiblity')
              "
              @click="$emit('openButtonWizardDialog')"
            />
          </div>
        </div>
      </div>
      ...
      <div class="p-fluid" v-if="olapDesignerMode">
        <Button
          :label="$t('documentExecution.olap.sidebar.closeDesigner')"
          class="p-button-plain kn-button--secondary"
          @click="closeOlapDesigner"
        />
        <Button
          :label="$t('documentExecution.olap.sidebar.saveTemplate')"
          class="p-button-plain kn-button--primary p-mt-2"
          @click="$emit('saveOlapDesigner')"
        />
      </div>
    </div>
  </div>
...
export default defineComponent({
  ...
  methods: {
    ...
    isButtonVisible(buttonName: string) {
      let isVisible = false;
      const index = this.olap?.modelConfig.toolbarVisibleButtons?.findIndex(
        (el: any) => el === buttonName
      );
      ...
      await this.$http
        .post(
          process.env.VUE_APP_RESTFUL_SERVICES_PATH +
            `1.0/locker/${this.olap.modelConfig.artifactId}`,
          null,
          {
            headers: {
        ...
        .then((response: AxiosResponse<any>) => {
          if (
            (response.data.status === "unlocked" ||
              response.data.status === "locked_by_user" ||
              response.data.status === "locked_by_other") &&
            this.olap
          ) {
