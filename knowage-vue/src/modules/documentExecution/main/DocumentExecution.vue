  ...
  methods: {
    ...
    async loadFilters(initialLoading: boolean = false) {
      ...
      await this.$http
        ...
        .then((response: AxiosResponse<any>) => (tempFiltersData = response.data))
        .catch((error: any) => {
      ...
      tempFiltersData?.filterStatus?.forEach((el: iParameter) => {
        ...
        if (el.driverDefaultValue?.length > 0) {
          ...
          if (el.type === "DATE" && !el.selectionType && el.valueSelection === "man_in" && el.showOnPanel === "true" && el.visible) {
            ...
            if (el.driverMaxValue) {
              const validDate = getValidDate("" + el.driverMaxValue, this.dateFormat);
    ...
    openCrossNavigationInNewWindow(popupOptions: any, crossNavigationDocument: any, navigationParams: any) {
      ...
      const url = process.env.VUE_APP_HOST_URL + `/knowage/restful-services/publish?PUBLISHER=documentExecutionNg&OBJECT_ID=${crossNavigationDocument.document.id}&OBJECT_LABEL=${crossNavigationDocument.document.label}&SELECTED_ROLE=${this.sessionRole}&SBI_EXECUTION_ID=null&OBJECT_NAME=${crossNavigationDocument.document.name}&CROSS_PARAMETER=${parameters}`;
