<template>
  <div class="kn-height-full detail-page-container">
      ...
      <template #end>
        <div class="p-d-flex p-jc-around">
          <Button
            ...
            v-if="
              document?.typeCode === 'DOCUMENT_COMPOSITE' &&
                documentMode === 'VIEW'
            "
    ...
    <div
      ...
      id="document-execution-view"
      class="p-d-flex p-flex-row myDivToPrint"
    >
      <div
        v-if="parameterSidebarVisible"
        id="document-execution-backdrop"
      ...
      <DocumentExecutionSchedulationsTable
        ...
        :propSchedulations="schedulations"
...
export default defineComponent({
  ...
  methods: {
    ...
    createMenuItems() {
      ...
      if (
        this.user.functionalities.includes("SendMailFunctionality") &&
        this.document.typeCode === "REPORT"
      ) {
        const index = this.toolbarMenuItems.findIndex(
          (item: any) => item.label === this.$t("common.info.info")
        );
        if (index !== -1) {
      ...
      if (
        this.user.functionalities.includes("SeeSnapshotsFunctionality") &&
        this.user.enterprise
      ) {
        const index = this.toolbarMenuItems.findIndex(
          (item: any) => item.label === this.$t("common.shortcuts")
        );
        if (index !== -1)
          this.toolbarMenuItems[index].items.unshift({
            icon: "",
    ...
    export(type: string) {
      if (this.document.typeCode === "OLAP") {
        ...
        const tempIndex = this.breadcrumbs.findIndex(
          (el: any) => el.label === this.document.label
        );
    ...
    closeDocument() {
      const link = this.$route.path.includes("workspace")
        ? "/workspace"
        : "/document-browser";
    ...
    async loadFilters(initialLoading: boolean = false) {
      if (
        ...
        this.parameterValuesMap[this.document.label + "-" + this.tabKey] &&
        initialLoading
      ) {
        this.filtersData = this.parameterValuesMap[
          this.document.label + "-" + this.tabKey
        ];
        ...
        if (tempFilters) {
          ...
          this.filtersData.filterStatus?.forEach((filter: any) => {
            if (filter.type === "DATE" && filter.parameterValue[0].value) {
              filter.parameterValue[0].value = new Date(
                filter.parameterValue[0].value
              );
            }
          });
          this.setFiltersForBreadcrumbItem();
      ...
      await this.$http
        ...
        .then(
          (response: AxiosResponse<any>) => (this.filtersData = response.data)
        )
        .catch((error: any) => {
      ...
      this.filtersData?.filterStatus?.forEach((el: iParameter) => {
        el.parameterValue = el.multivalue
          ? []
          : [{ value: "", description: "" }];
        if (el.driverDefaultValue?.length > 0) {
          ...
          if (el.metadata?.colsMap) {
            valueIndex = Object.keys(el.metadata?.colsMap).find(
              (key: string) =>
                el.metadata.colsMap[key] === el.metadata.valueColumn
            ) as any;
            descriptionIndex = Object.keys(el.metadata?.colsMap).find(
              (key: string) =>
                el.metadata.colsMap[key] === el.metadata.descriptionColumn
            ) as any;
          }
          ...
          if (
            el.type === "DATE" &&
            !el.selectionType &&
            el.valueSelection === "man_in" &&
            el.showOnPanel === "true" &&
            el.visible
          ) {
            el.parameterValue[0].value = moment(
              el.parameterValue[0].description?.split("#")[0]
            ).toDate() as any;
          }
        }
        if (el.data) {
        ...
        if (
          (el.selectionType === "COMBOBOX" || el.selectionType === "LIST") &&
          el.multivalue &&
          el.mandatory &&
          el.data.length === 1
        ) {
        ...
        if (el.parameterValue[0] && !el.parameterValue[0].description) {
          el.parameterValue[0].description = el.parameterDescription
            ? el.parameterDescription[0]
            : "";
        }
      });
      ...
      Object.keys(this.document.navigationParams).forEach((key: string) => {
        for (let i = 0; i < this.filtersData.filterStatus.length; i++) {
          ...
          if (key === tempParam.urlName || key === tempParam.label) {
            tempParam.parameterValue[0].value = this.document.navigationParams[
              key
            ];
            if (
              this.document.navigationParams[key + "_field_visible_description"]
            )
              tempParam.parameterValue[0].description = this.document.navigationParams[
                key + "_field_visible_description"
              ];
            if (tempParam.selectionType === "COMBOBOX")
              this.setCrossNavigationComboParameterDescription(tempParam);
            if (
              tempParam.type === "DATE" &&
              tempParam.parameterValue[0] &&
              tempParam.parameterValue[0].value
            ) {
              tempParam.parameterValue[0].value = new Date(
                tempParam.parameterValue[0].value
              );
            }
          }
    ...
    setCrossNavigationComboParameterDescription(tempParam: any) {
      if (tempParam.parameterValue[0]) {
        const index = tempParam.data.findIndex(
          (option: any) => option.value === tempParam.parameterValue[0].value
        );
        if (index !== -1)
          tempParam.parameterValue[0].description =
            tempParam.data[index].description;
      }
    },
    formatParameterDataOptions(parameter: iParameter, data: any) {
      ...
      const valueIndex = Object.keys(parameter.metadata.colsMap).find(
        (key: string) => parameter.metadata.colsMap[key] === valueColumn
      );
      const descriptionIndex = Object.keys(parameter.metadata.colsMap).find(
    ...
    async loadExporters() {
      await this.$http
        .get(
          process.env.VUE_APP_RESTFUL_SERVICES_PATH +
            `2.0/exporters/${this.urlData?.engineLabel}`
        )
        .then(
    ...
    async sendForm(documentLabel: string | null = null) {
      let tempIndex = this.breadcrumbs.findIndex(
        (el: any) => el.label === this.document.label
      ) as any;
      ...
      for (let i in paramsFromUrl) {
        if (typeof paramsFromUrl !== "function") {
          postObject.params[paramsFromUrl[i].split("=")[0]] = paramsFromUrl[
            i
          ].split("=")[1];
        }
      }
      ...
      postForm.target =
        tempIndex !== -1 ? "documentFrame" + tempIndex : documentLabel;
    ...
    async onExportCSV() {
      ...
      await this.$http
        .post(
          process.env.VUE_APP_RESTFUL_SERVICES_PATH + `2.0/export/cockpitData`,
          postData
        )
        .then(() => {
    ...
    async getRank() {
      ...
      await this.$http
        .post(
          process.env.VUE_APP_RESTFUL_SERVICES_PATH + `documentrating/getvote`,
    ...
    async onSaveRank(newRank: any) {
      if (newRank) {
        ...
        await this.$http
          .post(
            process.env.VUE_APP_RESTFUL_SERVICES_PATH + `documentrating/vote`,
    ...
    getFormattedDate(date: any, useDefaultFormat?: boolean) {
      ...
      return luxonFormatDate(
        date,
        format,
        useDefaultFormat ? undefined : this.dateFormat
      );
    },
    ...
    findCrossTargetByCrossName(angularData: any, temp: any[]) {
      ...
      const index = temp.findIndex(
        (el: any) =>
          el.crossName === angularData.targetCrossNavigation.crossName
      );
    ...
    async loadCrossNavigation(crossNavigationDocument: any, angularData: any) {
      ...
      const navigationParams = this.formatNavigationParams(
        angularData.otherOutputParameters,
        crossNavigationDocument ? crossNavigationDocument.navigationParams : []
      );

      const popupOptions = crossNavigationDocument?.popupOptions
        ? JSON.parse(crossNavigationDocument.popupOptions)
        : null;
      ...
      if (crossNavigationDocument?.crossType === 2) {
        this.openCrossNavigationInNewWindow(
          popupOptions,
          crossNavigationDocument,
          navigationParams
        );
        ...
        if (index !== -1) {
          ...
          this.breadcrumbs.push({
            ...
            document: this.document,
            crossBreadcrumb:
              crossNavigationDocument.crossBreadcrumb ?? this.document.name,
          });
        }
    ...
    openCrossNavigationInNewWindow(
      ...
      window.open(
        url,
        "_blank",
        `toolbar=0,status=0,menubar=0,width=${popupOptions.width ||
          "800"},height=${popupOptions.height || "600"}`
      );
    },
    formatAngularOutputParameters(otherOutputParameters: any[]) {
      const startDocumentInputParameters = deepcopy(this.document.drivers);
      const keys = [] as any[];
      otherOutputParameters.forEach((parameter: any) =>
        keys.push(Object.keys(parameter)[0])
      );
      ...
      for (let i = 0; i < startDocumentInputParameters.length; i++) {
        if (!keys.includes(startDocumentInputParameters[i].label)) {
          ...
          tempObject[
            startDocumentInputParameters[i].label
          ] = this.getParameterValueForCrossNavigation(
            startDocumentInputParameters[i].label
          );
    ...
    getParameterValueForCrossNavigation(parameterLabel: string) {
      ...
      const index = this.filtersData.filterStatus?.findIndex(
        (param: any) => param.label === parameterLabel
      );
      return index !== -1
        ? this.filtersData.filterStatus[index].parameterValue[0].value
        : "";
    },
    formatNavigationParams(
      otherOutputParameters: any[],
      navigationParams: any
    ) {
      ...
      otherOutputParameters.forEach((el: any) => {
        ...
        for (let i = 0; i < Object.keys(navigationParams).length; i++) {
          if (
            navigationParams[Object.keys(navigationParams)[i]].value.label ===
            Object.keys(el)[0]
          ) {
            found = true;
        ...
        if (found) {
          ...
          formatedParams[label + "_field_visible_description"] =
            el[Object.keys(el)[0]];
        }
      });

      this.setNavigationParametersFromCurrentFilters(
        formatedParams,
        navigationParams
      );
    ...
    setNavigationParametersFromCurrentFilters(
      ...
      const formattedParametersKeys = formattedParameters
        ? Object.keys(this.getFormattedParameters())
        : [];
      if (
        ...
        for (let i = 0; i < navigationParamsKeys.length; i++) {
          const index = formattedParametersKeys.findIndex(
            (key: string) => key === navigationParamsKeys[i]
          );
          if (index !== -1) {
    ...
    isOrganizerEnabled() {
      return (
        ...
        this.user.functionalities.includes("SaveIntoFolderFunctionality")
      );
    },
    ...
    async loadUserConfig() {
      await this.$http
        .get(process.env.VUE_APP_RESTFUL_SERVICES_PATH + `1.0/user-configs`)
        .then((response: AxiosResponse<any>) => {
          if (response.data) {
            this.sessionEnabled =
              response.data["SPAGOBI.SESSION_PARAMETERS_MANAGER.enabled"] ===
              "false"
                ? false
                : true;
            this.dateFormat =
              response.data["SPAGOBI.DATE-FORMAT-SERVER.format"] === "%Y-%m-%d"
                ? "dd/MM/yyyy"
                : response.data["SPAGOBI.DATE-FORMAT-SERVER.format"];
          }
        });
    },
