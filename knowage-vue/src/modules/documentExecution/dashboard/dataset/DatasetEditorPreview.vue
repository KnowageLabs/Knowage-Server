<template>
    <div class="p-d-flex p-flex-column p-ai-stretch p-jc-center kn-overflow">
        <div class="preview-table-container">
            <Toolbar class="kn-toolbar kn-toolbar--secondary p-col-12" :style="mainDescriptor.style.maxWidth">
                <template #start>
            ...
            <div class="p-d-flex p-flex-column kn-flex workspace-scrollable-table">
                ...
                <KnParameterSidebar
                    ...
                    :prop-q-b-e-parameters="dataset.parameters"
                    :user-role="userRole"
                    :dataset="dataset"
...
export default defineComponent({
    ...
    components: { DatasetPreviewTable, Message, KnParameterSidebar },
    props: { propDataset: { type: Object }, previewType: String, loadFromDatasetManagement: Boolean },
    emits: ['close'],
    setup() {
    ...
    computed: {
        isParameterSidebarVisible(): boolean {
            ...
            if (this.dataset && this.dataset.parameters) {
                if (this.dataset.parameters.length !== 0) {
                    parameters = this.dataset.parameters.length !== 0
                }
            }
            return parameterVisible || parameters
        }
    },
    watch: {
        propDataset: {
    ...
    methods: {
        async loadPreview() {
            ...
            if (this.dataset.label && this.dataset.parameters.length === 0 && (this.filtersData.isReadyForExecution === undefined || this.filtersData.isReadyForExecution)) {
        ...
        async loadPreviewData() {
            ...
            if (this.dataset.parameters.length > 0) {
                postData.pars = [...this.dataset.parameters]
            }
            if (this.filtersData.filterStatus?.length > 0) {
                postData.drivers = this.formatDriversForPreviewData()
            }
        ...
        async onExecute(datasetParameters: any[]) {
            if (!this.dataset.parameters) this.dataset.parameters = []
            this.dataset.parameters = datasetParameters
        ...
        async onRoleChange(role: string) {
            ...
            if (!hasError && this.dataset.label && this.dataset.parameters.length === 0 && (this.filtersData.isReadyForExecution === undefined || this.filtersData.isReadyForExecution)) {
                this.loadFromDatasetManagement ? await this.loadPreSavePreview() : await this.loadPreviewData()
                this.parameterSidebarVisible = false
            } else {
                this.parameterSidebarVisible = true
