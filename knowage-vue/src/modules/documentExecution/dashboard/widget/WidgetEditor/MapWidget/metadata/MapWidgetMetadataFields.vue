<template>
    <div class="p-grid p-jc-center p-ai-center p-p-4">
        ...
        <div v-for="(field, index) in fields" :key="index" class="dynamic-form-item p-grid p-col-12 p-ai-center">
            <div class="p-grid p-ai-center p-mt-2">
                <div class="p-col-12 p-d-flex p-flex-row">
                    ...
                    <div class="p-d-flex p-flex-row p-jc-center p-ai-center p-ml-3 p-mb-2">
                        <i class="pi pi-trash kn-cursor-pointer" @click="removeField(index)"></i>
                    </div>
                </div>

                <div class="p-col-12 p-d-flex p-flex-row">
                    <span class="p-field p-col-12 p-lg-6 p-jc-center p-mt-3 p-pl-3">
                        <InputSwitch v-model="field.properties.aggregateBy" />
                        <label class="kn-material-input-label p-ml-3"> {{ $t('dashboard.widgetEditor.map.metadata.aggregateBy') }} </label>
                    ...
                    <span v-if="field.fieldType === 'MEASURE'" class="p-field p-col-12 p-lg-6 p-jc-center p-mt-3 p-pl-3">
                        ...
                        <label class="kn-material-i47nput-label p-ml-3"> {{ $t('dashboard.widgetEditor.map.metadata.showOnMap') }} </label>
                    </span>

                    <span v-if="field.fieldType === 'ATTRIBUTE'" class="p-field p-col-12 p-lg-6 p-jc-center p-mt-3 p-pl-3">
                        <InputSwitch v-model="field.properties.showFilter" />
                        <label class="kn-material-input-label p-ml-3"> {{ $t('dashboard.widgetEditor.map.metadata.showOnFilters') }} </label>
                    </span>

                    <span class="p-field p-col-12 p-lg-6 p-jc-center p-mt-3 p-pl-3">
                        <InputSwitch v-model="field.properties.modal" />
                        <label class="kn-material-input-label p-ml-3"> {{ $t('common.modal') }} </label>
