<template>
  {{ chartModel?.yAxis?.min }}
  {{ chartModel?.yAxis?.max }}
  <div
    v-show="!error"
    :id="chartID"
...
export default defineComponent({
  ...
  methods: {
    ...
    onRefreshChart() {
      this.chartModel = this.widgetModel.settings.chartModel
        ? this.widgetModel.settings.chartModel.model
        : null;
    ...
    updateChartModel() {
      ...
      // TODO - Uncomment
      this.widgetModel.settings.chartModel.setData(
        this.dataToShow,
        this.widgetModel
      );
      ...
      this.widgetModel.settings.chartModel.updateSeriesAccessibilitySettings(
        this.widgetModel
      );
      this.widgetModel.settings.chartModel.updateSeriesLabelSettings(
        this.widgetModel
      );
      ...
      this.widgetModel.settings.chartModel.updateChartColorSettings(
        this.widgetModel
      );
      ...
      this.highchartsInstance = Highcharts.chart(
        this.chartID,
        modelToRender as any
      );
      // TODO
    ...
    updateLegendSettings() {
      if (this.chartModel.plotOptions.pie)
        this.chartModel.plotOptions.pie.showInLegend = true;
      if (this.chartModel.plotOptions.gauge)
        this.chartModel.plotOptions.gauge.showInLegend = true;
      return this.widgetModel.settings.chartModel.updateFormatterSettings(
        this.chartModel.legend,
        "labelFormat",
        "labelFormatter",
        "labelFormatterText",
        "labelFormatterError"
    ...
    updateDataLabels() {
      console.log(
        ">>>>>>> CHART MODEL PLOT OPTIONS: ",
        this.chartModel.plotOptions
      );
      console.log(
        ">>>>>>> this.chartModel.chart.type: ",
        this.chartModel.chart.type
      );
      const dataLabels =
        this.chartModel.plotOptions &&
        this.chartModel.plotOptions[this.chartModel.chart.type]
          ? this.chartModel.plotOptions[this.chartModel.chart.type].dataLabels
          : null;
      if (dataLabels) {
        this.error =
          this.widgetModel.settings.chartModel.updateFormatterSettings(
            dataLabels,
            "format",
            "formatter",
            "formatterText",
    ...
    updateTooltipSettings() {
      ...
      hasError = this.widgetModel.settings.chartModel.updateFormatterSettings(
        this.chartModel.tooltip,
        null,
        "pointFormatter",
        "pointFormatterText",
    ...
    setSelection(event: any) {
      if (
        this.editorMode ||
        !this.widgetModel.settings.interactions.selection ||
        !this.widgetModel.settings.interactions.selection.enabled
      )
        return;
      const serieClicked = event.point?.options;
      if (!serieClicked || !serieClicked.name) return;
      updateStoreSelections(
        this.createNewSelection([serieClicked.name]),
    ...
    createNewSelection(value: (string | number)[]) {
      const attributeColumn = this.widgetModel.columns.find(
        (column: IWidgetColumn) => column.fieldType === "ATTRIBUTE"
      );
      const selection = {
