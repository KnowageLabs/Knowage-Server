                    ...
                    <template #content>
                        <div class="p-d-flex p-flex-column">
                            ...
                            <div class="kn-flex p-mt-3" v-if="documentsAvailable">
                                <Tree id="document-tree" :value="nodes" selectionMode="checkbox" v-model:selectionKeys="selectedDocuments" :filter="true" filterMode="lenient" @node-expand="setOpenFolderIcon($event)" @node-collapse="setClosedFolderIcon($event)"></Tree>
                            </div>
                        </div>
                    </template>
                </Card>
    ...
    methods: {
        ...
        async loadDocumentSelection() {
            ...
            for (let i = 0; i < this.nodes.length; i++) {
                this.filterDocuments(this.nodes[i], null)
            }
            this.loading = false
            ...
            await axios.get(process.env.VUE_APP_RESTFUL_SERVICES_PATH + '2.0/folders?includeDocs=true').then((response) => {
                ...
                this.folderStructure.map((folder: any) => {
                    ...
                    folder.biObjects.map((document: iFile) => {
                        ...
                        console.log(document.name + ': index: ' + index)
                        if (index >= 0) {
                            tempFolder.data.hasDocuments = true
                            tempFolder.children.push({ key: document.id, icon: 'pi pi-file', id: document.id, label: document.name, children: [], data: document.name, exportable: false })
                        }
                    })
        ...
        filterDocuments(folder, parentFolder) {
            if (folder.data.hasDocuments) {
                ...
                if (parentFolder && folder.parentId) {
                    const index = parentFolder.children.findIndex((el: any) => el.id === folder.id)
                    console.log('CALLED FOR', folder.label)
