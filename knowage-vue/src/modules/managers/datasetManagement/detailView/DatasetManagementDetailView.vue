...
export default defineComponent({
    ...
    methods: {
        ...
        async saveDataset() {
            ...
            if (this.user?.functionalities?.includes('DataPreparation') && dsToSave.id) {
                await this.$http
                    ...
                    .catch((err) => {
                        this.$store.commit('setError', { title: this.$t('common.toast.errorTitle'), msg: this.$t(err.data.errors[0].message) })
                    })
            } else {
                this.checkDerived(dsToSave)
            }
        ...
        async checkDerived(dsToSave) {
            await this.$http
                .get(process.env.VUE_APP_RESTFUL_SERVICES_PATH + '1.0/datasets/dataset/' + dsToSave.label + '/derived', { headers: { 'X-Disable-Interceptor': 'true' } })
                .then((response: AxiosResponse<any>) => {
                    if (response.data) {
                        this.$confirm.require({
                            icon: 'pi pi-exclamation-triangle',
