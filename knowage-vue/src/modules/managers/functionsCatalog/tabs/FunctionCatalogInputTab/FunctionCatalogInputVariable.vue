<template>
    <div v-if="inputVariable" class="p-d-flex p-flex-row p-ai-center">
        <div class="variable-name-input kn-flex ">
            <span class="p-float-label">
                <InputText class="kn-material-input" v-model.trim="inputVariable.name" :disabled="readonly" />
        ...
        <div class="p-field kn-flex p-m-2">
            <span>
                ...
                <Dropdown class="kn-material-input" v-model="inputVariable.type" :options="functionCatalogInputTabDescriptor.variableTypes" optionLabel="value" optionValue="value" :disabled="readonly" @change="onTypeChange" />
            </span>
        </div>
        <div class="p-field kn-flex">
            <label class="kn-material-input-label">{{ $t('managers.functionsCatalog.variableDefaultValue') }}</label>
            <InputText v-if="inputVariable.type !== 'DATE'" :type="inputVariable.type === 'NUMBER' ? 'number' : 'text'" class="kn-material-input" v-model.trim="inputVariable.value" :disabled="readonly" />
...
export default defineComponent({
    ...
    methods: {
        ...
        deleteVariableConfirm() {
            // console.log('deleteVariableConfirm() event: ', inputVariable)
