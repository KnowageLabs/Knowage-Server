...
export default defineComponent({
    ...
    methods: {
        ...
        getCategoryTagsUrl() {
            switch (this.propCategory.type) {
                case 'DATASET_CATEGORY':
                    return process.env.VUE_APP_RESTFUL_SERVICES_PATH + `3.0/category/dataset/${this.propCategory.id}`
                case 'BM_CATEGORY':
                    return process.env.VUE_APP_RESTFUL_SERVICES_PATH + `3.0/category/metamodel/${this.propCategory.id}`
                case 'GEO_CATEGORY':
                    return process.env.VUE_APP_RESTFUL_SERVICES_PATH + `3.0/category/geolayer/${this.propCategory.id}`
                case 'KPI_KPI_CATEGORY':
                    return process.env.VUE_APP_RESTFUL_SERVICES_PATH + `3.0/category/kpi/${this.propCategory.id}`
                case 'KPI_TARGET_CATEGORY':
                    return process.env.VUE_APP_RESTFUL_SERVICES_PATH + `3.0/category/kpitarget/${this.propCategory.id}`
                case 'KPI_MEASURE_CATEGORY':
                    return process.env.VUE_APP_RESTFUL_SERVICES_PATH + `3.0/category/kpiruleoutput/${this.propCategory.id}`
            }
        },
        async handleSubmit() {
        ...
        sendRequest() {
            if (this.operation === 'insert') return this.$http.post(process.env.VUE_APP_RESTFUL_SERVICES_PATH + '3.0/category', this.category)
            else {
                delete this.category.occurrences
                return this.$http.post(process.env.VUE_APP_RESTFUL_SERVICES_PATH + '3.0/category/update', this.category)
            }
        },
        closeTemplate() {
            this.$emit('close')
