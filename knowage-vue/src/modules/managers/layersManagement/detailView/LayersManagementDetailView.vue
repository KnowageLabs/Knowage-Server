    ...
    methods: {
        ...
        async getRolesForLayer() {
            if (this.layer.layerId) {
                await this.$http.post(import.meta.env.VUE_APP_RESTFUL_SERVICES_PATH + `layers/postitem`, this.selectedLayer).then((response: AxiosResponse<any>) => (this.layer.roles = response.data))
            }
        },
        ...
        async loadFilters() {
            if (this.layer) {
                this.loading = true
                await this.$http.get(import.meta.env.VUE_APP_RESTFUL_SERVICES_PATH + `layers/getFilter?id=${this.layer.layerId}`).then((response: AxiosResponse<any>) => (this.filters = response.data))
                this.loading = false
            }
        },
        saveOrUpdateMessage(layer) {
            let toSend = layer
            let url = import.meta.env.VUE_APP_RESTFUL_SERVICES_PATH + 'layers'
            if (this.layer.layerFile) {
                ...
                if (layer.layerId) {
                    url = import.meta.env.VUE_APP_RESTFUL_SERVICES_PATH + 'layers/updateData'
                } else url = import.meta.env.VUE_APP_RESTFUL_SERVICES_PATH + 'layers/addData'
                toSend = formData
            }
            if (layer.layerId) {
