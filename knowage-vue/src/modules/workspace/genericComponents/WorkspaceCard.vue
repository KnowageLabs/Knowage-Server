    ...
    computed: {
        ...
        documentImageSource(): any {
            if (this.document[this.documentFields.image]) {
                return {
                    'background-image': `url(${import.meta.env.VUE_APP_HOST_URL}${descriptor.imgPath}${this.document[this.documentFields.image]}),url(${require('@/assets/images/workspace/documentTypes/' + cardDescriptor.defaultImages.missing)})`
                }
            }
            return {
