<template>
    <div>
        <div class="p-d-flex p-flex-row p-m-2">
            <div class="p-m-2">
                <Checkbox v-model="document.saveassnapshot" :binary="true" data-test="snapshot-checkbox" @change="removeProperty('saveassnapshot')" />
                <span class="p-ml-2">{{ $t('managers.scheduler.saveAsSnapshot') }}</span>
            </div>
            <div class="p-m-2">
                <Checkbox v-model="document.saveasfile" :binary="true" data-test="file-checkbox" @change="removeProperty('saveasfile')" />
                <span class="p-ml-2">{{ $t('managers.scheduler.saveAsFile') }}</span>
            </div>
            <div class="p-m-2">
                <Checkbox v-model="document.saveasdocument" :binary="true" data-test="document-checkbox" @change="removeProperty('saveasdocument')" />
                <span class="p-ml-2">{{ $t('managers.scheduler.saveAsDocument') }}</span>
            </div>
            <div class="p-m-2">
                <Checkbox v-model="document.sendtojavaclass" :binary="true" data-test="java-checkbox" @change="removeProperty('sendtojavaclass')" />
                <span class="p-ml-2">{{ $t('managers.scheduler.sendToJavaClass') }}</span>
            </div>
            <div class="p-m-2">
                <Checkbox v-model="document.sendmail" :binary="true" data-test="mail-checkbox" @change="removeProperty('sendmail')" />
                <span class="p-ml-2">{{ $t('managers.scheduler.sendMail') }}</span>
            </div>
        </div>
        <div v-if="document">
            <SchedulerSnapshotAccordion v-if="document.saveassnapshot" class="p-m-3" :prop-document="document" data-test="snapshot-accordion"></SchedulerSnapshotAccordion>
            <SchedulerFileAccordion v-if="document.saveasfile" class="p-m-3" :prop-document="document" data-test="file-accordion"></SchedulerFileAccordion>
            <SchedulerDocumentAccordion v-if="document.saveasdocument" class="p-m-3" :prop-document="document" :functionalities="functionalities" :datasets="datasets" :job-info="jobInfo" data-test="document-accordion"></SchedulerDocumentAccordion>
            <SchedulerJavaClassAccordion v-if="document.sendtojavaclass" class="p-m-3" :prop-document="document" data-test="java-accordion"></SchedulerJavaClassAccordion>
            <SchedulerMailAccordion v-if="document.sendmail" class="p-m-3" :prop-document="document" :datasets="datasets" :job-info="jobInfo" data-test="mail-accordion"></SchedulerMailAccordion>
        </div>
    </div>
</template>
