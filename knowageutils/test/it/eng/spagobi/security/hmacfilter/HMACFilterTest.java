...
public class HMACFilterTest {
	...
	public void testDoFilter() throws IOException, NoSuchAlgorithmException, HMACSecurityException {
		...
		headers.put(HMACUtils.HMAC_SIGNATURE_HEADER, getSignature("/hmac", "a=b", "", token));
		...
		// test success post
		headers.put(HMACUtils.HMAC_TOKEN_HEADER, token);
		headers.put(HMACUtils.HMAC_SIGNATURE_HEADER, getSignature("/hmac", "", body, token));
		...
		headers.put(HMACUtils.HMAC_SIGNATURE_HEADER, getSignature("/hmac", "g=h&y=i", body, token));
		RestUtilities.makeRequest(HttpMethod.Post, "http://localhost:8080/hmac?g=h&y=i", headers, body);
		Assert.assertTrue(DummyServlet.arrived);
		DummyServlet.arrived = false;

		// test fail post
		headers.put(HMACUtils.HMAC_SIGNATURE_HEADER, "i0pe5");
		RestUtilities.makeRequest(HttpMethod.Post, "http://localhost:8080/hmac?g=h&y=i", headers, body);
	...
	public void testDoFilterDelete() throws IOException, NoSuchAlgorithmException, HMACSecurityException {
		...
		headers.put(HMACUtils.HMAC_SIGNATURE_HEADER, getSignature("/hmac", "g=h&y=i", "", token));
