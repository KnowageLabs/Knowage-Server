 ...
 */
public class JDBCRedShiftDataSet extends AbstractJDBCDataset {

	private static transient Logger logger = Logger.getLogger(JDBCRedShiftDataSet.class);

	/**
	 * Instantiates a new empty JDBC RedShift data set.
	 */
	public JDBCRedShiftDataSet() {
		super();
		setDataProxy(new JDBCRedShiftDataProxy());
		setDataReader(createDataReader());
	}

	public JDBCRedShiftDataSet(SpagoBiDataSet dataSetConfig) {
		super(dataSetConfig);
	}
