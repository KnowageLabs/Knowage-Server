 ...
 */
public class JDBCSpannerDataSet extends AbstractJDBCDataset {

	private static Logger logger = Logger.getLogger(JDBCSpannerDataSet.class);

	/**
	 * Instantiates a new empty JDBC RedShift data set.
	 */
	public JDBCSpannerDataSet() {
		super();
		setDataProxy(new JDBCSpannerDataProxy());
		setDataReader(createDataReader());
	}

	public JDBCSpannerDataSet(SpagoBiDataSet dataSetConfig) {
		super(dataSetConfig);
	}
