...
public class ScriptDataSet extends ConfigurableDataSet {
	...
	public SpagoBiDataSet toSpagoBiDataSet() {
		 ...
		 * JSONObject(); jsonConf.put(SCRIPT, getScript() ); jsonConf.put(SCRIPT_LANGUAGE, getScriptLanguage() ); sbd.setConfiguration(jsonConf.toString());
		 * }catch (Exception e){ logger.error("Error while defining dataset configuration.  Error: " + e.getMessage()); } //sbd.setScript( getScript() );
	...
	public ScriptDataProxy getDataProxy() {
		...
		if (!(dataProxy instanceof ScriptDataProxy))
