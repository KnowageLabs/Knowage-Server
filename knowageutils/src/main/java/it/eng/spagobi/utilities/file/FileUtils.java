...
public class FileUtils {
	...
	public static void doForEach(File rootDir, IFileTransformer transformer) {
		...
		Assert.assertTrue(rootDir.exists() && rootDir.isDirectory(),
				"rootDir parameter [" + rootDir + "] is not an existing directory");
	...
	/**
	 * Checks if upload file: - is not empty; - does not exceed specified max size; - has one of the specified extensions. A SpagoBIRuntimeException is thrown in
	 * case one of the conditions is not satisfied.
	 *
	...
	public static void checkUploadedFile(FileItem uploaded, Integer maxSize, List<String> admissibleFilesExtension) {
		...
		if (admissibleFilesExtension != null) {
			...
			if (!admissibleFilesExtension.contains(fileExtension.toLowerCase())
					&& !admissibleFilesExtension.contains(fileExtension.toUpperCase())) {
		...
		try {
			...
			if (!toReturn.exists()) {
				...
				if (!mkdirResult) {
					...
					throw new SpagoBIRuntimeException(
							"Cannot create directory " + directory + " into server resources");
				}
			}
		} catch (Throwable t) {
	...
	/**
	 * Throws a SecurityException if a path traversal attack is detected. See
	 * https://wiki.spagobi.org/xwiki/bin/view/spagobi_standard_dev/Sicurezza?srid=TP2i4TZR#HPathtraversal
	 *
	...
	public static void checkPathTraversalAttack(String fileName, File targetDirectory) {
		...
		String completeFileName = targetDirectory.getAbsolutePath() + File.separator + fileName;
		File file = new File(completeFileName);
		File parent = file.getParentFile();
		// Prevent directory traversal (path traversal) attacks
		if (!targetDirectory.equals(parent)) {
			logger.error("Trying to access the file [" + file.getAbsolutePath() + "] that is not inside ["
