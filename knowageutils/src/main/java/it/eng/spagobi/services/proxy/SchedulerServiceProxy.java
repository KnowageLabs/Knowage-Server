...
public final class SchedulerServiceProxy extends AbstractServiceProxy {
	...
	private static final String SERVICE_NAME = "Scheduler Service";

	private static final QName SERVICE_QNAME = new QName("http://scheduler.services.spagobi.eng.it/", "SchedulerService");

	private static Logger logger = Logger.getLogger(SchedulerServiceProxy.class);
	...
	private SchedulerService lookUp() throws SecurityException {
		try {
			...
			if (serviceUrl != null) {
				...
				service = Service.create(serviceUrlWithWsdl, SERVICE_QNAME).getPort(SchedulerService.class);
			} else {
