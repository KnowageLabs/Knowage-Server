	...
	private void completeJWT(DecodedJWT jwtToken, Builder builder) {
		...
		if (jwtClaimsMap != null) {
			...
			for (int i = 0; i < listClaimsMap.length; i++) {
				...
				if (valoreObj instanceof String) {
					builder.withClaim(nameValueMap, (String) map.get(nameValueMap));
				} else if (valoreObj instanceof List) {
					@SuppressWarnings("unchecked")
					List<?> valore = (List<String>) valoreObj;
