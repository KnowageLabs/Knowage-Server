...
public class FileContentValidator {
	 ...
	 * @throws SpagoBIRuntimeException
  if validation fails
	 */
	public static InputStream validateFileContent(InputStream inputStream, String fileName) throws IOException {
		if (inputStream == null || fileName == null) {
			throw new SpagoBIRuntimeException ("Invalid parameters: inputStream and fileName cannot be null");
		}
		...
		try {
			...
			throw new SpagoBIRuntimeException("Invalid or corrupted file: " + e.getMessage(), e);
		}
	...
	private static void validateXML(byte[] content) throws Exception {
		if (content.length == 0) {
			throw new SpagoBIRuntimeException("File is empty");
		}
		...
		if (!trimmedHeader.startsWith("<?xml") && !trimmedHeader.startsWith("<")) {
			throw new SpagoBIRuntimeException("File does not appear to be valid XML (missing XML header or root element)");
		}
		...
		try {
			...
			throw new SpagoBIRuntimeException("XML file is corrupted or malformed: " + e.getMessage());
		}
	}
	...
	private static void validateJSON(byte[] content) {
		...
		if (!preview.startsWith("{") && !preview.startsWith("[")) {
			throw new SpagoBIRuntimeException("File does not appear to be valid JSON (must start with { or [)");
		}
		...
		try {
			...
			try {
				...
				throw new SpagoBIRuntimeException("JSON file is corrupted or malformed: " + je.getMessage());
			}
		}
	}
	...
	private static void validateCSV(byte[] content) throws Exception {
		...
		if (startsWithSignature(content, ZIP_SIGNATURE) || startsWithSignature(content, OLE2_SIGNATURE)) {
			throw new SpagoBIRuntimeException("File appears to be binary (ZIP/XLS) but declared as CSV");
		}
		...
		try (InputStreamReader reader = new InputStreamReader(new ByteArrayInputStream(content), StandardCharsets.UTF_8);
			...
			if (header == null || header.length == 0) {
				throw new SpagoBIRuntimeException("CSV file has no valid header");
			}
			...
			throw new SpagoBIRuntimeException("CSV file is corrupted or malformed: " + e.getMessage());
		}
	}
	...
	private static void validateXLS(byte[] content) throws Exception {
		...
		if (!startsWithSignature(content, OLE2_SIGNATURE)) {
			throw new SpagoBIRuntimeException("File does not have valid XLS signature (expected OLE2 format)");
		}
		...
		try (HSSFWorkbook workbook = new HSSFWorkbook(new ByteArrayInputStream(content))) {
			...
			throw new SpagoBIRuntimeException("XLS file is corrupted or cannot be opened: " + e.getMessage());
		}
	}
	...
	private static void validateXLSX(byte[] content) throws Exception {
		...
		if (!startsWithSignature(content, ZIP_SIGNATURE)) {
			throw new SpagoBIRuntimeException("File does not have valid XLSX signature (expected ZIP format)");
		}
		...
		try (XSSFWorkbook workbook = new XSSFWorkbook(new ByteArrayInputStream(content))) {
			...
			throw new SpagoBIRuntimeException("XLSX file is corrupted or cannot be opened: " + e.getMessage());
		}
	}
