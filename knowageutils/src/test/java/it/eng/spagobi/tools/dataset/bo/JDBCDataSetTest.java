...
//			String conf = "{\"query\":\"SELECT catgroup FROM category LIMIT 10 \",\"queryScript\":\"\",\"queryScriptLanguage\":\"\",\"dataSource\":\"redshift_postgre\"}";
			String conf = "{\"query\":\"select d.YEAR,d.MONTH,d.WEEK,d.CALDATE,e.eventname, v.VENUESTATE,v.VENUECITY, qtysold, pricepaid,v.VENUESEATS\r\n"
					+ "from table_select s \r\n" + "inner join event e on e.eventid = s.eventid\r\n" + "inner join venue v on e.venueid = v.venueid\r\n"
					+ "inner join date d on e.dateid = d.dateid \",\"queryScript\":\"\",\"queryScriptLanguage\":\"\",\"dataSource\":\"redshift_postgre\"}";

			dataSetConfig.setConfiguration(conf);
			// dataSetConfig.setQuery("SELECT fullname as 'Full Name' FROM CUSTOMER LIMIT 10");

			SpagoBiDataSource ds = TestDataSetFactory.createSpagoBiDataSource();
			ds.setJdbcPoolConfiguration((String) new JDBCDataSourcePoolConfigurationJSONSerializer().serialize(new JDBCDataSourcePoolConfiguration()));
			dataSetConfig.setDataSource(ds);
