...
public class LightNavigationManager {
	...
	public static final String LIGHT_NAVIGATOR_ID = LightNavigationConstants.LIGHT_NAVIGATOR_ID;
	...
	public static SourceBean controlLightNavigation(Object request, SourceBean serviceRequest) throws Exception {
		...
		if (request == null) {
			TracerSingleton.log(Constants.NOME_MODULO, TracerSingleton.CRITICAL,
					"LightNavigationManager: " + "controlLightNavigation: request object at input is null.");
			throw new Exception("request object is null.");
		}
		if (serviceRequest == null) {
			TracerSingleton.log(Constants.NOME_MODULO, TracerSingleton.CRITICAL,
					"LightNavigationManager: " + "controlLightNavigation: the service request SourceBean at input is null.");
		...
		try {
			...
			if ("true".equalsIgnoreCase(lightNavigatorDestroy)) {
				TracerSingleton.log(Constants.NOME_MODULO, TracerSingleton.DEBUG,
						"LightNavigationManager: controlLightNavigation: " + "Destroying light navigator.");
			...
			if ("true".equalsIgnoreCase(lightNavigatorDisabled)) {
				TracerSingleton.log(Constants.NOME_MODULO, TracerSingleton.DEBUG,
						"LightNavigator disabled; " + "return from the method controlNavigation without any modifications to the original request.");
			...
			if (lightNavigatorBackTo != null) {
				...
				TracerSingleton.log(Constants.NOME_MODULO, TracerSingleton.DEBUG,
						"LightNavigationManager: controlLightNavigation: " + "returning request at position " + position + " of the stack.");
			...
			if (lightNavigatorBackToMark != null) {
				...
				TracerSingleton.log(Constants.NOME_MODULO, TracerSingleton.DEBUG,
						"LightNavigationManager: controlLightNavigation: " + "returning request with mark " + lightNavigatorBackToMark + " of the stack.");
				TracerSingleton.log(Constants.NOME_MODULO, TracerSingleton.DEBUG, "The requests stack now is:\n" + lightNavigator.toString());
				return markedRequest.getRequest();
			}
			...
			TracerSingleton.log(Constants.NOME_MODULO, TracerSingleton.MAJOR,
					"LightNavigationManager: " + "controlLightNavigation: an exception occurred. " + "Returning the original request.", e);
		...
		if (request instanceof PortletRequest) {
			...
			if (lightNavigatorObj != null) {
				if (lightNavigatorObj instanceof LightNavigator) {
					TracerSingleton.log(Constants.NOME_MODULO, TracerSingleton.DEBUG,
							"LightNavigationManager: " + "retrieveLightNavigatorFromSession: LightNavigator retrieved from session correctly.");
					lightNavigator = (LightNavigator) lightNavigatorObj;
				} else {
					TracerSingleton.log(Constants.NOME_MODULO, TracerSingleton.CRITICAL,
							"LightNavigationManager: " + "retrieveLightNavigatorFromSession: session attribute with key = [" + sessionAttributeKey
									+ "] is not a " + "LightNavigator object!! Cannot proceed!!");
					throw new Exception("Session attribute with key = [" + sessionAttributeKey + "] is not a LightNavigator object!!");
				}
			} else {
				TracerSingleton.log(Constants.NOME_MODULO, TracerSingleton.DEBUG,
						"LightNavigationManager: " + "retrieveLightNavigatorFromSession: LightNavigator not existing. Creating a new one and putting it "
	...
	private static String getSessionAttributeKey(HttpServletRequest servRequest) {
		...
		TracerSingleton.log(Constants.NOME_MODULO, TracerSingleton.DEBUG,
				"LightNavigationManager: " + "getSessionAttributeKey: LIGHT_NAVIGATOR_ID parameter in httprequest is " + navigatorId);
	...
	private static void destroyLightNavigatorFromSession(Object request) throws Exception {
		if (!(request instanceof PortletRequest) && !(request instanceof HttpServletRequest)) {
			TracerSingleton.log(Constants.NOME_MODULO, TracerSingleton.CRITICAL,
					"LightNavigationManager: " + "destroyLightNavigatorFromSession: request object is neither a PortletRequest " + "nor an HttpServletRequest");
			throw new Exception("request object is neither a PortletRequest nor an HttpServletRequest");
		}
		...
		if (request instanceof PortletRequest) {
			...
			if (lightNavigatorObj != null) {
				if (lightNavigatorObj instanceof LightNavigator) {
					TracerSingleton.log(Constants.NOME_MODULO, TracerSingleton.DEBUG,
							"LightNavigationManager: " + "destroyLightNavigatorFromSession: LightNavigator present on session with key = ["
									+ sessionAttributeKey + "]. " + "It will be removed.");
					session.removeAttribute(sessionAttributeKey);
				} else {
					TracerSingleton.log(Constants.NOME_MODULO, TracerSingleton.CRITICAL,
							"LightNavigationManager: " + "destroyLightNavigatorFromSession: session attribute with key = [" + sessionAttributeKey
									+ "] is not a " + "LightNavigator object!! Cannot destroy it!!");
					throw new Exception("Session attribute with key = [" + sessionAttributeKey + "] is not a LightNavigator object!!");
				}
			} else {
				TracerSingleton.log(Constants.NOME_MODULO, TracerSingleton.DEBUG, "LightNavigationManager: "
