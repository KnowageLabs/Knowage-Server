...
public class TalendWork implements Work {
	 ...
	 */
	public TalendWork(String command, String[] envr, File executableJobDir, List filesToBeDeletedAfterJobExecution, Map parameters) {
	...
	public void run() {
		...
		while (paramKeysIt.hasNext()) {
			...
			if (!key.equalsIgnoreCase("template") && !key.equalsIgnoreCase("biobjectId") && !key.equalsIgnoreCase("cr_manager_url")
					&& !key.equalsIgnoreCase("events_manager_url") && !key.equalsIgnoreCase("user") && !key.equalsIgnoreCase("SPAGOBI_AUDIT_SERVLET")
					&& !key.equalsIgnoreCase("spagobicontext") && !key.equalsIgnoreCase("SPAGOBI_AUDIT_ID") && !key.equalsIgnoreCase("username")) {
		...
		try {
			...
			eventServiceProxy.fireEvent(startExecutionEventDescription + parametersList, startEventParamsStr, TALEND_ROLES_HANDLER_CLASS_NAME,
					EventType.ETL.name());
		...
		try {
			...
			/*
			 * this.talendJobClass = Class.forName(talendJobClassName); this.runJob = this.talendJobClass.getMethod("runJob", new Class[]{String[].class}); int
			 * status = ((Integer)runJob.invoke(null, mainMethodParams)).intValue();
			 */
		...
		try {
			...
			eventServiceProxy.fireEvent(endExecutionEventDescription + parametersList, endEventParamsStr, TALEND_ROLES_HANDLER_CLASS_NAME,
					EventType.ETL.name());
