...
public class SbiMetaDsTabRelDAOHibImpl extends AbstractHibernateDAO implements ISbiMetaDsTabRel {
	...
	private static Logger logger = Logger.getLogger(SbiMetaDsTabRelDAOHibImpl.class);
	...
	public void insertRelation(SbiMetaDsTabRel SbiMetaDsTabRel) throws EMFUserError {
		...
		try {
			...
			SbiMetaDsTabRel hibMeta = new SbiMetaDsTabRel(SbiMetaDsTabRel.getRelationId());
	...
	public void deleteRelation(SbiMetaDsTabRel SbiMetaDsTabRel) throws EMFUserError {
		...
		try {
			...
			SbiMetaDsTabRel hibMeta = (SbiMetaDsTabRel) tmpSession.load(SbiMetaDsTabRel.class,
					new Integer(SbiMetaDsTabRel.getRelationId()));
	...
	public List<SbiMetaDsTabRel> loadByDatasetId(Integer datasetId) throws EMFUserError {
		...
		try {
			...
			Criterion labelCriterrion = Restrictions.eq("datasetId", datasetId);
	...
	public SbiMetaDsTabRel loadDsIdandTableId(Integer datasetId, Integer tableId) throws EMFUserError {
		...
		try {
			...
			List<SbiMetaDsTabRel> relations = tmpSession.createCriteria(SbiMetaDsTabRel.class)
					.add(Restrictions.eq("datasetId", datasetId)).add(Restrictions.eq("tableId", tableId)).list();
