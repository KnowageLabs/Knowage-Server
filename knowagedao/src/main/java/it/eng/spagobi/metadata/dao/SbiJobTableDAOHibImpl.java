...
public class SbiJobTableDAOHibImpl extends AbstractHibernateDAO implements ISbiJobTableDAO {
	...
	private static Logger logger = Logger.getLogger(SbiJobTableDAOHibImpl.class);
	...
	public void modifyJobTable(SbiMetaJobTable aMetaJobTable) throws EMFUserError {
		...
		try {
			aSession = getSession();
			tx = aSession.beginTransaction();
			SbiMetaJobTableId hibId = new SbiMetaJobTableId(aMetaJobTable.getId().getJobId(),aMetaJobTable.getId().getTableId());
	...
	public void insertJobTable(Session session, SbiMetaJobTable aMetaJobTable) throws EMFUserError {
		...
		try {
			...
			SbiMetaJobTableId hibId = new SbiMetaJobTableId(aMetaJobTable.getSbiMetaJob().getJobId(),aMetaJobTable.getSbiMetaTable().getTableId());
