...
public class CommunityManager {
			...
			if (community != null && community.getCommunityId() != null) {
				...
				if (communities != null) {
					for (int i = 0; i < communities.size(); i++) {
						...
						LOGGER.debug("Erase user by community " + comm.getName() + " for new insertion to "
								+ community.getName());
					}
				}
	...
	private SbiCommunity populateCommunity(String userId, String communityName, String functCode) {
	...
	/**
	 * This method executes the following actions after user deletion: - if user is owner of a community and there are no other members--> the community is deleted
	 * - if user is owner of a community and there are other members --> the ownership shifts to the oldest member - if he is just a member --> the relationship
	 * with the community is deleted
	 *
