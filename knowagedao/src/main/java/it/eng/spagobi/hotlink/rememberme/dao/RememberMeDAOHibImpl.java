...
public class RememberMeDAOHibImpl extends AbstractHibernateDAO implements IRememberMeDAO {
	...
	public List<RememberMe> getMyRememberMe(String userId) throws EMFInternalError {
		...
		Session aSession = null;
		Transaction tx = null;
		List<RememberMe> toReturn = new ArrayList<>();
		try {
			aSession = getSession();
			tx = aSession.beginTransaction();
			Criterion userIdCriterion = Restrictions.eq("userName", userId);
			Criteria criteria = aSession.createCriteria(SbiRememberMe.class);
			criteria.add(userIdCriterion);
			List<SbiRememberMe> list = criteria.list();
			Iterator<SbiRememberMe> it = list.iterator();
			while (it.hasNext()) {
				SbiRememberMe hibObj = it.next();
	 ...
	 * (non-Javadoc)
	 *
	 * @see it.eng.spagobi.hotlink.rememberme.dao.IRememberMeDAO#saveRememberMe(java.lang.String, java.lang.String, java.lang.Integer, java.lang.Integer,
	 * java.lang.String, java.lang.String)
	 */
	@Override
	public boolean saveRememberMe(String name, String description, Integer docId, Integer subObjId, String userId,
			String parameters) throws EMFInternalError {
		LOGGER.debug("IN");
		Session aSession = null;
		Transaction tx = null;
		try {
			...
			/*
			 * The following code checks if a equal Remember Me exists, i.e. if a Remember Me for the same user, same document, same parameters or same subobject already
			 * exists. In this case the remember me is not saved. This does not work on Ingres because the '=' function (which is the sql translation of hibernate
			 * Expression.eq) does not work on Long nvarchar fields. Criteria criteria = aSession.createCriteria(SbiRememberMe.class); Criterion userIdCriterion =
			 * Expression.eq("userName", userId); criteria.add(userIdCriterion); SbiObjects obj = (SbiObjects) aSession.load(SbiObjects.class, docId); Criterion
			 * docIdCriterion = Expression.eq("sbiObject", obj); criteria.add(docIdCriterion); SbiSubObjects subObj = null; if (subObjId != null) { subObj = (SbiSubObjects)
			 * aSession.load(SbiSubObjects.class, subObjId); Criterion subObjIdCriterion = Expression.eq("sbiSubObject", subObj); criteria.add(subObjIdCriterion); }
			 * Criterion parametersCriterion = Expression.eq("parameters", parameters); criteria.add(parametersCriterion);
			 * //criteria.setResultTransformer(Criteria.DISTINCT_ROOT_ENTITY); List list = criteria.list(); if (list.isEmpty()) { SbiRememberMe temp = new SbiRememberMe();
			 * temp.setName(name); temp.setDescription(description); temp.setUserName(userId); temp.setSbiObject(obj); temp.setSbiSubObject(subObj);
			 * temp.setParameters(parameters); aSession.save(temp); tx.commit(); return true; } else { logger.debug("RememberMe for user " + userId +
			 * " for document with id " + docId + " with parameters [" + parameters + "] is already present."); return false; }
			 */

			/*
			 * The following code does not check if an equivalent Remember Me already exists, it simply inserts it.
			 */
	...
	public RememberMe getRememberMe(Integer rememberMeId) throws EMFInternalError {
		...
		try {
			aSession = getSession();
			tx = aSession.beginTransaction();
			Criterion userIdCriterion = Restrictions.eq("id", rememberMeId);
			Criteria criteria = aSession.createCriteria(SbiRememberMe.class);
			criteria.add(userIdCriterion);
			List<SbiRememberMe> list = criteria.list();
			Iterator<SbiRememberMe> it = list.iterator();
