...
public interface IObjMetadataDAO extends ISpagoBIDao {
	...
	/**
	 * Loads all detail information for a metadata identified by its <code>type</code>. All these informations, achived by a query to the DB, are stored into a
	 * <code>metadata</code> object, which is returned.
	 *
	 ...
	 *
	 * @return True if the metadata is used by one or more objects, else false
	 *
	 ...
	 * @param id The objMetadata identifier
	 *
	 * @return True if the metadata is used by one or more subobjects, else false
	 *
	 * @throws EMFUserError If any exception occurred
	 */
