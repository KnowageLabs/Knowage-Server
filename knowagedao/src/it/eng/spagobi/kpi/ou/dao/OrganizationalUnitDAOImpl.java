...
public class OrganizationalUnitDAOImpl extends AbstractHibernateDAO implements IOrganizationalUnitDAO {
	...
	public List<OrganizationalUnitNode> getRootNodes(Integer hierarchyId) {
		...
		try {
			...
			Query hibQuery = aSession.createQuery(" from SbiOrgUnitNodes n where n.sbiOrgUnitHierarchies.id = ? " + " and n.sbiOrgUnitNodes is null");
			hibQuery.setInteger(0, hierarchyId);
	...
	public List<OrganizationalUnitGrantNode> getNodeGrants(Integer nodeId, Integer grantId) {
		...
		try {
			...
			Query hibQuery = aSession.createQuery(" from SbiOrgUnitGrantNodes s where s.id.nodeId = ? " + " and s.id.grantId = ?");
			hibQuery.setInteger(0, nodeId);
	...
	public boolean existsNodeInHierarchy(String path, Integer hierarchyId, Session aSession) {
		boolean toReturn = false;
		logger.debug("IN: path = " + path + ", hierarchy = " + hierarchyId);
		Query hibQuery = aSession.createQuery(" from SbiOrgUnitNodes n where n.sbiOrgUnitHierarchies.id = ? " + " and n.path = ? ");
		hibQuery.setInteger(0, hierarchyId);
	...
	public void insertNodeGrants(List<OrganizationalUnitGrantNode> grantNodes, Integer grantId) {
		...
		try {
			...
			while (it.hasNext()) {
				...
				logger.debug("Saving grant node with node Id:" + grantNodeId.getNodeId() + " modelInst Id " + grantNodeId.getKpiModelInstNodeId()
						+ " ang grant Id " + grantNodeId.getGrantId());
	...
	private OrganizationalUnitNodeWithGrant getNodeWithGrants(OrganizationalUnitNode node, Integer grantId, Session aSession) {
		logger.debug("IN");
		...
		Query hibQuery = aSession.createQuery(" from SbiOrgUnitGrantNodes n where n.id.nodeId = ? " + " and n.id.grantId = ?");
	...
	public List<OrganizationalUnitGrantNode> getGrantsValidByDate(Integer kpiModelInstanceId, Date now) {
		...
		try {
			...
			Query hibQuery = aSession
					.createQuery(" from SbiOrgUnitGrantNodes n where n.id.kpiModelInstNodeId = ? and ? between n.sbiOrgUnitGrant.startDate and n.sbiOrgUnitGrant.endDate");
			hibQuery.setInteger(0, kpiModelInstanceId);
	...
	public OrganizationalUnit toOrganizationalUnit(SbiOrgUnit hibOrgUnit) {
		OrganizationalUnit ou = new OrganizationalUnit(hibOrgUnit.getId(), hibOrgUnit.getLabel(), hibOrgUnit.getName(), hibOrgUnit.getDescription());
		return ou;
	}

	public OrganizationalUnitHierarchy toOrganizationalUnitHierarchy(SbiOrgUnitHierarchies hibOrgUnitHierarchies) {
		OrganizationalUnitHierarchy hierarchy = new OrganizationalUnitHierarchy(hibOrgUnitHierarchies.getId(), hibOrgUnitHierarchies.getLabel(),
				hibOrgUnitHierarchies.getName(), hibOrgUnitHierarchies.getDescription(), hibOrgUnitHierarchies.getTarget(), hibOrgUnitHierarchies.getCompany());
		return hierarchy;
	}
	...
	public OrganizationalUnitNode toOrganizationalUnitNode(SbiOrgUnitNodes hibOrgUnitNode) {
		...
		OrganizationalUnitNode node = new OrganizationalUnitNode(hibOrgUnitNode.getNodeId(), ou, hierarchy, hibOrgUnitNode.getPath(),
	...
	public OrganizationalUnitGrantNode toOrganizationalUnitGrantNode(SbiOrgUnitGrantNodes hibGrantNode, Session aSession) {
