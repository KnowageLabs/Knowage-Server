...
import it.eng.spagobi.engines.whatif.template.initializer.executor.InitializationExecutorFactory;
...
public class WhatIfXMLTemplateParser implements IWhatIfTemplateParser {
	...
	private WhatIfTemplate parse(SourceBean template) {
		...
		try {
			...
			InitializationExecutorFactory.getExecutor(template).init(template, toReturn);
