	...
	public String buildClause(Query query, Map<String, Map<String, String>> entityAliasesMaps) {
		...
		for (ISelectField orderByField : orderByFields) {
			...
			if (orderByField.isSimpleField()) {
				...
				CustomFunction customFunction = getCustomFunction(modelField);
				if (customFunction != null) {
					fieldName = customFunction.apply(fieldName);
					logger.debug("order clause after custom function applciation[" + fieldName + "]");
				}
