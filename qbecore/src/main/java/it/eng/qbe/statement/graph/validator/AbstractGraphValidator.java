...
public abstract class AbstractGraphValidator implements IGraphValidator {
	...
	public boolean isValid(Graph G, Set<IModelEntity> unjoinedEntities){
		...
		if((unjoinedEntities == null || unjoinedEntities.isEmpty()) && (G == null || vertexSet.isEmpty())){
			return true;
		}

		/*
		 * Check if the graph contains at least all the
		 * required entities.
		 */
		if (!vertexSet.isEmpty() && !vertexSet.containsAll(unjoinedEntities)) {
			return false;
		}
