...
//		enableFilters(session, getRuntimeDrivers());
		IStatement filteredStatement = this.getStatement();
		String statementStr = filteredStatement.getQueryString();
		logger.debug("Compiling query statement [" + statementStr + "]");
	 ...
	 */
	private void enableFilters(Session session, Map<String, String> runtimeDrivers) {

		Filter filter = session.enableFilter("getName");
		filter.setParameter("a", runtimeDrivers.get("filter"));
...
>>>>>>> resolved documentDetails correlation bugs
