...
public class AbstractSelectStatementClause extends AbstractStatementClause {
	...
	public static transient Logger logger = Logger.getLogger(AbstractSelectStatementClause.class);
		...
		try {
			...
			for (ISelectField selectAbstractField : selectFields) {
				if (selectAbstractField.isSimpleField()) {
					...
					addInlineCalculatedField((InLineCalculatedSelectField) selectAbstractField, entityAliasesMaps);
	...
	private void addInlineCalculatedField(InLineCalculatedSelectField selectField, Map entityAliasesMaps) {

		String entity = selectField.getEntity();
		// Must get entity unique name

		if (entity != null) {
			IModelEntity modelEntity = parentStatement.getDataSource().getModelStructure().getEntityByName(entity);
			if (modelEntity != null) {
				String rootEntityAlias = (String) entityAliases.get(modelEntity.getUniqueName());
				if (rootEntityAlias == null) {
					rootEntityAlias = parentStatement.getNextAlias(entityAliasesMaps);
					entityAliases.put(modelEntity.getUniqueName(), rootEntityAlias);
				}
			}
		} else {
			logger.error("calculated field has no entity information, try re-generating the model");
		}
		logger.debug("select clause element succesfully added to select clause");
	}
