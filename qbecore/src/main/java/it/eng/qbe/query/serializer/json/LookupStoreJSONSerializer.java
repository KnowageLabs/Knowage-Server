	...
	public Object serialize(IDataStore dataStore) throws SerializationException {
		...
		try {
			...
			for(int i = 0; i < dataStore.getMetaData().getFieldCount(); i++) {
				...
				if(calculated != null && calculated.booleanValue()) {
				...
				for(int i = 0; i < dataStore.getMetaData().getFieldCount(); i++) {
					...
					if(field.getValue() != null && !field.getValue().equals("")) {
						if(Date.class.isAssignableFrom(fieldMetaData.getType())) {
							fieldValue = DATE_FORMATTER_V2.format(LocalDateTime.ofInstant(((Date) field.getValue()).toInstant(), ZoneId.systemDefault()));
						} else {
