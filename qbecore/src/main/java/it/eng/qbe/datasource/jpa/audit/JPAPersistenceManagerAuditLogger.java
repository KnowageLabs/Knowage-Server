...
public class JPAPersistenceManagerAuditLogger {
	...
	public JPAPersistenceManagerAuditLogger(JPAPersistenceManager jpaPersistenceManager) {
		this.jpaPersistenceManager = jpaPersistenceManager;
	}

	public JPAPersistenceManager getJPAPersistenceManager() {
		return jpaPersistenceManager;
	}

	public void log(Operation operation, JSONObject oldRecord, JSONObject newRecord, Integer changesNumber, RegistryConfiguration registryConf) {
		...
		String modelName = this.getJPAPersistenceManager().getDataSource().getConfiguration().getModelName();
	...
	private void logInternal(String userId, Operation operation, String modelName, String entityName, String oldRecordAsString, String newRecordAsString,
		...
		try {
			...
			MDC.put("entityName", StringEscapeUtils.escapeSql(modelName));
			...
			auditLogger.info("User [" + userId + "] is performing operation [" + operation.name() + "] on entity [" + entityName + "] from model [" + modelName
					+ "] for record: old one is [" + oldRecordAsString + "], new one is [" + newRecordAsString + "]");
		} finally {
			MDC.clear();
		}
	}
