...
app.controller('metaModelDefinitionController', ['$scope','sbiModule_translate', 'sbiModule_restServices',metaModelDefinitionControllerFunction ]);
app.controller('metaModelCreationController', ['$scope','sbiModule_translate', 'sbiModule_restServices','parametersBuilder',metaModelCreationControllerFunction ]);
...
function metaDefinitionControllerFunction($scope,sbiModule_translate,sbiModule_restServices){
	...
	$scope.createMeta= function() {
		...
		sbiModule_restServices.promisePost("1.0/metaWeb","create",dataToSent)
...
function metaModelCreationControllerFunction($scope,sbiModule_translate,sbiModule_restServices,parametersBuilder){
...
//		                                        		return  "<md-checkbox ng-model='row.identifier' aria-label='isPrimaryKey'></md-checkbox>"
		                                        	}
		                                        },
	...
	$scope.selectPhysicalModel=function(node){
		...
		angular.copy(parametersBuilder.extractCategories($scope.selectedPhysicalModel.properties),$scope.currentPhysicalModelParameterCategories);
	}
	...
	$scope.isInbound=function(item){
		return angular.equals(item.sourceTableName,$scope.selectedBusinessModel.name);
	}
...
//			}
//			
////			propertiesStructure[struct[0]][propertiesStructure[struct[0]].length]= tmpProp.value;
