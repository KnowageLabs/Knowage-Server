(function () {
  var app = angular.module("metaManager", [
    "ngMaterial",
    "angular_table",
    "sbiModule",
    "componentTreeModule",
  ...
  app.service("metaModelServices", function (sbiModule_jsonServices) {
    ...
    this.observe = function (observerObj) {
      ...
      bms.originalMetaModelObserver = sbiModule_jsonServices.observe(
        bms.observerObject
      );
      bms.cleanObserverObject();
      bms.metaModelObserve = sbiModule_jsonServices.observe(
        bms.cleanedObserverObject
      );
    };
  ...
  app.controller("checkModelController", [
    "$scope",
    "sbiModule_translate",
    ...
    "dataToSend",
    checkModelControllerFunction,
  ]);

  function checkModelControllerFunction(
    $scope,
    sbiModule_translate,
    sbiModule_restServices,
    sbiModule_config,
    ...
    $scope.saveModel = function () {
      sbiModule_restServices
        .promisePost(
          "1.0/metaWeb",
          "generateModel",
          metaModelServices.createRequestRest($scope.dataToSend)
        )
        .then(
          function (response) {
            sbiModule_restServices.errorHandler(
              sbiModule_translate.load("sbi.catalogues.toast.updated"),
              ""
            );
          },
          function (response) {
            sbiModule_restServices.errorHandler(
              response.data,
              sbiModule_translate.load("sbi.meta.model.generate.error")
            );
          }
        );
    };
  ...
  app.controller("metaDefinitionController", [
    "$scope",
    "sbiModule_translate",
    "sbiModule_restServices",
    "sbiModule_config",
    ...
    "sbiModule_config",
    "sbiModule_user",
    metaDefinitionControllerFunction,
  ]);

  function metaDefinitionControllerFunction(
    $scope,
    sbiModule_translate,
    sbiModule_restServices,
    sbiModule_config,
    ...
    $scope.saveModel = function () {
      ...
      sbiModule_restServices
        .promisePost(
          "1.0/metaWeb",
          "checkRelationships",
          metaModelServices.createRequestRest(dataToSend)
        )
        .then(
          function (response) {
            //check if any error was found during the validation
            if (
              response.data.incorrectRelationships !== undefined &&
              response.data.incorrectRelationships.length > 0
            ) {
              ...
              $mdDialog.show({
                ...
                preserveScope: true,
                locals: {
                  incorrectRelationships: response.data.incorrectRelationships,
                  dataToSend: dataToSend,
                },
                templateUrl:
                  sbiModule_config.contextName +
                  "/js/src/meta/templates/checkModel.jsp",
          ...
          function (response) {
            sbiModule_restServices.errorHandler(
              response.data,
              sbiModule_translate.load("sbi.meta.model.checkmodel.error")
            );
          }
        );
    };
    ...
    $scope.closeMetaDefinition = function () {
      ...
      $mdDialog.show(confirm).then(function () {
        window.parent.postMessage({ action: "closeDialog" }, location.origin);
      });
    };
    ...
    $scope.continueToMeta = function () {
      if ($scope.physicalModels.length == 0) {
        sbiModule_restServices.errorHandler(
          sbiModule_translate.load("sbi.meta.model.physical.select.required"),
          ""
        );
      } else {
        $scope.createMeta();
      ...
      sbiModule_restServices
        ...
        .then(
          ...
          function (response) {
            sbiModule_restServices.errorHandler(
              response.data,
              sbiModule_translate.load("sbi.kpi.rule.load.datasource.error")
            );
          }
        );
    };
  ...
  angular
    .module("metaManager")
    .filter("filterByMainCategory", function (sbiModule_user) {
      var showDataProfiling =
        sbiModule_user.functionalities.indexOf("MetaModelDataProfiling") > -1;
  ...
  angular
    .module("metaManager")
    .filter("filterByCategory", function (sbiModule_user) {
      var showDataProfiling =
        sbiModule_user.functionalities.indexOf("MetaModelDataProfiling") > -1;
  ...
  angular.module("metaManager").filter("filterByDataType", function ($filter) {
    return function (items) {
      ...
      angular.forEach(items, function (item) {
        if (
          !isOneOfDataTypes(dataTypeValue, ["date", "timestamp"]) &&
          item["structural.dateformat"]
        ) {
        } else if (
          !isOneOfDataTypes(dataTypeValue, ["time"]) &&
          item["structural.timeformat"]
        ) {
        } else if (
          isOneOfDataTypes(dataTypeValue, ["date", "time", "timestamp"]) &&
          item["structural.format"]
        ) {
        } else {
          filtered.push(item);
        }
  ...
  angular
    .module("metaManager")
    .filter("filterByProductType", function (sbiModule_config, sbiModule_user) {
      return function (items, prop) {
        var showSpatialDimension =
          sbiModule_user.functionalities.indexOf("SpatialDimension") > -1;
        var showTemporalDimension =
          sbiModule_user.functionalities.indexOf("TemporalDimension") > -1;
        var toReturn = [];
        angular.copy(items, toReturn);
        ...
        if (angular.equals(prop[key].propertyType.name, "Type")) {
          if (
            sbiModule_config.productTypes.indexOf("KnowageLI") == -1 ||
            !showSpatialDimension
          ) {
          ...
          if (
            sbiModule_config.productTypes.indexOf("KnowageSI") == -1 ||
            !showTemporalDimension
          ) {
