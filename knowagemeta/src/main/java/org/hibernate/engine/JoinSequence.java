...
public class JoinSequence {
	...
	public JoinFragment toJoinFragment(
		...
		for ( int i = 0; i < joins.size(); i++ ) {
			...
			Joinable currJoinable = join.getJoinable();
			String currAlias = join.getAlias();
			if ( currJoinable != null ) {
				// joinFragment.addCrossJoin( currJoinable.getTableName(), currAlias );
				String filterCondition = currJoinable.filterFragment( currAlias, enabledFilters );
				// JoinProcessor needs to know if the where clause fragment came from a dynamic filter or not so it
				// can put the where clause fragment in the right place in the SQL AST.   'hasFilterCondition' keeps track
				// of that fact.
				joinFragment.addCondition( filterCondition );
			}
