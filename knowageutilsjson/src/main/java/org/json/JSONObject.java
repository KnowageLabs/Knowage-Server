...
//			mapper.setNodeFactory(JsonNodeFactory.withExactBigDecimals(true));
	...
	public static Object wrap(Object object) throws JSONException {
		...
		try {
			...
			if (object instanceof JSONObject || object instanceof JSONArray || NULL.equals(object) || object instanceof Byte || object instanceof Character
					|| object instanceof Short || object instanceof Integer || object instanceof Long || object instanceof Boolean || object instanceof Float
	...
	private void populate(Object bean) {
		...
		if (JSONObject.class.equals(klass)) {
			JSONObject _bean = (JSONObject) bean;

			Iterator<String> keys = _bean.keys();

			while (keys.hasNext()) {
				String key = keys.next();

				try {
					put(key, _bean.get(key));
				} catch (JSONException e) {
					// If the value was in the original object it could be in the new without problems
				}
			}
		} else {
