...
function chartExecutionFunction($scope,$http,chartExecutionWebServiceManagerFactory,sbiModule_translate,sbiModule_messaging,chartEngineSettings) {
	...
	/**
	 * Function defined inside the 'highcharts414Initializer.jsp' and the 'd3js244Initializer.jsp' files for these two chart libraries.
	 * Serves for connecting the JS code (that libraries use to control the rendering of the chart) with the HTML element that is used 
	 * as a container of the rendered chart. Also it sets values for other three input parameters (drill up text, decimal point and 
	 * thousands separator). These values are currently used only in the Highcharts charts.
	 */
	if(typeof initChartLibrary != 'undefined') {
		
		initChartLibrary
		(
			"mainPanel", // the ID of the panel (DIV) inside of which the chart will render
			sbiModule_translate.load('sbi.chartengine.viewer.drilluptext'), 
			sbiModule_translate.load('sbi.chartengine.viewer.decimalpoint'), 
			sbiModule_translate.load('sbi.chartengine.viewer.thousandsep')
		);
		
	}

	var thisContextNameParam = (typeof thisContextName != 'undefined') ?  thisContextName.replace('/', '') : null; // e.g. 'knowagechartengine'
	...
	if(typeof isCockpit != 'undefined' && isCockpit) {
...
//		Sbi.chart.viewer.ChartTemplateContainer.metaData = '<%=metaData%>';
		...
		parameters = {
			jsonTemplate: (typeof jsonTemplate != 'undefined') ? jsonTemplate : null,
			driverParams: (typeof driverParams != 'undefined') ? driverParams : null
		};
	}
	
//	console.info("Chart parameters: ", parameters);
	
	var serviceConfig = {};
	
//	var setServiceConfig = function(protocol,hostName,serverPort,thisContextNameParam,sbiExecutionId,userId) {
//		
//		return {
//			protocol: protocol,
//			hostName: hostName,
//			tcpPort: serverPort,
//			context: '/'+thisContextNameParam,
//			wsPrefix: '/api/1.0/',
...
//	}
	...
	// The configuration for the 'chartExecutionWebServiceManagerFactory' service (all needed parameters).
	/* This configuration is commone for the chart execution (rendering) and for the chart exporting. The specific part is the webservice prefix 
	 * ('/api/1.0/' for chart execution and '/' for the chart exporting).*/
	var serviceConfig = {
...
//		wsPrefix: '/api/1.0/',
	...
	// For chart execution
	serviceConfig[wsPrefixPropText] = '/api/1.0/';
	
//	serviceConfig = setServiceConfig(protocol,hostName,serverPort,thisContextNameParam,sbiExecutionId,userId);	
	
	chartExecutionWebServiceManager = chartExecutionWebServiceManagerFactory;
	
	chartExecutionWebServiceManagerFactory.setConfig(serviceConfig);
	
	/**
	 * -------------------------------------------------
	 * Register all needed REST services. [START]
	 * -------------------------------------------------
	 */
