...
function funzione(sbiModule_translate, sbiModule_restServices, $q, $scope, $mdDialog, $filter, $timeout, $mdToast,sbiModule_messaging) {
	...
	function resetForm(ele) {
		angular.element(
				document.querySelector('.md-chip-input-container .md-whiteframe-z1 input'))
				.val("");
		angular.element(
				document
		...
		if (ele == undefined) {
			...
			// downloading all word informations
			getWord(ele);
		}
	};

	// new words autocomplete

	ctr.loadProperty = function() {
		sbiModule_restServices.get("1.0/udp", "loadUdp", "FAMILY=Glossary")
			...
			if (data.hasOwnProperty("errors")) {
				...
				for (var i = 0; i < data.length; i++) {
					if (JSON.stringify(ctr.newWord.SBI_GL_WORD_ATTR)
							.toString()
							.toLowerCase()
							.indexOf(data[i].ATTRIBUTE_NM.toString().toLowerCase()) != -1) {
	...
	ctr.querySearchProp = function(query) {
		...
		.get("1.0/udp", "loadUdp", "LABEL=" + query + "&FAMILY=Glossary")
		.success(function(data, status, headers, config) {
			if (data.hasOwnProperty("errors")) {
				console.log("attributeLike non Ottenuti");
			} else {
			...
			if (data.hasOwnProperty("errors")) {
				...
				if (ctr.newWord.hasOwnProperty("PARENT")) {
					...
					.success(function(data) {
						...
						ctr.newWord = JSON
						.parse(JSON.stringify(EmptyWord));
						showToast(
								sbiModule_translate
	...
	ctr.deleteWord = function(ev) {
		...
		.then(function() {
			...
			// checking if the element is in modification status
	...
	ctr.deleteGlossary = function(ev) {
		...
		.then(function() {
			showPreloader();
			sbiModule_restServices.remove("1.0/glossary/business",
					"deleteGlossary",
	...
	ctr.createNewGlossary = function(event,gl) {
		$mdDialog.show({
			...
			controller : function($mdDialog) {
				...
				if (gl != undefined) {
					// load glossary data
					gctl.headerTitle = sbiModule_translate.load("sbi.glossary.modify");
					showPreloader();
					...
					gctl.headerTitle = sbiModule_translate.load("sbi.glossary.save");
				...
				gctl.submit = function() {
					...
					.success(function(data, status, headers, config) {
						if (data.hasOwnProperty("errors")) {
							...
							if (gctl.newGloss.SaveOrUpdate == "Save") {
								...
								ctr.newGloss = JSON.parse(JSON.stringify(EmptyGloss));
								$mdDialog.hide();
								showToast(sbiModule_translate.load("sbi.glossary.save.success"),3000);
							} else {
								// modification success
								gl.GLOSSARY_NM = gctl.newGloss.GLOSSARY_NM;
								ctr.newGloss = JSON.parse(JSON.stringify(EmptyGloss));
								$mdDialog.hide();
								showToast(sbiModule_translate.load("sbi.glossary.modify.success"), 3000);
							}
						}
						hidePreloader();
					})
					.error(function(data, status, headers, config) {
						$mdDialog.hide();
						showErrorToast("glossary not saved " + status, sbiModule_translate.load("sbi.glossary.save.error"));
						hidePreloader();
					})
				};
	...
	ctr.TreeOptionsWord = {
		...
		beforeDrop : function(event) {
			...
			// locking the automatical moving of the element
	...
	ctr.hideAddFiglioBox = function() {
		angular.element(document.getElementsByClassName('addFiglioBox')).css(
				"display", "none");
		var x = angular.element(document.querySelector('.chipsTree .angular-ui-tree-empty'))
		.css("display", "none");
	};
	...
	ctr.TreeOptions = {
			accept : function(sourceNodeScope, destNodesScope, destIndex) {
				...
				if (sourceNodeScope.$modelValue.hasOwnProperty("WORD")) {
					...
					// checking the uniqueness of the word in the glossary 
					// checking the presence of the same element as a child 
