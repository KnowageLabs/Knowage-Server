...
public class EventsManager {
	 ...
	 */
	public List getRegisteredEvents(IEngUserProfile profile, Map<String, Object> properties) {
		logger.debug("IN");
		List registeredEventsList = null;
		try {
			EventLogDAOHibImpl eventLogDAO = new EventLogDAOHibImpl();
			registeredEventsList = eventLogDAO.loadEventsLogByUser(profile,properties);
		} catch (EMFUserError e) {
	...
	/**
	 * Get the size of registered events (generated by the user at input) ordered by date.
	 * 
	 * @param profile The user profile
	 * 
	 * @return The list of events generated by the user
	 */
	public int getRegisteredEventsSize(IEngUserProfile profile) {
		logger.debug("IN");
		List registeredEventsList = null;
		try {
			EventLogDAOHibImpl eventLogDAO = new EventLogDAOHibImpl();
			return eventLogDAO.loadEventsSizeLogByUser(profile);
		} catch (EMFUserError e) {
			logger.error("Error while loading events list for the user '" + ((UserProfile)profile).getUserId().toString(), e);
			throw new SpagoBIRuntimeException("Error while loading events list for the user '" + ((UserProfile)profile).getUserId().toString(), e);
		}finally{
			logger.debug("OUT");
		}
	...
	public List getRegisteredEvents(IEngUserProfile profile) {
		return getRegisteredEvents(profile, new HashMap<String, Object>());
	}
