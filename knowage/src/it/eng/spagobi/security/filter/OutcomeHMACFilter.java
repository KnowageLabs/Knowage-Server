...
package it.eng.spagobi.security.filter;
...
public class OutcomeHMACFilter implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
		...
		if (userId != null && !userId.trim().equals("")) {
			HMACFilter.checkHMAC(httpRequest, tokenValidator, key);
			try {
				...
				if (profile == null) {
					...
					if (userId != null && !userId.trim().equals("")) {
						...
						// Attribute used to handle hmac_adapter
						// session.setAttribute(FORWARD_HMAC_ADAPTER, true);
