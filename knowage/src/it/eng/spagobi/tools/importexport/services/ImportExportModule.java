...
import it.eng.spagobi.tools.importexportOLD.IExportManager;
...
import it.eng.spagobi.tools.importexportOLD.UserAssociationsKeeper;
...
public class ImportExportModule extends AbstractHttpModule {
	...
	/*
	 * private List extractObjId(List requests) { logger.debug("IN"); List toReturn = new ArrayList(); Iterator iter = requests.iterator(); while
	 * (iter.hasNext()) { String id_path = (String) iter.next(); String id = id_path.substring(0, id_path.indexOf('_')); toReturn.add(id); }
	...
	private void associateEngines(SourceBean request, SourceBean response) throws EMFUserError {
		...
		try {
			...
			if (impManager.getImpAssMode().equals(IImportManager.IMPORT_ASS_PREDEFINED_MODE) && !impManager.associateAllExportedDataSourcesByUserAssociation()) {
				...
				if (metaAss.isEmpty()) {
					response.setAttribute(ImportExportConstants.PUBLISHER_NAME, "ImportExportSkipExistingMetadataAssociation");
				} else {
	...
	/**
	 * Manages the request of the user to associate some exported data sources to the data sources of the portal in use
	 * 
	 * @param request
	 *            Spago SourceBean request
	 * @param response
			...
			while (iterExpConn.hasNext()) {
				...
				if (!assDsIds.equals("")) {
					...
					if (existingDSObj != null && exportedDSObj != null) {
						...
						impManager.getUserAssociation().recordDataSourceAssociation(Integer.valueOf(exportedDataSource.getDsId()),
								Integer.valueOf(existingDataSource.getDsId()));
					}
				}
				// else {
	...
	private void associateMetadata(SourceBean request, SourceBean response) throws EMFUserError {
		...
		try {
			...
			if (assFile != null)
				response.setAttribute(ImportExportConstants.IMPORT_ASSOCIATION_FILE, assFile);
		} catch (EMFUserError emfue) {
			logger.error("Error in associating engines: " + emfue);
			if (impManager != null)
				impManager.stopImport();
	...
	/**
	 * Manages the request of the user to go back from the data sources association to the engines association
	 * 
	 * @param request
	 *            Spago SourceBean request
	 * @param response
	...
	/**
	 * Manages the request of the user to go back from the metadata association to the data sources association
	 * 
	 * @param request
	 *            Spago SourceBean request
	 * @param response
