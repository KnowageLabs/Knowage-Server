...
public class ExportUtilities {
	...
	public static String getExportTempFolderPath() {
		...
		try {
			...
			SourceBean importerSB = (SourceBean) conf.getAttribute("IMPORTEXPORT.EXPORTEROLD");
	...
	public static IExportManager getExportManagerInstance() throws Exception {
		...
		try {
			ConfigSingleton conf = ConfigSingleton.getInstance();
			SourceBean exporterSB = (SourceBean) conf.getAttribute("IMPORTEXPORT.EXPORTEROLD");
	 ...
	 */
	/*
	 * public static DataConnection getDataConnection(Connection con) throws EMFInternalError { logger.debug("IN"); DataConnection dataCon = null; try { Class
	 * mapperClass = Class.forName("it.eng.spago.dbaccess.sql.mappers.OracleSQLMapper"); SQLMapper sqlMapper = (SQLMapper)mapperClass.newInstance(); dataCon =
	 * new DataConnection(con, "2.1", sqlMapper); } catch(Exception e) { logger.error("Error while getting Spago  DataConnection " , e); throw new
	 * EMFInternalError(EMFErrorSeverity.ERROR, "cannot build DataConnection object"); }finally{ logger.debug("OUT"); } return dataCon; }
	 */
