...
public class ManageDataSetsForREST {
			...
			if (name != null && label != null && datasetTypeName != null && !datasetTypeName.equals("")) {
				...
				if (ds != null) {
					...
					if (scopeID == null) {
						...
						throw new SpagoBIDataSetException("Impossible to save Data Set. The scope in not set");
					} else {
					...
					if (scopeCode.equalsIgnoreCase(SpagoBIConstants.DS_SCOPE_ENTERPRISE) || scopeCode.equalsIgnoreCase(SpagoBIConstants.DS_SCOPE_TECHNICAL)) {
						if (catTypeCd == null || catTypeCd.equals("")) {
							...
							throw new SpagoBIDataSetException("Impossible to save DataSet. The category is mandatory for Data Set Enterprise or Technical");
						}
					}
					...
					if (datasetTypeName != null && !datasetTypeName.equals("")) {
						...
						if (dsRecalc != null) {
							...
							if ((recalculateMetadata == null || recalculateMetadata.trim().equals("yes") || recalculateMetadata.trim().equals("true"))
								...
								try {
									...
									throw new SpagoBIDataSetException("sbi.ds.test.error.metadata");
								}
								...
								for (int i = 0; i < currentMetadata.getFieldCount(); i++) {
									...
									if (aliases.contains(alias)) {
										...
										throw new SpagoBIDataSetException("sbi.ds.test.error.duplication");
									}
									aliases.add(alias);
								}
									...
									if (previousIdInteger != 0) {
										...
										if (!federationsAssociated.isEmpty()) {
											...
											if (isRemoving) {
												...
												// warning
												throw new SpagoBIDataSetException("sbi.ds.deleteOrRenameMetadata");
						...
						throw new SpagoBIDataSetException("sbi.ds.dsTypeError");
					}
				} else {
					logger.error("DataSet type is not existent");
					throw new SpagoBIDataSetException("sbi.ds.dsTypeError");
	...
	private IDataSet getDataSet(String datasetTypeName, boolean savingDataset, JSONObject json, UserProfile userProfile) throws Exception {
		...
		if (datasetTypeName.equalsIgnoreCase(DataSetConstants.DS_FILE)) {
			...
			if ((dsId == null) || (dsId.isEmpty())) {
				...
				if (storeToHDFS && savingDataset) {
					// take the resourcePath from local fileSystem in order to
					// move it to HDFS
						...
						if (storeToHDFS) {
							...
							try {
								if (hdfs.exists(source)) {
									...
									// first time the upload file is done in
									// local, get the file from local and store
									// in hdfs
		...
		if (datasetTypeName.equalsIgnoreCase(DataSetConstants.DS_CKAN)) {
			...
			// 4* String fileName =
			// getAttributeAsString(DataSetConstants.FILE_NAME);
		...
		if (datasetTypeName.equalsIgnoreCase(DataSetConstants.DS_REST_TYPE)) {
			dataSet = manageRESTDataSet(savingDataset, jsonDsConfig, json);
		}
		...
		if (datasetTypeName.equalsIgnoreCase(DataSetConstants.DS_WS)) {
			...
			String wsAddress = json.optString(DataSetConstants.WS_ADDRESS);
			String wsOperation = json.optString(DataSetConstants.WS_OPERATION);
		...
		if (datasetTypeName.equalsIgnoreCase(DataSetConstants.DS_QBE)) {
			...
			if (sourceDatasetLabel != null && !sourceDatasetLabel.trim().equals("")) {
				try {
					...
					throw new SpagoBIDataSetException("Cannot retrieve source dataset information");
				}
			}
	...
	private String getDataSetParametersAsString(JSONObject json) {
		...
		try {
			...
			throw new SpagoBIDataSetException("An unexpected error occured while deserializing dataset parameters");
	...
	private void getPersistenceInfo(IDataSet ds, JSONObject json) throws EMFUserError, JSONException {
		...
		if (isPersisted) {
			...
			// }
			// }
			// 6* String persistTableName = getAttributeAsString();
			String persistTableName = json.optString(DataSetConstants.PERSIST_TABLE_NAME);
		...
		if (ds != null) {
			...
			try {
				if (id != null && !id.equals("") && !id.equals("0")) {
					...
					if (existing != null) {
						throw new SpagoBIDataSetException("sbi.ds.labelAlreadyExistent");
					}
				...
				throw new SpagoBIDataSetException(es.getMessage());
			} catch (Throwable e) {
				try {
					AuditLogUtilities.updateAudit(req, profile, "DATA_SET.ADD", logParam, "KO");
				} catch (Exception e1) {
				...
				throw new SpagoBIDataSetException("sbi.ds.saveDsError");
			}
		} else {
			try {
			...
			throw new SpagoBIDataSetException("sbi.ds.fillFieldsError");
		}
	}
				...
				if (triggers != null && !triggers.isEmpty() && !ds.isScheduled()) {
					...
					if (execOutSB != null) {
						...
						if (outcome.equalsIgnoreCase("fault")) {
							...
							throw new SpagoBIDataSetException("Job " + previousDataset.getLabel() + " not deleted by the web service");
						}
					}
					try {
	...
	public void insertPersistenceAndScheduling(IDataSet ds, HashMap<String, String> logParam, JSONObject json, HttpServletRequest req) throws Exception {
		...
		if (ds.isPersisted()) {
			...
			if (parsListJSON.length() > 0) {
				...
				throw new SpagoBIDataSetException("sbi.ds.dsCannotPersist");
			}
			...
			if (ds.isScheduled()) {
				...
				if (jobName != null) {
					...
					throw new SpagoBIDataSetException("The job is not saved correctly!");
				}
			} else {
				ISchedulerServiceSupplier schedulerService = SchedulerServiceSupplierFactory.getSupplier();
				...
				if (execOutSB != null) {
					...
					if (outcome.equalsIgnoreCase("fault")) {
						...
						throw new SpagoBIDataSetException("Job " + ds.getLabel() + " not deleted by the web service");
					}
				}
				try {
					AuditLogUtilities.updateAudit(req, profile, "SCHED_TRIGGER.DELETE", logParam, "OK");
