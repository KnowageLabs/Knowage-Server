...
public class ParameterValuesEncoder {
	...
	public String encode(BIObjectParameter biobjPar) {
		...
		if (parameter != null) {
			...
			if (modValue != null) {
				...
				if (typeCode.equalsIgnoreCase(SpagoBIConstants.INPUT_TYPE_QUERY_CODE) && multivalue) {
					return encodeMultivaluesQueryParam(biobjPar.getParameterValues(), type);
				} else {
					if (!multivalue) {
	 ...
	 */
	private String encodeMultivaluesQueryParam(List values, String parameterType) {
		logger.debug("IN");
		String value = "";
