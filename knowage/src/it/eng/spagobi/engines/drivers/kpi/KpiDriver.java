...
import it.eng.spagobi.engines.drivers.kpi.util.FullKpiEngineUtil;
import it.eng.spagobi.engines.kpi.KpiEnginData;
import it.eng.spagobi.engines.kpi.KpiParametrization;
...
public class KpiDriver extends GenericDriver {
	...
	public FullKpiTemplateConfiguration templateConfiguration = new FullKpiTemplateConfiguration("KPI_DEFAULT_PUB", "KPI_METADATA_DEFAULT_PUB",
			"TREND_DEFAULT_PUB", false, "MODEL", null, null, null, null, null, false, true, false, false, false, false, true, false, false, false, false, false,
			null, null);

	public KpiEnginData data;

	public KpiParametrization parameters = new KpiParametrization(new Date(), null, "default", null, null, null, null, null);

	protected FullKpiValueComputation computation;
	...
	public void executeByKpiEngineJob(RequestContainer requestContainer, SourceBean response) throws EMFUserError, SourceBeanException {
		...
		this.data = FullKpiEngineUtil.setGeneralVariables(requestContainer);
		...
		if (!parameters.getParametersObject().containsKey("ParKpiDate")) {
			String dateForDataset = FullKpiEngineUtil.getDateForDataset(parameters.getDateOfKPI());
		...
		logger.debug("Set the List of Resources related to the specified Model Instance");
		computation = new FullKpiValueComputation(this);
		KpiLineVisibilityOptions options = FullKpiEngineUtil.setVisibilityOptions(this.templateConfiguration);
		...
		if (cascade != null && cascade.equals("true")) {// in case all the kpi children have to be calculated too
			...
			try {
				if (this.resources == null || this.resources.isEmpty()) {
					...
					logger.debug("Set the tree Root.");
	...
	public void execute(RequestContainer requestContainer, BIObject obj, SourceBean response) throws EMFUserError {
		...
		try {
			// **************get the template*****************
			SourceBean content = FullKpiEngineUtil.getTemplate(documentId);
			logger.debug("Got the template.");

			// Date for which we want to see the KpiValues
			this.parameters.setDateOfKPI(new Date());
			this.parameters.setParametersObject(FullKpiEngineUtil.readParameters(obj.getBiObjectParameters(), this));
			...
			logger.debug("Set the configuration of the template");
			...
			logger.debug("Set the List of Kpis that does not need to be persisted in db");
			if (this.resources == null || this.resources.isEmpty()) {
				logger.debug("There are no resources assigned to the Model Instance");
				KpiResourceBlock block = new KpiResourceBlock();
				block.setD(this.parameters.getDateOfKPI());
	...
	public void calculateAndInsertKpiValueWithResources(ModelInstanceNode modI, List resources) throws EMFUserError, EMFInternalError, SourceBeanException {
		...
		if (kpiI != null) {
			...
			logger.debug("Set the KpiValue Instance ID:" + kpiInstanceID);
				...
				if (ouList != null && !ouList.isEmpty()) {
					if (templateConfiguration.isUse_ou()) {
						for (int i = 0; i < ouList.size(); i++) {
							...
							kVal = computation.recursiveGetKpiValueFromKpiRel(kpi, dataSet, temp, kVal, parameters.getDateOfKPI(), kVal.getEndDate(),
									modI.getModelInstanceNodeId());
							kVal = computation.getKpiValueFromDataset(dataSet, temp, kVal, parameters.getDateOfKPI(), kVal.getEndDate(), true,
									modI.getModelInstanceNodeId());
	...
	private void setGUIInformation(KpiLine line, KpiInstance kpiI, ModelInstanceNode node, Kpi k) throws EMFUserError {
	...
	private void setVisibilityInformation(KpiLine line, ModelInstanceNode modI) throws EMFUserError {
	...
	public KpiLine getBlock(ModelInstanceNode modI, Resource r) throws EMFUserError, EMFInternalError, SourceBeanException {
		...
		if (kpiI == null && modI.getModelInstaceReferenceLabel() != null) {
			ModelInstanceNode modelInstanceRefered = DAOFactory.getModelInstanceDAO().loadModelInstanceByLabel(modI.getModelInstaceReferenceLabel(),
					parameters.getDateOfKPI());
	...
	private KpiLine retrieveKpiLine(KpiLine line, KpiValue value, KpiInstance kpiI, Integer miId, Resource r, boolean alreadyExistent)
	...
	/**
	 * Method created to fill ouList class attribute, each time a model instance node is examined. If execution modality is document execution: * if node has
	 * grants but no document parameter ParKpiOU is passed, than warning (dataset will fail) * if node has grants but ParKpiOU = another organizational unit not
	 * granted for the node, than warning (dataset will fail) * if node has grants and ParKpiOU = granted organizational unit for the node, than add this org
	 * unit to the ouList * if node doesn't have grants, than standard flow no elements in ouList, no warnings
	 *
	 * If execution modality is scheduled execution: * if no ParKpiOU document parameter is passed, than all granted ou are added to ouList and kpi value will
	 * be calculated for each ou, generating a record on db * if ParKpiOU document parameter = one ou, than the behaviour is the same as document execution
	 * modality.
	 *
	 * @param miId
	 * @param paramLabelOU
	...
	private KpiValue getValueDependingOnBehaviour(KpiInstance kpiI, Integer miId, Resource r, boolean alreadyExistent, OrganizationalUnitGrantNode grantNode)
			throws EMFUserError, EMFInternalError, SourceBeanException {
		logger.debug("IN");
		...
		if (alreadyExistent) {// use diplay behaviour since value already exists
			...
			if (parameters.getBehaviour().equalsIgnoreCase("timeIntervalDefault")) {
				if (parameters.getDateIntervalFrom() != null && parameters.getDateIntervalTo() != null) {
					value = DAOFactory.getKpiDAO().getKpiValueFromInterval(kpiI.getKpiInstanceId(), parameters.getDateIntervalFrom(),
							parameters.getDateIntervalTo(), r, grantNodeToUse);
					if (value == null) {
						IDataSet dataSet = DAOFactory.getKpiDAO().getDsFromKpiId(kpiI.getKpi());
						logger.info("Retrieved the Dataset to be calculated: " + (dataSet != null ? dataSet.getId() : "null"));
				...
				if (parameters.getDateIntervalFrom() != null && parameters.getDateIntervalTo() != null) {
					DAOFactory.getKpiDAO().deleteKpiValueFromInterval(kpiI.getKpiInstanceId(), parameters.getDateIntervalFrom(), parameters.getDateIntervalTo(),
							r, grantNodeToUse);
					IDataSet dataSet = DAOFactory.getKpiDAO().getDsFromKpiId(kpiI.getKpi());
					logger.info("Retrieved the Dataset to be calculated: " + (dataSet != null ? dataSet.getId() : "null"));
					value = computation.getNewKpiValue(dataSet, kpiI, r, miId, grantNodeToUse);
				...
				if (parameters.getBehaviour().equalsIgnoreCase("default") || // If the behaviour is default
						(parameters.getBehaviour().equalsIgnoreCase("recalculate") && kpiI.getPeriodicityId() != null)) {// or the behaviour is recalculate and
																															// the kpiinstance has a setted
																															// periodicity
				...
				if (value == null || // If the retrieved value is null
						...
						(parameters.getBehaviour().equalsIgnoreCase("recalculate") && kpiI.getPeriodicityId() == null)) {// or the behaviour is recalculate and
																															// the kpiinstance hasn't a setted
																															// periodicity
	...
	protected KpiValue setKpiValuesFromDataset(KpiValue kpiValueToReturn, List fields, IMetaData d, Date begD, Date endDate, String datasetLabel,
			...
			if (f != null) {
				if (f.getValue() != null) {
					...
					if (fieldName.equalsIgnoreCase("DESCR")) {
						...
						logger.debug("Set the kpiValue description:" + descr);
						...
						if (fieldValue != null) {
							...
							logger.info("Set the kpiValue Resource with resource name:" + fieldValue);
						}
					} /*
						 * else if(fieldName.equalsIgnoreCase("ORG_UNIT_ID")){ String fieldValue = f.getValue().toString(); if (fieldValue!=null){ Resource
						 * rTemp = DAOFactory.getResourceDAO().loadResourcesByNameAndModelInst(fieldValue); kpiValueToReturn.setR(rTemp); logger.info(
						 * "Set the kpiValue Resource with resource name:"+fieldValue); } }
						 */ else {
						String fieldValue = f.getValue().toString();
						if (fieldValue != null) {
		...
		if (kpiValueToReturn == null && valueFound == true) {
			...
			MissingKpiValueException missingKpiValueException = new MissingKpiValueException(
					"cjheck dataset " + datasetLabel + " because no value field for kpi was found");

			IKpiErrorDAO dao = DAOFactory.getKpiErrorDAO();
			dao.setUserProfile(data.getProfile());
			dao.insertKpiError(missingKpiValueException, modInstId, kpiVal.getR() != null ? kpiVal.getR().getName() : null);
		}
		monitor.stop();
