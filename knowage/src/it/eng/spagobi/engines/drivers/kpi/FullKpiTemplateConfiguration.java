...
import it.eng.spagobi.engines.kpi.KpiValueComputation;
...
public class FullKpiTemplateConfiguration {
	...
	public FullKpiTemplateConfiguration(String publisher_Name,
	 ...
	 * @param fullKpiDriver TODO
	 * @param content The template SourceBean containing parameters configuration
	 */
	public void getSetConf(KpiDriver fullKpiDriver, SourceBean content) {
		KpiDriver.logger.debug("IN");
		fullKpiDriver.confMap = new HashMap();
		...
		if (content.getAttribute("name") != null) {
			...
			titleChart = replaceParsInString(titleChart, fullKpiDriver);
			fullKpiDriver.setName(titleChart);
		} 
		...
		if (styleTitleSB != null) {
			...
			try {
				...
				fullKpiDriver.styleTitle = new StyleLabel(fontS, size, color);
			} catch (Exception e) {
				KpiDriver.logger.error("Wrong style Title settings, use default",e);
			}
	
		} else {
			fullKpiDriver.styleTitle = new StyleLabel("Arial", 16, new Color(255, 0, 0));
		}
		fullKpiDriver.confMap.put("styleTitle", fullKpiDriver.styleTitle);
		...
		if (styleSubTitleSB != null) {
			...
			subTitle = replaceParsInString(subTitle, fullKpiDriver);
			fullKpiDriver.setSubName(subTitle);		
			...
			try {
				Color color = Color.decode(colorS);
				int size = Integer.valueOf(sizeS).intValue();
				fullKpiDriver.styleSubTitle = new StyleLabel(fontS, size, color);
			} catch (Exception e) {
				KpiDriver.logger.error("Wrong style SubTitle settings, use default");
			}
		} else {
			fullKpiDriver.styleSubTitle = new StyleLabel("Arial", 12, new Color(0, 0, 0));
		}
		fullKpiDriver.confMap.put("styleSubTitle", fullKpiDriver.styleSubTitle);
		...
		try {
			...
			fullKpiDriver.confMap.put("closed_tree", isClosed_tree());
			...
			fullKpiDriver.confMap.put("display_semaphore", isDisplay_semaphore());
			...
			fullKpiDriver.confMap.put("display_bullet_chart", isDisplay_bullet_chart());
			...
			fullKpiDriver.confMap.put("display_threshold_image", isDisplay_threshold_image());
			...
			fullKpiDriver.confMap.put("display_weight", isDisplay_weight());
			...
			fullKpiDriver.confMap.put("display_alarm", isDisplay_alarm());
			...
			fullKpiDriver.confMap.put("use_ou", isUse_ou());
			...
			if(!isRegister_par_setted()){//the spagobi register_values if setted has priority
				...
				fullKpiDriver.confMap.put("register_values", isRegister_values());
			}
			...
			fullKpiDriver.confMap.put("show_axis", isShow_axis());
			...
			fullKpiDriver.confMap.put("weighted_values", isWeighted_values());
			...
			if (dataParameters.get("model_title") != null
				...
				setModel_title(msgBuild.getMessage("sbi.kpi.modelLineTitle", fullKpiDriver.data.getLocale()));					
			}
			fullKpiDriver.confMap.put("model_title", getModel_title());
	
			if (dataParameters.get("kpi_title") != null) {
				String fil = (String) dataParameters.get("kpi_title");
				if (fil!=null) setKpi_title(fil);
			}
			fullKpiDriver.confMap.put("kpi_title", getKpi_title());
	
			if (dataParameters.get("weight_title") != null) {
				String fil = (String) dataParameters.get("weight_title");
				if (fil!=null) setWeight_title(fil);
			}
			fullKpiDriver.confMap.put("weight_title", getWeight_title());
			...
			fullKpiDriver.confMap.put("bullet_chart_title", getBullet_chart_title());
			...
			fullKpiDriver.confMap.put("threshold_image_title", getThreshold_image_title());
	
			if (dataParameters.get("value_title") != null) {
				String fil = (String) dataParameters.get("value_title");
				if (fil!=null) setValue_title(fil);
			}
			fullKpiDriver.confMap.put("value_title", getValue_title());
			...
			KpiDriver.logger.error("error in reading template parameters");
		}
	}
	String replaceParsInString(String title, KpiDriver fullKpiDriver){
		logger.debug("IN");
		SimpleDateFormat f = new SimpleDateFormat();
		f.applyPattern(fullKpiDriver.data.getInternationalizedFormat());
		SimpleDateFormat fServ = new SimpleDateFormat();
		fServ.applyPattern(fullKpiDriver.data.getFormatServer());
		if (title != null) {
			String tmpTitle = title;
			while (!tmpTitle.equals("")) {
				if (tmpTitle.indexOf("$P{") >= 0) {
					String parName = tmpTitle.substring(tmpTitle.indexOf("$P{") + 3, tmpTitle.indexOf("}"));
					String parValue = (fullKpiDriver.parameters.getParametersObject().get(parName) == null) ? "" : (String) fullKpiDriver.parameters.getParametersObject()
							.get(parName);
