...
public class UploadDatasetFileResource extends AbstractSpagoBIResource {
	...
	public Response uploadFileDataset(@MultipartForm MultipartFormDataInput input) {
		logger.debug("IN");
		try {
			FileItem uploaded = (FileItem) input;
 ...
 * 
 * @Override public int getStatusCode() { if (e != null) { return
 * JSONResponse.FAILURE; } return JSONResponse.SUCCESS; }
 * 
 * @Override public String getFileName() { return null; }
 * 
 * @Override public String getContentType() { return "text/html"; }
 * 
 * @Override public String getContent() throws IOException { JSONObject
 * toReturn = new JSONObject(); if (e != null) { try {
 * toReturn.put("success", false); toReturn.put("msg", e.getMessage());
 * return toReturn.toString(); } catch (JSONException jSONException) {
 * logger.error(jSONException); } } toReturn = new JSONObject(); try {
 * toReturn.put("success", true); toReturn.put("fileExtension",
 * fileExtension); toReturn.put("file", "null"); } catch (JSONException
 * jSONException) { logger.error(jSONException); } // return
 * "{success:true, file:null}"; return toReturn.toString();
 * 
 * }
 * 
 * });
 ...
 */
/*
 * private int getDatasetsNumberUsingFile(FileItem uploaded) { String
 * configuration; String fileName =
 * SpagoBIUtilities.getRelativeFileNames(uploaded.getName()); String
 * fileToSearch = "\"fileName\":\"" + fileName + "\""; IDataSet iDataSet; int
 * datasetUisng = 0;
 * 
 * try { IDataSetDAO ds = DAOFactory.getDataSetDAO(); List<IDataSet> datasets =
 * ds.loadDataSets(); if (datasets != null) { for (Iterator<IDataSet> iterator =
 * datasets.iterator(); iterator.hasNext();) { iDataSet = iterator.next();
 * configuration = iDataSet.getConfiguration(); if
 * (configuration.indexOf(fileToSearch) >= 0) { datasetUisng++; } } } } catch
 * (Exception e) {
 * logger.error("Error checking if the file is used by other datasets ", e);
 * throw new SpagoBIServiceException(getActionName(),
 * "Error checking if the file is used by other datasets ", e); } return
 * datasetUisng;
 * 
 * }
 * 
 ...
 * 
 * // check if the uploaded file is empty if (uploaded.getSize() == 0) { throw
 * new SpagoBIServiceException(getActionName(), "The uploaded file is empty"); }
 * 
 * fileExtension = uploaded.getName().lastIndexOf('.') > 0 ?
 * uploaded.getName().substring(uploaded.getName().lastIndexOf('.') + 1) : null;
 * 
 * if ("ZIP".equalsIgnoreCase(fileExtension)) {
 * logger.debug("Decompress zip file"); uploaded = unzipUploadedFile(uploaded);
 * 
 * } else if ("GZ".equalsIgnoreCase(fileExtension)) {
 * logger.debug("Decompress gzip file"); uploaded =
 * ungzipUploadedFile(uploaded); }
 * 
 * return uploaded; }
 * 
 ...
 * 
 * try { ZipInputStream zippedInputStream = new
 * ZipInputStream(uploaded.getInputStream()); ZipEntry zipEntry = null;
 * 
 ...
 * " ]");
 * 
 * if (zipEntry.isDirectory()) { throw new
 * SpagoBIServiceException(getActionName(),
 * "The uploaded file is a folder. Zip directly the file."); }
 * 
 * DiskFileItemFactory factory = new DiskFileItemFactory(); tempFileItem =
 * factory.createItem(uploaded.getFieldName(), "application/octet-stream",
 * uploaded.isFormField(), zipItemName); OutputStream tempFileItemOutStream =
 * tempFileItem.getOutputStream();
 * 
 ...
 * 
 * logger.debug("Method unzipUploadedFile(): End"); return tempFileItem;
 * 
 * } catch (Throwable t) {
 * logger.error("Error while unzip file. Invalid archive file: " + t); throw new
 * SpagoBIServiceException(getActionName(),
 * "Error while unzip file. Invalid archive file", t);
 * 
 * } }
 * 
 ...
 * FileItem tempFileItem = null;
 * 
 * try { GZIPInputStream zippedInputStream = new
 * GZIPInputStream(uploaded.getInputStream());
 * 
 * String gzipItemName = uploaded.getName().lastIndexOf('.') > 0 ?
 * uploaded.getName().substring(0, uploaded.getName().lastIndexOf('.')) : null;
 * 
 * if (gzipItemName == null || gzipItemName.equals("")) { throw new
 * SpagoBIServiceException(getActionName(), "Invalid filename for gzip file"); }
 * 
 * DiskFileItemFactory factory = new DiskFileItemFactory(); tempFileItem =
 * factory.createItem(uploaded.getFieldName(), "application/octet-stream",
 * uploaded.isFormField(), gzipItemName); OutputStream tempFileItemOutStream =
 * tempFileItem.getOutputStream();
 * 
 * IOUtils.copy(zippedInputStream, tempFileItemOutStream);
 * 
 ...
 * zippedInputStream.close();
 * 
 * logger.debug("Method ungzipUploadedFile(): End"); return tempFileItem;
 * 
 * } catch (Throwable t) {
 * logger.error("Error while unzip file. Invalid archive file: " + t); throw new
 * SpagoBIServiceException(getActionName(),
 ...
 * } }
 * 
 * private void checkDatasetFileMaxSize(FileItem uploaded, UserProfile
 * userProfile) {
 * 
 * logger.debug("Method checkDatasetFileMaxSize(): Start");
 * 
 * // first check that the user profile isn't null if (userProfile == null) {
 * throw new SpagoBIServiceException(getActionName(), "Impossible to check [ " +
 * DATASET_FILE_MAX_SIZE + "] attribute without a valide user profile"); }
 * 
 * try {
 * 
 * if (userProfile.getUserAttributes().containsKey(DATASET_FILE_MAX_SIZE)) {
 * 
 * // the user profile contains the attribute that defines dataset // file max
 * size
 * 
 * logger.debug("Method checkDatasetFileMaxSize(): Attribute [ " +
 * DATASET_FILE_MAX_SIZE + " ] defined for [" + userProfile.getUserName() +
 * " ] profile. Validation needed");
 * 
 * String datasetFileMaxSizeStr = (String)
 * userProfile.getUserAttribute(DATASET_FILE_MAX_SIZE);
 * 
 ...
 * 
 * throw new SpagoBIServiceException(getActionName(),
 * "The uploaded file exceeds the maximum size assigned to the user, that is " +
 * datasetFileMaxSize + " bytes"); } } else {
 * 
 * checkDatasetFileMaxSizeSystem(uploaded); }
 * 
 * } else { logger.debug("Method checkDatasetFileMaxSize(): Attribute [ " +
 * DATASET_FILE_MAX_SIZE + " ] not defined for [" + userProfile.getUserName() +
 * " ] profile. Check default system max dimension"); // check if the uploaded
 * file exceeds the maximum default // dimension
 * checkDatasetFileMaxSizeSystem(uploaded); }
 * 
 * logger.debug("Method checkDatasetFileMaxSize(): End"); } catch (Throwable t)
 * { logger.error("Error retrieving user attribute [ " + DATASET_FILE_MAX_SIZE +
 * " ] " + t); throw new SpagoBIServiceException(getActionName(),
 * "The uploaded file exceeds the maximum size", t);
 * 
 * }
 * 
 * }
 * 
 * private void checkDatasetFileMaxSizeSystem(FileItem uploaded) { int maxSize =
 * GeneralUtilities.getDataSetFileMaxSize(); if (uploaded.getSize() > maxSize) {
 * throw new SpagoBIServiceException(getActionName(),
 * "The uploaded file exceeds the maximum size, that is " + maxSize + " bytes");
 * } }
 * 
 * private String getActionName() { return "FILE_DATASET_UPLOAD"; } }
 */