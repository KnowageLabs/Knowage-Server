...
//			limit = getAttributeAsInteger(LIMIT);
	...
	public static String getLovResult(IEngUserProfile profile, ILovDetail lovDefinition,
			List<ObjParuse> dependencies, BIObject biObject, HttpServletRequest req, boolean retrieveIfNotcached) 
					throws Exception {
		...
		if (lovDefinition instanceof QueryDetail) {
			...
			if (cache.contains(cacheKey)) {
				...
				lovResult = lovDefinition.getLovResult(profile, dependencies, biObject.getBiObjectParameters(),
						req.getLocale());
	...
	/**
	 * This method finds out the cache to be used for lov's result cache. This key is composed mainly by the user identifier and the lov definition. 
	 * Note that, in case when the lov is a query and there is correlation, the executed statement if different from the original query
	 * (since correlation expression is injected inside SQL query using in-line view construct), therefore we should consider the modified query.
	 *
	 * @param profile
	 *            The user profile
	 * @param lovDefinition
	 ...
	 */
	private static String getCacheKey(IEngUserProfile profile, ILovDetail lovDefinition,
			List<ObjParuse> dependencies, BIObject biObject) {
...
//			LovResultCacheManager executionCacheManager = new LovResultCacheManager();
			...
			lovResult = DocumentExecutionUtils.getLovResult(
					profile,
					lovProvDet,
