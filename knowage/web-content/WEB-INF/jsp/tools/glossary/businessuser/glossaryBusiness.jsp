...
<body class="bodyStyle kn-glossary-definition">
	...
	<div ng-controller="Controller as ctrl" class="h100">
		...
		<angular-list-detail layout-fill> <list
			label="translate.load('sbi.glossary.word')"
			new-function="ctrl.newWordFunction">
		<div layout="column" layout-wrap layout-fill class="absolute">
			...
			<div layout="column" flex layout-wrap class="glossaryListBox">
				...
				<md-content flex class=" noBorder "> <angular-list
			...
			label="ctrl.selectedGloss.GLOSSARY_NM==undefined ? translate.load('sbi.glossary.select.messages') :  ctrl.selectedGloss.GLOSSARY_NM"
			ng-if="ctrl.activeTab=='Glossari'"> <md-content layout-fill
			class="absolute"> <!-- 				 	<p ng-if="ctrl.selectedGloss.GLOSSARY_NM==undefined">{{translate.load("sbi.glossary.select.messages");}}</p> -->
		...
		{{translate.load("sbi.generic.safeMode");}} </md-checkbox> <glossary-tree
			layout-fill tree-id="GlossTree" tree-options=ctrl.TreeOptions
			...
			ng-if="ctrl.selectedGloss.GLOSSARY_NM!=undefined "> </glossary-tree>


		</md-content> </detail> <detail
		...
		<form name="wordForm" class="wordForm " novalidate
			...
			<div flex="100">
				...
				<label>{{translate.load("sbi.glossary.word");}}</label> <!-- 										 <md-icon md-font-icon="fa  fa-newspaper-o " class="wo">  </md-icon>  -->
				<input ng-model="ctrl.newWord.WORD" maxlength="100" type="text">
				</md-input-container>
			</div>

			<div flex="100">
				<md-input-container class="md-block"> <label>{{translate.load("sbi.glossary.description");}}</label>
			...
			<div layout="row">
				<div flex="50">
					<md-input-container class="md-block"> <label>{{translate.load("sbi.glossary.status");}}</label>
					<md-select ng-model="ctrl.newWord.STATE"> <md-option
						value="-1"></md-option> <md-option ng-repeat="st in ctrl.state"
						value="{{st.VALUE_ID}}">
					{{translate.load(st.VALUE_NM)}} </md-option> </md-select> </md-input-container>
				</div>

				<div flex="50">
					<md-input-container class="md-block"> <label>
						{{translate.load("sbi.glossary.category");}}</label> <md-select
						ng-model="ctrl.newWord.CATEGORY"> <md-option
						value="-1"></md-option> <md-option ng-repeat="ct in ctrl.category"
						value="{{ct.VALUE_ID}}">
					{{translate.load(ct.VALUE_NM)}} </md-option> </md-select> </md-input-container>
				</div>
			</div>

			<div flex="100">
				<md-input-container class="md-block"> <label>{{translate.load("sbi.glossary.formula");}}</label>
				<textarea id="formulaText" ng-model="ctrl.newWord.FORMULA"
					columns="1" md-maxlength="500" maxlength="500"></textarea> </md-input-container>
			...
			<div layout="row" layout-wrap class="chipsNewWordDiv">
				<div flex="100">
					...
					<md-input-container class="input-chips-container md-block"
					...
					<div class="linkChips">
						<md-contact-chips ng-model="ctrl.newWord.LINK"
							...
							md-require-match="" filter-selected="true"> </md-contact-chips>
					</div>
			...
			<div layout="row">
				<div flex="40">
					...
					<md-input-container class="md-block"> <label>
						{{translate.load("sbi.glossary.attributes");}}</label> <md-select
						ng-model="ctrl.tmpAttr.Prop" md-on-open="ctrl.loadProperty()">
					<md-option value="-1"></md-option> <md-option ng-value="attr"
						ng-repeat="attr in ctrl.propertyList">
					{{attr.ATTRIBUTE_NM}} </md-option> </md-select> </md-input-container>
				</div>

				<div flex>
					<md-input-container class="md-block"> <label>{{translate.load("sbi.generic.value");}}</label>
					<textarea ng-model="ctrl.tmpAttr.Val" columns="1"
						md-maxlength="500" maxlength="500"></textarea> </md-input-container>

				</div>

				<div flex=10 flex="initial" layout-align="top center">
					...
					<md-tooltip> {{translate.load("sbi.generic.add");}} </md-tooltip> <md-icon
			...
			<div flex="100">
				...
				<md-list> <md-list-item class="md-2-line box-list-option"
				...
				<div class="md-item-text md-whiteframe-z1" flex>
					...
					<textarea class="attText" style="padding-top: 0px !important;"
						ng-model="attr.VALUE" columns="1" maxlength="500">
		...
		</md-card> </detail> <detail ng-if="false"> <md-content layout="column"
			class="absolute" layout-fill> <md-content layout-fill
			ng-if="ctrl.activeTab=='Glossari'">
		<div layout="row" layout-wrap>
			<div flex="100">
				...
				<md-checkbox ng-model="ctrl.safeMode"
					...
					class="safeModeCheckBox"
					ng-if="ctrl.selectedGloss.GLOSSARY_NM!=undefined ">
				{{translate.load("sbi.generic.safeMode");}} </md-checkbox>

				<glossary-tree tree-id="GlossTree" tree-options=ctrl.TreeOptions
		...
		<div ng-if="ctrl.activeTab=='Vocabolo'" layout="column"
			...
			<div class="md-toolbar-tools h100">
				...
				<div style="position: absolute; right: 0px" class="h100">
					<md-button type="button" tabindex="-1"
						class="md-raised md-ExtraMini " style=" margin-top: 2px;"
						ng-click="ctrl.createNewWord(true)">
					{{translate.load("sbi.browser.defaultRole.cancel");}} </md-button>
					<md-button type="button" ng-click="ctrl.addWord()"
						class="md-raised md-ExtraMini " style=" margin-top: 2px;"
			...
			<md-content flex
				style="    border: 1px solid #B0BEC5;    padding: 0px 4px;">
			...
			<form name="wordForm" class="wordForm " novalidate
				...
				<div layout="row" layout-wrap>
					<div flex="100">
						...
						<label>{{translate.load("sbi.glossary.description");}}</label> <textarea
							id="descrText" ng-model="ctrl.newWord.DESCR" columns="1"
							md-maxlength="500" maxlength="500"></textarea> </md-input-container>
					</div>
				</div>
				...
				<div layout="row" layout-wrap>
					<div flex="50">
						<md-input-container class="small"> <label>{{translate.load("sbi.glossary.status");}}</label>
						<md-select ng-model="ctrl.newWord.STATE"> <md-option
							value="-1"></md-option> <md-option ng-repeat="st in ctrl.state"
							value="{{st.VALUE_ID}}">
						{{translate.load(st.VALUE_NM)}} </md-option> </md-select> </md-input-container>
					...
					<div flex="50">
						<md-input-container class="small"> <label>
							{{translate.load("sbi.glossary.category");}}</label> <md-select
							ng-model="ctrl.newWord.CATEGORY"> <md-option
							value="-1"></md-option> <md-option
							ng-repeat="ct in ctrl.category" value="{{ct.VALUE_ID}}">
						{{translate.load(ct.VALUE_NM)}} </md-option> </md-select> </md-input-container>
					</div>
				</div>
				...
				<div layout="row" layout-wrap>
					<div flex="100">
						<md-input-container class="md-block small counter">
						<label>{{translate.load("sbi.glossary.formula");}}</label> <textarea
							id="formulaText" ng-model="ctrl.newWord.FORMULA" columns="1"
							md-maxlength="500" maxlength="500"></textarea> </md-input-container>
				...
				<div layout="row" layout-wrap>
					<div flex="100">
						...
						<md-input-container class="input-chips-container md-block small"
							ng-class="{ 'md-input-has-value-copy' : ctrl.newWord.LINK.length > 0  }">
						<label>{{translate.load("sbi.glossary.link");}}</label>
		...
		</md-content> </detail> </angular-list-detail>
