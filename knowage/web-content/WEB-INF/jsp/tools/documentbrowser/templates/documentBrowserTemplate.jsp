...
<md-content ng-controller="documentBrowserController" layout="column"
	ng-cloak layout-fill>
<div layout-fill ng-hide="hideProgressCircular"
	style="z-index: 10000; position: absolute; background-color: rgba(0, 0, 0, 0.21);">
	<md-progress-circular md-mode="indeterminate" md-diameter="60"
		style="left: 50%;top: 50%;margin-left: -30px;margin-top: -30px;">
	</md-progress-circular>
</div>

<!-- Toolbar --> <md-toolbar class="documentBrowserToolbar ">
...
</md-toolbar> <md-content layout="row" flex> <md-content layout="row"
	flex ng-show="!showSearchView"> <md-sidenav
	class="md-sidenav-left md-whiteframe-4dp" md-component-id="left"
	md-is-locked-open="$mdMedia('gt-sm')"
	ng-class="{'full-screen-sidenav': smallScreen}"> <!-- 					<md-toolbar class=" ternaryToolbar"> -->
<!-- 		       			<h3 class="md-toolbar-tools">{{translate.load("sbi.browser.filtrpanel.filtergroup.opt.folders")}}</h3> -->
<!-- 	     			</md-toolbar> --> <md-content> <document-tree
	ng-model="folders" highlights-selected-item="true" create-tree="true"
	selected-item="selectedFolder" click-function="setSelectedFolder(item)"></document-tree>
</md-content> </md-sidenav> <md-content flex layout="column" class="mainContent"> <bread-crumb
	item-name='name' ng-model="breadModel" selected-item="selectedFolder"
...
<div layout="row" ng-show='showDocumentGridView' class="documentGridViewInput">
	...
	<md-select ng-model="selectedOrder"
		...
		ng-repeat="orderElement in orderElements"
		value="{{orderElement.name}}">{{orderElement.label}}</md-option> </md-select> </md-input-container>
</div>
...
<!-- From Andrea		I have deleted option style='overflow:auto' - --> <!-- 					because on IE the document-view break layout -->
<!-- 					when pass the mouse over the "play button" --> <document-view
	...
	ordering-document-cards=selectedOrder> </document-view> </md-whiteframe> </md-content> </md-content> <md-content
...
<h3 class="md-title" ng-show="searchDocuments.length > 0">{{searchDocuments.length
	|| 0}} {{translate.load("sbi.browser.document.found")}}</h3>
...
</md-list-item> </md-list> </md-content> </md-sidenav> </md-content> </md-content>
