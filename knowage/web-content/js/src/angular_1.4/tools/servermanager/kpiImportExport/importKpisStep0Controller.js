angular.module('impExpKpis').controller('importKpiControllerStep0', ['sbiModule_download','sbiModule_device',"$scope", "$mdDialog", "$timeout", "sbiModule_logger", "sbiModule_translate","sbiModule_restServices","sbiModule_config","importExportDocumentModule_importConf","$mdToast",importKpiStep0FuncController]);
 
function importKpiStep0FuncController(sbiModule_download,sbiModule_device,$scope, $mdDialog, $timeout, sbiModule_logger, sbiModule_translate, sbiModule_restServices,sbiModule_config,importExportDocumentModule_importConf,$mdToast) {
	$scope.upload = function(ev){
		if($scope.IEDConf.fileImport.fileName == "" || $scope.IEDConf.fileImport.fileName == undefined){
			...
			.success(function(data, status, headers, config) {
				...
				//	if($scope.checkRole()){
						$scope.IEDConf.roles.exportedKpi = data.exportedKpi;
				//	}
					
					
					$scope.stepControl.insertBread({name: sbiModule_translate.load('SBISet.impexp.exportedKpis','component_impexp_messages')});
