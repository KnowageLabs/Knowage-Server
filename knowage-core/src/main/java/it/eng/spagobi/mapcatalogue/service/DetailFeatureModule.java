...
public class DetailFeatureModule extends AbstractHttpModule {
	...
	private void sendError(ServletOutputStream out) {
		try {
			...
			TracerSingleton.log(SpagoBIConstants.NAME_MODULE, TracerSingleton.MAJOR,
					"DetailFeatureAction :: sendError : " + "Unable to write into output stream ", e);
		}
	}

	/**
	 * Gets the detail of a feature choosed by the user from the features list. It reaches the key from the request and asks to the DB all detail feature
	 * information, by calling the method <code>loadFeatureByID</code>.
	 *
	 ...
	 * @throws SourceBeanException If a SourceBean Exception occurs
	 */
	private void delDetailFeature(SourceBean request, String mod, SourceBean response) throws EMFUserError, SourceBeanException {
		IEngUserProfile profile = (IEngUserProfile) this.getHttpRequest().getSession().getAttribute(IEngUserProfile.ENG_USER_PROFILE);
		try {
