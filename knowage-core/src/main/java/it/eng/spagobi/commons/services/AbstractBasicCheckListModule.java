...
public class AbstractBasicCheckListModule extends AbstractListLookupModule {
	...
	public static void clearSession(SessionContainer session, String moduleName) {
		...
		// TODO patch this
		SourceBean moduleConfig = (SourceBean) spagoBiConfig.getFilteredSourceBeanAttribute("MODULES.MODULE", "NAME",
				moduleName);
	...
	public String getObjectKey(SourceBean object) {
		...
		objectIdValue = SpagoBIUtilities.encode(objectIdValue);
	...
	public void createCheckedObjectMap(SourceBean request) throws Exception {
		...
		try {
			...
			String tmpElements = (request.getAttribute("checkedElements") == null) ? ""
					: (String) request.getAttribute("checkedElements");
			...
			SpagoBITracer.major(SpagoBIConstants.NAME_MODULE, this.getClass().getName(), "createCheckedObjectMap",
					e.getMessage(), e);
		} finally {
	...
	public void preprocess(SourceBean request) throws Exception {
		...
		if (getRequestContainer().getSessionContainer().getAttribute(CHECKED_OBJECTS) != null) {
			...
			if (checkedObjectsMap != null) {
				...
				Object[] arrElements = colAllElements.toArray();
				for (int i = 0; i < arrElements.length; i++)
					allElements.add(arrElements[i]);
			}
			pageNumber = 1;
	...
	public void service(SourceBean request, SourceBean response) throws Exception {
		...
		if (message == null || message.equalsIgnoreCase("INIT_CHECKLIST")) {
			...
			if (request.getAttribute("save") != null) {
				...
				request.updAttribute("LIST_PAGE", Integer.toString(pageNumber));
				super.service(request, response);
				postprocess(response);
				response.setAttribute("PUBLISHER_NAME", "CheckLinksDefaultPublischer");
				return;
			...
			if (request.getAttribute("checkFilter") != null
					&& request.getAttribute("checkFilter").equals("checkFilter")) {
