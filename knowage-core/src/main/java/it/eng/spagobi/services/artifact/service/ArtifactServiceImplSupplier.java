...
public class ArtifactServiceImplSupplier {
	...
	public DataHandler getArtifactContentByNameAndType(String name, String type) {
		...
		try {
			...
			Content content = artdao.loadArtifactContentById(artifact.getId());
	...
	public SpagoBIArtifact[] getArtifactsByType(String type) {
		...
		try {
			...
			for (int i = 0; i < list.size(); i++) {
				...
				toReturn[i] = new SpagoBIArtifact(artifact.getId(), artifact.getName(), artifact.getType(),
						artifact.getDescription(), artifact.getCurrentContentId());
			}
			return toReturn;
		} catch (Exception e) {
