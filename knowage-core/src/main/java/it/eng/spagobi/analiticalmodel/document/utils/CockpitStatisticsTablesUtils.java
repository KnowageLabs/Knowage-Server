...
public class CockpitStatisticsTablesUtils {
	...
	private static final Logger LOGGER = Logger.getLogger(CockpitStatisticsTablesUtils.class);
	...
	public static void deleteCockpitWidgetsTable(BIObject biObject, Session session, boolean isImport) {
		...
		List<SbiCockpitWidget> sbiCockpitWidgetList = session.createCriteria(SbiCockpitWidget.class)
				.add(Restrictions.eq("biobjId", biobjId)).list();
		for (SbiCockpitWidget sbiCockpitWidget : sbiCockpitWidgetList) {

			LOGGER.debug(
					String.format("Removing SbiCockpitWidget with id [%s]", sbiCockpitWidget.getSbiCockpitWidgetId()));
		...
		List<SbiCockpitAssociation> sbiCockpitAssociationList = session.createCriteria(SbiCockpitAssociation.class)
				.add(Restrictions.eq("biobjId", biobjId)).list();
		...
		for (SbiCockpitAssociation sbiCockpitAssociation : sbiCockpitAssociationList) {
			LOGGER.debug(String.format("Removing SbiCockpitAssociation with id [%s]",
					sbiCockpitAssociation.getSbiCockpitAssociationId()));
	...
	public static void updateCockpitWidgetsTable(BIObject biObject, Session session, boolean isImport) {
		...
		SbiObjects sbiObjects = (SbiObjects) session.createCriteria(SbiObjects.class)
				.add(Restrictions.eq("biobjId", biObject.getId())).uniqueResult();
		Transaction tx = null;
		if (!isImport) {
			tx = session.beginTransaction();
		}
	...
	public static void parseTemplate(SbiObjects sbiObjects, Session session, boolean initializer) {
		...
		if (sbiObjTemplates == null) {
			...
			try {
				...
				Set<String> associationsMap = handleDatasetAssociations(sbiObjects, template, session, dataSetMap,
						initializer);

				JSONArray sheets = template.optJSONArray("sheets");
				if (sheets != null) {
					LOGGER.debug(String.format("Document with label [%s] has [%s] sheets", sbiObjects.getLabel(),
							sheets.length()));
					...
					for (int i = 0; i < sheets.length(); i++) {
						...
						LOGGER.debug(String.format("Document with label [%s] has [%s] widgets", sbiObjects.getLabel(),
								widgets.length()));
						...
						for (int j = 0; j < widgets.length(); j++) {
							...
							LOGGER.debug(String.format("Parsing widget with id [%s] of type [%s]", widget.get("id"),
									widgetType));
							...
							if (widgetType.equals("map")) {
								...
								LOGGER.debug(
										String.format("Found [%s] dataset ids in content JSON object", dsIds.size()));
								for (Integer dsId : dsIds) {
									...
									session.flush();
									LOGGER.debug(String.format("Persisted SbiCockpitWidget with id [%s] to database",
											sbiCockpitWidget.getSbiCockpitWidgetId()));
								}
							} else {
								SbiCockpitWidget sbiCockpitWidget = new SbiCockpitWidget();
								...
								updateSbiCommonInfo4Insert(sbiObjects, sbiCockpitWidget, initializer);
								sbiCockpitWidget.getCommonInfo()
										.setOrganization(sbiObjects.getCommonInfo().getOrganization());
								session.save(sbiCockpitWidget);
								session.flush();
								LOGGER.debug(String.format("Persisted SbiCockpitWidget with id [%s] to database",
										sbiCockpitWidget.getSbiCockpitWidgetId()));
	...
	private static void setIsAssociative(Set<String> associationsMap, String datasetLabel,
			SbiCockpitWidget sbiCockpitWidget) {
		...
		if (datasetLabel != null) {
			...
			LOGGER.debug(String.format("Dataset with label [%s] is %s in associative logic", datasetLabel,
					isAssociative ? "" : "NOT"));
		}
	}

	private static void setUseCache(Map<Integer, JSONObject> dataSetMap, String datasetLabel,
			SbiCockpitWidget sbiCockpitWidget) {
	...
	private static Map<Integer, JSONObject> getDatasetMap(SbiObjects sbiObjects, JSONObject template, Session session)
			...
			if (datasets != null) {
				for (int index = 0; index < datasets.length(); index++) {
					...
					if (datasetInConf != null && datasetInConf.has("dsId")) {
						...
						LOGGER.debug(String.format("Added dataset with id [%s] and label [%s] to datasetMap", dsId,
								datasetInConf.get("dsLabel")));
					}
				}
			}
		}
	...
	private static String parseFiltersObject(SbiCockpitWidget sbiCockpitWidget, String datasetLabel,
			JSONObject widgetOrContent) throws JSONException {
		LOGGER.debug("IN");
		// FILTERS
				...
				if (filter != null && filter.has("dataset")) {
					...
					if (contDataset != null && contDataset.has("dsId")) {
						...
						if (dsId != 0) {
							...
							LOGGER.debug(String.format("Widget has dataset with id [%s] and label [%s] associated",
									dsId, datasetLabel));
						}
					}
				}
			...
			if (layers != null) {
				for (int l = 0; l < layers.length(); l++) {
					...
					if (layer != null && layer.has("dsId")) {
						...
						if (dsId != 0) {
							dsIds.add(dsId);
							String label = layer.optString("label");
							LOGGER.debug(String.format("Map widget has dataset with [%s] and label [%s] associated",
									dsId, label));
						}
					}
				}
	...
	private static String parseContentObject(SbiCockpitWidget sbiCockpitWidget, String datasetLabel,
			JSONObject widget) {
		LOGGER.debug("IN");
		// CONTENT
		JSONObject content = widget.optJSONObject("content");
	...
	private static String parseDatasetObject(SbiCockpitWidget sbiCockpitWidget, String datasetLabel, JSONObject widget,
			Map<Integer, JSONObject> dataSetMap) throws JSONException {
		LOGGER.debug("IN");
		...
		if (dataset != null) {
			...
			if (oldId != null && oldId != 0) {
				...
				LOGGER.debug(String.format("Widget has dataset with id [%s] and label [%s] associated", oldId,
						datasetLabel));
			}
		}
		LOGGER.debug("OUT");
	...
	private static Set<String> handleDatasetAssociations(SbiObjects sbiObjects, JSONObject template, Session session,
			Map<Integer, JSONObject> dataSetMap, boolean initializer) throws JSONException {
		LOGGER.debug("IN");
		Set<String> associationsMap = new HashSet<>();
		try {
			JSONObject configuration = template.optJSONObject("configuration");
			if (configuration != null) {
				...
				if (isAssociative) {
					for (int k = 0; k < associations.length(); k++) {
						LOGGER.debug("Found " + associations.length() + " dataset associations in document with label "
								+ sbiObjects.getLabel());
						...
						if (fields != null) {
							...
							for (int i = 0; i < fields.length() - 1; i++) {
								...
								sbiCockpitAssociation.getCommonInfo()
										.setOrganization(sbiObjects.getCommonInfo().getOrganization());
								session.save(sbiCockpitAssociation);
								session.flush();

								LOGGER.debug(String.format("Field [%s-%s] associated with field [%s-%s] in association",
										fromDataset, fromColumn, toDataset, toColumn));

							}
							String associationDescription = association.getString("description");
							associationsMap.add(associationDescription);
							LOGGER.debug(String.format("Added association with description [%s] to associationMap",
	 ...
	 * @param b
	 */
	private static void updateSbiCommonInfo4Insert(SbiObjects sbiObjects, SbiHibernateModel sbiCockpitWidget,
			boolean initializer) {
	...
	private CockpitStatisticsTablesUtils() {
