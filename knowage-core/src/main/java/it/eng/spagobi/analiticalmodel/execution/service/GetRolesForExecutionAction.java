	...
	public void doService() {
		...
		try {
			...
			if (!StringUtils.isEmpty(documentLabel)) {
				...
				try {
					...
					throw new SpagoBIServiceException(SERVICE_NAME,
							"Object with label equals to [" + documentLabel + "] not found", error);
				}
			...
			Assert.assertNotNull(obj, "Impossible to load document ["
					+ (StringUtils.isEmpty(documentLabel) ? documentId : documentLabel) + "]");
			...
			if (roleNames == null || roleNames.isEmpty()) {
				...
				while (it.hasNext()) {
					...
					try {
						...
						throw new SpagoBIServiceException(SERVICE_NAME,
								"Role with name equals to [" + roleName + "] not found", error);
					}
			...
			try {
				JSONArray dataJSON = (JSONArray) SerializerFactory.getSerializer("application/json").serialize(roles,
						null);
				rolesJSON = (JSONObject) JSONStoreFeedTransformer.getInstance().transform(dataJSON, "id", "name",
						"description", new String[] { "id", "name", "description" }, new Integer(roleNames.size()));
