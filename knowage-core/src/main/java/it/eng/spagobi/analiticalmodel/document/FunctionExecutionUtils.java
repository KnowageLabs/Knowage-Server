...
public class FunctionExecutionUtils {
	...
	public static HashMap<String, Object> getLovDefaultValues(String executionRole, BIObject biObject, BIObjectParameter objParameter, JSONObject requestVal,
		...
		try {
			...
			if (lovProvDet.getLovType() != null && lovProvDet.getLovType().contains("tree")) {
				// valuesJSONArray = getChildrenForTreeLov(lovProvDet, rows, mode, treeLovNodeLevel, treeLovNodeValue); //c'era
			} else {
	 ...
	 * valueJSON.put("id", attribute.getValue() + NODE_ID_SEPARATOR + (treeLovNodeLevel + 1)); notNullNode = true; } valueJSON.put("leaf", true); } else if
