...
public class GetMetadataAction extends AbstractSpagoBIAction {
	...
	public void doService() {
		...
		try {
			...
			HttpServletRequest httpRequest = getHttpssssRequest();
			...
			/*
			 * //Obj Rating ObjMetadata metaObjRating = new ObjMetadata(); String textRating = msgBuild.getMessage(RATING, locale);
			 * metaObjRating.setName(textRating); metaObjRating.setDataTypeCode("GENERAL_META"); ObjMetacontent metaContentObjRating = new ObjMetacontent();
			 * Double temp = DAOFactory.getBIObjectRatingDAO().calculateBIObjectRating(obj); String docRating = ( temp != null ? temp.toString() : "" );
			 * metaContentObjRating.setContent(docRating.getBytes("UTF-8")); ObjMetaDataAndContent metaAndContentObjRating = new ObjMetaDataAndContent();
			 * metaAndContentObjRating.setMeta(metaObjRating); metaAndContentObjRating.setMetacontent(metaContentObjRating);
