 ...
 *
 *         This module read the parameters values. It is invoked by a lookup window from execution parameters form; the result is published into the same
 *         window.
 *
 */
...
public class SelectParametersLookupModule extends AbstractBasicListModule {
	...
	public ListIFace getList(SourceBean request, SourceBean response) throws Exception {
		...
		if (selectionType != null && selectionType.equals("CHECK_LIST"))
			isChecklist = true;
	 ...
	 *            The modality in use
	 * @param request
	 *            The SourceBean request
	 * @param list
	 *            The list to be filtered
	 * @param parametersMap
	 *            The map for the parameters to be propagated into the list
	 * @return the filtered list according to the parameters correlation
	 * @throws EMFUserError
	 */
	public static ListIFace filterListForParametersCorrelation(ParameterUse paruse, SourceBean request, ListIFace list, HashMap parametersMap,
			EMFErrorHandler errorHandler) throws EMFUserError {
		logger.debug("IN");
	...
	private static ListIFace evaluateExpression(String expr, ListIFace list, List ops, SourceBean request, HashMap parametersMap,
	...
	private static ListIFace evaluateSingleLogicOperation(ObjParuse obpuLeft, ObjParuse obpuRight, ListIFace list, SourceBean request, HashMap parametersMap,
			EMFErrorHandler errorHandler) {
	...
	private static ListIFace filterForCorrelation(ListIFace list, ObjParuse objParuse, SourceBean request, HashMap parametersMap,
			EMFErrorHandler errorHandler) {
		try {
			...
			case 1:
				...
				if (valueFilter != null && !valueFilter.equals(""))
					return DelegatedBasicListService.filterList(list, valueFilter, valueTypeFilter, objParuse.getFilterColumn(), objParuse.getFilterOperation(),
							errorHandler);
				else
					return list;
			default:
				return DelegatedBasicListService.filterList(list, valuesFilter, valueTypeFilter, objParuse.getFilterColumn(), objParuse.getFilterOperation(),
						errorHandler);
			}
		} catch (Exception e) {
	...
	private ListIFace loadList(SourceBean request, SourceBean response, Integer parId, String roleName) throws Exception {
		logger.debug("IN");
		...
		if (isChecklist) {
			...
			moduleConfigStr.append("				var valuesArrayStr = parent.document.getElementById('<PARAMETER name='" + RETURN_PARAM
					+ "' scope='SERVICE_REQUEST'/>').value;");
			...
			moduleConfigStr.append("				var descriptionsArrayStr = parent.document.getElementById('<PARAMETER name='" + RETURN_PARAM
					+ "' scope='SERVICE_REQUEST'/>Desc').value;");
			...
			moduleConfigStr.append(
					"				if (descriptionsArray == null || (descriptionsArray.length == 1 && descriptionsArray[0] == '')) descriptionsArray = new Array();");
			...
			moduleConfigStr.append("				parent.document.getElementById('<PARAMETER name='" + RETURN_PARAM
					+ "' scope='SERVICE_REQUEST'/>').value = valuesArray.join(';');");
			moduleConfigStr.append("				parent.document.getElementById('<PARAMETER name='" + RETURN_PARAM
					+ "' scope='SERVICE_REQUEST'/>Desc').value = descriptionsArray.join(';');");
			...
			// sets value and its description on parameters form (that is on parent window)
			moduleConfigStr.append("				parent.document.getElementById('<PARAMETER name='" + RETURN_PARAM
					+ "' scope='SERVICE_REQUEST'/>').value='<PARAMETER name='" + valColName + "' scope='LOCAL'/>';");
			moduleConfigStr.append("				parent.document.getElementById('<PARAMETER name='" + RETURN_PARAM
					+ "' scope='SERVICE_REQUEST'/>Desc').value='<PARAMETER name='" + descriptionColName + "' scope='LOCAL'/>';");
			// hides this window
	...
	private SourceBean executeQuery(String lovProvider, SourceBean response, IEngUserProfile profile) throws Exception {
		...
		/*
		 * if (qd.requireProfileAttributes()) { String message = PortletUtilities.getMessage("scheduler.noProfileAttributesSupported",
		 * "component_scheduler_messages"); response.setAttribute(SpagoBIConstants.MESSAGE_INFO, message); return result; }
		 */
	...
	private SourceBean executeFixedList(String lovProvider, SourceBean response, IEngUserProfile profile) throws Exception {
		logger.debug("IN");
		logger.debug("lovProvider." + lovProvider);
	...
	private SourceBean executeJavaClass(String lovProvider, SourceBean response, IEngUserProfile profile) throws Exception {
		logger.debug("IN");
		SourceBean resultSB = null;
