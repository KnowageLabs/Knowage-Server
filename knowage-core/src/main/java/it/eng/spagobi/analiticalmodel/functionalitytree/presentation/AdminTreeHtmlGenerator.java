...
/**
 * Contains all methods needed to generate and modify a tree object for Administration. There are methods to generate tree, configure, insert and modify
 * elements.
 */
public class AdminTreeHtmlGenerator implements ITreeHtmlGenerator {
	...
	private void makeJSFunctionForMenu(StringBuffer htmlStream) {
		...
		htmlStream.append(
				"			if(urlExecution!='') divM.innerHTML = divM.innerHTML + '<div onmouseout=\"this.style.backgroundColor=\\'white\\'\"  onmouseover=\"this.style.backgroundColor=\\'#eaf1f9\\'\" ><a class=\"dtreemenulink\" href=\"'+urlExecution+'\">"
						+ capExec + "</a></div>';\n");
		/*
		 * String capMetadata = msgBuilder.getMessage("SBISet.objects.captionMetadata", "messages", httpRequest); htmlStream.
		 * append("			if(urlMetadata!=''){ divM.innerHTML = divM.innerHTML + '<div onmouseout=\"this.style.backgroundColor=\\'white\\'\"  onmouseover=\"this.style.backgroundColor=\\'#eaf1f9\\'\" ><a class=\"dtreemenulink\" href=\"javascript:makePopup(\\''+prog+'\\',\\''+urlMetadata+'\\')\" >"
		 * +capMetadata+"</a></div>'; }\n");
		 */
	...
	/**
	 * The function makeTree builds the tree. It makes a separate root for user personal folders (the reason is that admin can view all personal folders).
	 *
	 * @param objectsList
	 *            the objects list
	 * @param httpReq
		...
		while (it.hasNext()) {
			...
			if (initialPath != null) {
				if (initialPath.equalsIgnoreCase(folder.getPath()))
					addItemForJSTree(htmlStream, folder, false, true);
				else
					addItemForJSTree(htmlStream, folder, false, false);
			} else {
				if (folder.getParentId() == null)
					addItemForJSTree(htmlStream, folder, true, false);
				else
					addItemForJSTree(htmlStream, folder, false, false);
			}
		}
	...
	private void addItemForJSTree(StringBuffer htmlStream, LowFunctionality folder, boolean isRoot, boolean isInitialPath) {
		...
		if (isInitialPath)
			parentId = new Integer(dTreeRootId);
		else
				...
				for (Iterator it = objects.iterator(); it.hasNext();) {
					...
					if (biObjState.equals("DEV")) {
						...
						htmlStream.append(treeName + ".add(" + dTreeObjects-- + ", " + idFolder + ",'<img src=\\'" + stateIcon + "\\' /> " + localizedName
								+ "', 'javascript:linkEmpty()', '', '', '" + userIcon + "', '', '', 'menu" + requestIdentity + "(" + prog + ", event, \\'"
								+ createExecuteObjectLink(idObj) + "\\',\\'" + createMetadataObjectLink(idObj) + "\\', \\'" + createDetailObjectLink(idObj)
								+ "\\', \\'" + createEraseObjectLink(idObj, idFolder) + "\\', \\'" + createMoveDownObjectLink(idObj) + "\\', \\'"
								+ createMoveUpObjectLink(idObj) + "\\')' );\n");
						...
						htmlStream.append(treeName + ".add(" + dTreeObjects-- + ", " + idFolder + ",'<img src=\\'" + stateIcon + "\\' /> " + localizedName
								+ "', 'javascript:linkEmpty()', '', '', '" + userIcon + "', '', '', 'menu" + requestIdentity + "(" + prog + ", event, \\'"
								+ createExecuteObjectLink(idObj) + "\\', \\'" + createMetadataObjectLink(idObj) + "\\',\\'" + createDetailObjectLink(idObj)
								+ "\\', \\'" + createEraseObjectLink(idObj, idFolder) + "\\', \\'\\', \\'\\')' );\n");
					}
				}
		...
		if (codeType.equalsIgnoreCase(SpagoBIConstants.USER_FUNCTIONALITY_TYPE_CODE)) {
			...
			htmlStream.append(treeName + ".add(" + idFolder + ", " + dMyFolderId + ",'" + name + "', '', '', '', '" + imgFolder + "', '" + imgFolderOp
					+ "', '', 'true');\n");
