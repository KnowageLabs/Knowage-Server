...
public class KpiListener extends AbstractAlertListener {
	...
	public void executeListener(String jsonParameters) throws AlertListenerException {
		...
		List<KpiValue> resultSet = DAOFactory.getKpiDAO().findKpiValues(par.getKpiId(), par.getKpiVersion(), null,
				false, null, null, new HashMap());
		if (resultSet != null && !resultSet.isEmpty()) {
			...
			writeAlertLog(JsonConverter.objectToJson(par, par.getClass()), null, null,
					message.getMessage("KpiListener.noResult"));
		}

		logger.info("KpiListener ended");
	}

	private void loadThresholdMap(InputParameter par) {
	...
	private String makeJsonKpiInfo(List<Integer> thresholdValues) throws JSONException {
		logger.debug("IN");
			...
			if (values != null && !values.isEmpty()) {
				...
				for (ThresholdValue tValue : thresholds) {
					if (tValue.getId().equals(thresholdId)) {
						...
						sb.append(clean(tValue.getColor()).isEmpty() ? "<td></td>"
								: "<td style=\"background-color:" + clean(tValue.getColor()) + ";\"></td>");
