	...
	public String getHierarchiesTechnical(@QueryParam("dimension") String dimension) {
		...
		try {
			...
			String hierarchyCodeColumn = AbstractJDBCDataset.encapsulateColumnName(HierarchyConstants.HIER_CD,
					dataSource);
			String hierarchyNameColumn = AbstractJDBCDataset.encapsulateColumnName(HierarchyConstants.HIER_NM,
					dataSource);
			...
			String queryText = "SELECT DISTINCT(" + hierarchyCodeColumn + ")," + columns + " FROM " + tableName
					+ " WHERE " + typeColumn + "=\'TECHNICAL\' AND (" + bkpColumn + "= 0 OR " + bkpColumn
