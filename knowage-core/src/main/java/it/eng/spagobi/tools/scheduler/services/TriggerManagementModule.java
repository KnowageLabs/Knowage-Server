...
public class TriggerManagementModule extends AbstractHttpModule {
	...
	public void service(SourceBean request, SourceBean response) throws Exception {
		...
		profile = (IEngUserProfile) sessionContainer.getPermanentContainer()
	...
	private void runSchedule(SourceBean request, SourceBean response) throws EMFUserError {
		...
		try {
			...
			IEngUserProfile currProfile = (IEngUserProfile) permSess.getAttribute(IEngUserProfile.ENG_USER_PROFILE);
			...
			StringBuilder message = createMessageSaveSchedulation(triggerInfo, true, currProfile);
			...
			try {
				AuditLogUtilities.updateAudit(getHttpRequest(), currProfile, "SCHED_TRIGGER.RUN", logParam, "OK");
			} catch (Exception e) {
				try {
	...
	private void getSchedule(SourceBean request, SourceBean response) throws EMFUserError {
		try {
			...
			if (triggerDetailSB != null) {
				if (jobDetailSB != null) {
					TriggerInfo tInfo = SchedulerUtilities.getTriggerInfoFromTriggerSourceBean(triggerDetailSB,
							jobDetailSB);
			...
			List<LowFunctionality> functionalities = DAOFactory.getLowFunctionalityDAO()
					.loadAllLowFunctionalities(false);
			response.setAttribute(SpagoBIConstants.FUNCTIONALITIES_LIST, functionalities);
			List<IDataSet> allDatasets = DAOFactory.getDataSetDAO().loadDataSets();
	...
	private void saveScheduleForJob(SourceBean request, SourceBean response) throws EMFUserError {
		...
		try {
			...
			StringBuilder message = createMessageSaveSchedulation(triggerInfo, false, profile);
	...
	private void getSaveAsSnapshotOptions(SourceBean request, DispatchContext dispatchContext, int biobId, int index) {
		...
		if (saveassnap != null) {
			...
			String snapshotHistoryLength = (String) request
					.getAttribute("snapshothistorylength_" + biobId + "__" + index);
	...
	private void getSaveAsFileOptions(SourceBean request, DispatchContext dispatchContext, int biobId, int index) {
		...
		if (saveasfile != null) {
			...
			boolean zipFileDocument = "true"
					.equalsIgnoreCase((String) request.getAttribute("zipFileDocument_" + biobId + "__" + index));
	...
	private void getSaveAsJavaClassOptions(SourceBean request, DispatchContext dispatchContext, int biobId, int index) {
		...
		if (sendToJavaClass != null) {
			...
			try {
				...
				EMFValidationError emfError = new EMFValidationError(EMFErrorSeverity.ERROR,
						"sendtojavaclass_" + biobId + "__" + index, "12200");
				errorHandler.addError(emfError);

			} catch (Exception e) {
				logger.error("Error in istantiating class");
				EMFValidationError emfError = new EMFValidationError(EMFErrorSeverity.ERROR,
						"sendtojavaclass_" + biobId + "__" + index, "12100");
				errorHandler.addError(emfError);
			}
	...
	private void getSaveAsDocumentOptions(SourceBean request, DispatchContext dispatchContext, int biobId, int index)
			throws EMFUserError {
		...
		if (saveasdoc != null) {
			...
			boolean useFixedFolder = "true"
					.equalsIgnoreCase((String) request.getAttribute("useFixedFolder_" + biobId + "__" + index));
			...
			boolean useFolderDataset = "true"
					.equalsIgnoreCase((String) request.getAttribute("useFolderDataset_" + biobId + "__" + index));
			...
			if (useFolderDataset) {
				...
				String datasetParameterLabel = (String) request
						.getAttribute("datasetFolderParameter_" + biobId + "__" + index);
				...
				if (dsLabel == null || dsLabel.trim().equals("")) {
					...
					this.getErrorHandler().addError(new EMFValidationError(EMFErrorSeverity.ERROR, null,
							"errors.trigger.missingDataSet", params, "component_scheduler_messages"));
				}
				if (datasetParameterLabel == null || datasetParameterLabel.trim().equals("")) {
					...
					params.add(biobj.getName());
					this.getErrorHandler().addError(new EMFValidationError(EMFErrorSeverity.ERROR, null,
							"errors.trigger.missingDataSetParameter", params, "component_scheduler_messages"));
				}
	...
	private void getSaveAsMailOptions(SourceBean request, DispatchContext dispatchContext, int biobId, int index)
			throws EMFUserError {
		String sendmail = (String) request.getAttribute("sendmail_" + biobId + "__" + index);
		if (sendmail != null) {
			dispatchContext.setMailDispatchChannelEnabled(true);
			boolean useFixedRecipients = "true"
					.equalsIgnoreCase((String) request.getAttribute("useFixedRecipients_" + biobId + "__" + index));
			...
			if (useFixedRecipients) {
				...
				if (mailtos == null || mailtos.trim().equals("")) {
					...
					params.add(biobj.getName());
					this.getErrorHandler().addError(new EMFValidationError(EMFErrorSeverity.ERROR, null,
							"errors.trigger.missingFixedRecipients", params, "component_scheduler_messages"));
				}
			}
			boolean useDataset = "true"
					.equalsIgnoreCase((String) request.getAttribute("useDataset_" + biobId + "__" + index));
			...
			if (useDataset) {
				...
				String datasetParameterLabel = (String) request
						.getAttribute("datasetParameter_" + biobId + "__" + index);
			...
			boolean useExpression = "true"
					.equalsIgnoreCase((String) request.getAttribute("useExpression_" + biobId + "__" + index));
			...
			if (useExpression) {
				...
				if (expression == null || expression.trim().equals("")) {
					...
					params.add(biobj.getName());
					this.getErrorHandler().addError(new EMFValidationError(EMFErrorSeverity.ERROR, null,
							"errors.trigger.missingExpression", params, "component_scheduler_messages"));
				}
			}
			...
			if (!useFixedRecipients && !useDataset && !useExpression) {
				...
				params.add(biobj.getName());
				this.getErrorHandler().addError(new EMFValidationError(EMFErrorSeverity.ERROR, null,
						"errors.trigger.missingRecipients", params, "component_scheduler_messages"));
			}
			...
			// Mail
			boolean zipMailDocument = "true"
					.equalsIgnoreCase((String) request.getAttribute("zipMailDocument_" + biobId + "__" + index));
			dispatchContext.setZipMailDocument(zipMailDocument);

			boolean reportNameInSubject = "true"
					.equalsIgnoreCase((String) request.getAttribute("reportNameInSubject_" + biobId + "__" + index));
			dispatchContext.setReportNameInSubject(reportNameInSubject);

			boolean uniqueMail = "true"
					.equalsIgnoreCase((String) request.getAttribute("uniqueMail_" + biobId + "__" + index));
	...
	private void getSaveAsDistributionListOptions(SourceBean request, DispatchContext dispatchContext, int biobId,
			int index) throws EMFUserError {
		...
		if (sendtodl != null) {
			...
			List<DistributionList> dlist = DAOFactory.getDistributionListDAO().loadAllDistributionLists();
			Iterator<DistributionList> it = dlist.iterator();
			while (it.hasNext()) {
	...
	private void getTriggersForJob(SourceBean request, SourceBean response) throws EMFUserError {
		...
		try {
			...
			// ordering of list
			String typeOrder = (request.getAttribute("TYPE_ORDER") == null) ? " ASC"
					: (String) request.getAttribute("TYPE_ORDER");
			String fieldOrder = (request.getAttribute("FIELD_ORDER") == null) ? " triggerDescription"
	...
	private SourceBean orderJobList(SourceBean pageListSB, String typeOrder, String fieldOrder) throws EMFUserError {
		try {
			...
			while (tmpFieldList != null && !tmpFieldList.isEmpty()) {
	...
	// SERIALIZER
	// ==========================================================================================================
	private StringBuilder createMessageSaveSchedulation(TriggerInfo triggerInfo, boolean runImmediately,
	...
	private void serializeSaveParameterOptions(StringBuilder message, TriggerInfo triggerInfo, boolean runImmediately,
			IEngUserProfile profile) throws EMFUserError {
		...
		for (String uniqueDispatchContextName : uniqueDispatchContextNames) {
			...
			saveOptString += serializeSaveAsDistributionListOptions(dispatchContext, uniqueDispatchContextName,
					triggerInfo, runImmediately, profile);
	...
	private String serializeSaveAsDistributionListOptions(DispatchContext dispatchContext,
			String uniqueDispatchContextName, TriggerInfo triggerInfo, boolean runImmediately, IEngUserProfile profile)
			throws EMFUserError {
		String saveOptString = "";
		...
		if (dispatchContext.isDistributionListDispatchChannelEnabled()) {
			...
			if (!runImmediately) {
				...
				List<BIObject> biObjects = jobInfo.getDocuments();
				Iterator<BIObject> iterbiobj = biObjects.iterator();
				int index = 0;
				while (iterbiobj.hasNext()) {
					index++;
					BIObject biobj = iterbiobj.next();
					String objpref = biobj.getId().toString() + "__" + Integer.toString(index);
					if (uniqueDispatchContextName.equals(objpref)) {

						List<BIObjectParameter> pars = biobj.getDrivers();
						Iterator<BIObjectParameter> iterPars = pars.iterator();
						String queryString = "";
						while (iterPars.hasNext()) {
							BIObjectParameter biobjpar = iterPars.next();
