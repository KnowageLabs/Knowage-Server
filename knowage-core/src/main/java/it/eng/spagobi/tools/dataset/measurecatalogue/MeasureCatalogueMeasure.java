...
public class MeasureCatalogueMeasure implements IMeasureCatalogueField {
	...
	private MeasureCatalogueMeasure(MetaModelWrapper metaModel) {
		...
		datasetDimensions = new HashSet<>();
		measureProperties = new HashMap<>();
	}

	public MeasureCatalogueMeasure(IFieldMetaData fieldMetadata, MetaModelWrapper metaModel, IDataSet ds,
			Set<MeasureCatalogueDimension> datasetDimension) {
		this(metaModel);
		Assert.assertNotNull(fieldMetadata, "The field metadata of the measure is null");
		this.fieldMetadata = fieldMetadata;
		this.alias = fieldMetadata.getAlias();
		if (this.alias == null) {
			this.alias = fieldMetadata.getName();
		}
		Assert.assertNotNull(this.alias, "The alias of the field is null");
	...
	public void refreshDataSet(IDataSet ds) {
		...
		this.dataset = ds;
		Set<MeasureCatalogueDimension> dimensions = new HashSet<>();
	...
	public static boolean isMeasure(IFieldMetaData fieldMetadata) {
		boolean isMeasure = (fieldMetadata.getFieldType() != null
				&& fieldMetadata.getFieldType().name().equals(MeasureCatalogueCostants.MEASURE));
	...
	public Set<HierarchyWrapper> getHierarchies() {
		Set<HierarchyWrapper> hierarchies = new HashSet<>();
