...
public class XExecuteBIDocumentJob extends AbstractSpagoBIJob implements Job {
				...
				if (dataSet != null) {
					...
					if (dataStore != null && dataStore.getRecordsCount() > 0) {
						...
						if (returnVal != null) {
							...
							boolean validDS = (value.toString().equals("1") || value.toString().equals("true")) ? true
									: false;
	...
	private void executeInternal(JobExecutionContext jobExecutionContext) throws JobExecutionException {
		...
		try {
			...
			if (StringUtils.isNotEmpty(encodedGlobalDispatchContext)) {
			...
			for (int documentIndex = 0; documentIndex < documentLabels.length; documentIndex++) {
				...
				if (StringUtils.isNotEmpty(encodedDispatchContext)) {
			...
			for (int documentIndex = 0; documentIndex < documentLabels.length; documentIndex++) {
				...
				logger.debug("Runtime parameter configuration for document [" + documentLabel + "] is equal to ["
				...
				logger.debug("Formuula based parameter configuration for document [" + documentLabel + "] is equal to ["
						+ useFormulaParametersString + "]");
		...
		try {
			...
			try {
				...
				throw new SpagoBIServiceException(
						"Impossible to instatiate DAOs to access to document's metadata properties and values", e);
			}
			...
			Map<Integer, ObjMetacontent> metadataPropertyIdToPropertyValueMap = new HashMap<>();

			List<ObjMetacontent> documentMetadataPropertyValues = documentMetadataPropertyValuesDAO
					.loadObjOrSubObjMetacontents(document.getId(), null);
			...
			documentMetadataProperties = new ArrayList<>();
			for (ObjMetadata metadataPropertyDefinition : metadataPropertyDefinitions) {
				...
				documentMetadataProperty.setMetadataPropertyValue(
						metadataPropertyIdToPropertyValueMap.get(metadataPropertyDefinition.getObjMetaId()));
			...
			throw new SpagoBIRuntimeException(
					"An unexpected error occured while loading document [" + document + "] metadata", t);
		} finally {
			logger.debug("OUT");
		}
	}
	...
	private void retrieveParametersValues(BIObject biobj) throws Exception {
		...
		try {
			...
			while (it.hasNext()) {
				...
				if (retriever != null) {
					logger.debug("Document parameter with url name [" + parameter.getParameterUrlName()
							+ "] has a parameter values retriever: " + retriever);
					...
					try {
						...
						logger.error(
								"Error while retrieving values for parameter with url name ["
										+ parameter.getParameterUrlName() + "] of document [" + biobj.getLabel() + "].",
								e);
						throw e;
					}
	...
	private void setLoadAtRuntimeParameters(BIObject biobj, String loadAtRuntimeParametersString) {
		...
		try {
			...
			Map<String, String> loadAtRuntimeParametersMap = new HashMap<>();
			...
			while (it.hasNext()) {
				...
				if (loadAtRuntimeParametersMap.containsKey(parameter.getParameterUrlName())) {
					logger.debug("Document parameter with url name [" + parameter.getParameterUrlName()
							+ "] was configured to be calculated at runtime.");
		...
		try {
			...
			while (it.hasNext()) {
				...
				if (iterativeParametersList.contains(parameter.getParameterUrlName())) {
					logger.debug("Document parameter with url name [" + parameter.getParameterUrlName()
							+ "] was configured to be iterative.");
	...
	private void setUseFormulaParameters(BIObject biobj, String useFormulaParametersString) {
		...
		try {
			...
			Map<String, String> useFormulaParametersMap = new HashMap<>();
			...
			while (it.hasNext()) {
				...
				if (useFormulaParametersMap.containsKey(parameter.getParameterUrlName())) {
					logger.debug("Document parameter with url name [" + parameter.getParameterUrlName()
							+ "] was configured to use a formula.");
	 ...
	 * @throws Exception
	 */
	public Map<String, Object> getMailOptionsFromUniqueMaildispatcher(BIObject document,
			DispatchContext dispatchContext) throws Exception {

		Map<String, Object> mailOptions = new HashMap<>();
		...
		String containedFileName = dispatchContext.getContainedFileName() != null
				&& !dispatchContext.getContainedFileName().equals("") ? dispatchContext.getContainedFileName()
						: document.getName();
		String zipFileName = dispatchContext.getZipMailName() != null && !dispatchContext.getZipMailName().equals("")
		...
		String[] recipients = UniqueMailDocumentDispatchChannel.findRecipients(dispatchContext, document,
				emailDispatchDataStore);
