...
public class DatasetFieldsValidator implements IFieldsValidator {
	...
	public JSONArray validateFields(MultivaluedMap<String, String> parameters) throws EncodingException {
		...
		if (datasetMetadata != null) {
			...
			datasetMetadata = encoder.decodeFromURL(datasetMetadata);
			try {
					...
					for (int j = 0; j < columnsMetadataArray.length(); j++) {
						...
						if (columnJsonObject.getString("pname").equalsIgnoreCase("type")) {
							if (columnName == null) {
								 ...
								 * else if(Pattern.compile("\\s").matcher(columnName).find()){ validationErrors.put(new
								 * JSONObject("{message: 'Validation error: Space character not allowed for column name "+ columnName + "'}")); }else
								 * if(Pattern.compile("[ÀÈÌÒÙàèìòùÁÉÍÓÚÝáéíóúýÂÊÎÔÛâêîôûÃÑÕãñõÄËÏÖÜŸäëïöüŸ¡¿çÇŒœßØøÅåÆæÞþÐðÐð'.,&#@:?!()$\\/]").matcher(columnName).find()){
						...
						if (propertyName == null) {
							validationErrors
									.put(new JSONObject("{message: 'Validation error: Property name cannot be null'}"));
						} /*
							 * else if( Pattern.compile("\\s").matcher(propertyName).find()){ validationErrors.put(new
							 * JSONObject("{message: 'Validation error: Space character not allowed for Property name "+ propertyName + "'}")); }else if(
							 * Pattern.compile("[ÀÈÌÒÙàèìòùÁÉÍÓÚÝáéíóúýÂÊÎÔÛâêîôûÃÑÕãñõÄËÏÖÜŸäëïöüŸ¡¿çÇŒœßØøÅåÆæÞþÐðÐð'.,&#@:?!()$\\/]").matcher(propertyName).find()){
