...
public class DetailDataSourceModule extends AbstractHttpModule {
	...
	/**
	 * Gets the detail of a data source choosed by the user from the data sources list. It reaches the key from the request and asks to the DB all detail data
	 * source information, by calling the method <code>loadDataSourceByID</code>.
	 *
	 * @param key
	 *            The choosed data source id key
	 * @param response
	...
	private void getDataSource(SourceBean request, SourceBean response) throws EMFUserError {
		IDataSource ds = DAOFactory.getDataSourceDAO().loadDataSourceByID(new Integer((String) request.getAttribute("ID")));
	...
	/**
	 * Inserts/Modifies the detail of a data source according to the user request. When a data source is modified, the <code>modifyDataSource</code> method is
	 * called; when a new data source is added, the <code>insertDataSource</code>method is called. These two cases are differentiated by the <code>mod</code>
	 * String input value .
	 *
	 * @param request
	...
	private void modifyDataSource(SourceBean serviceRequest, String mod, SourceBean serviceResponse) throws EMFUserError, SourceBeanException {
		...
		try {
			...
			IDataSource dsNew = recoverDataSourceDetails(serviceRequest);
			EMFErrorHandler errorHandler = getErrorHandler();
			HashMap<String, String> logParam = new HashMap();
	 ...
	 *             If a SourceBean Exception occurs
	 */
	private void deleteDataSource(SourceBean request, String mod, SourceBean response) throws EMFUserError, SourceBeanException {
		HashMap<String, String> logParam = new HashMap();
		try {
	...
	/**
	 * Instantiates a new <code>datasource<code> object when a new data source insertion is required, in order to prepare the page for the insertion.
	 *
	 * @param response
	 *            The response SourceBean
	 * @throws EMFUserError
	...
	private IDataSource recoverDataSourceDetails(SourceBean serviceRequest) throws EMFUserError, SourceBeanException, IOException {
		IDataSource ds = DataSourceFactory.getDataSource();

		String idStr = (String) serviceRequest.getAttribute("ID");
		Integer id = new Integer(idStr);
		String dialectName = (String) serviceRequest.getAttribute("DIALECT_NAME");
