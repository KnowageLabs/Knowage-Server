			...
			while (iterbiobj.hasNext()) {
				...
				while (iterPars.hasNext()) {
					...
					if (strategyObj != null && strategyObj instanceof RuntimeLoadingParameterValuesRetriever) {
						...
						UserProfile userProfile = strategy.getUserProfile();
						String serialiedUser = SchedulerUtilitiesV2.serializeUserProfile(userProfile);
						String role = strategy.getRoleToBeUsed();
						loadAtRuntimeParameters.append(biobjpar.getParameterUrlName() + "(" + serialiedUser + "|" + role + ");");
		...
		while (iterbiobjs.hasNext()) {
			...
			while (iterbiobjpars.hasNext()) {
				...
				if (useFormula) {
					...
					strategy.setUserProfile((UserProfile) profile);
