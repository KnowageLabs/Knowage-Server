...
public class DocumentExecutionViewpoint extends AbstractSpagoBIResource {
	...
	public Response addViewoint(@Valid AddViewpointRequestDTO request) {
		...
		try {
			...
			obj = DriversRuntimeLoaderFactory.getDriversRuntimeLoader().loadBIObjectForExecutionByLabelAndRole(name, role);
	...
	@Path("/getViewpoints")
	@Produces(MediaType.APPLICATION_JSON + "; charset=UTF-8")
	public Response getViewpoints(@Valid @ExtendedAlphanumeric @QueryParam("label") String label, @Valid @ExtendedAlphanumeric @QueryParam("role") String role) {
		HashMap<String, Object> resultAsMap = new HashMap<String, Object>();
		List viewpoints;
	...
	public Response deleteViewpoint(@Valid DeleteViewpointRequestDTO request) {
		IViewpointDAO viewpointDAO = null;
		Viewpoint viewpoint = null;
		List<String> ids;
