...
public class UploadDatasetFileResource extends AbstractSpagoBIResource {
    ...
    public Map<String, String> uploadFileDataset(@Context HttpServletRequest req) {
        ...
        try {
			...
			throw new SpagoBIRestServiceException(s, getLocale(), e);
        } finally {
            logger.debug("OUT");
        }
    ...
    private FileItem unzipUploadedFile(FileItem uploaded) {
        ...
        DiskFileItem uploadedDiskFileItem = (DiskFileItem) uploaded;
        ...
        try (ZipFile zipFile = new ZipFile(uploadedDiskFileItem.getStoreLocation())) {
            ...
            while (entries.hasMoreElements()) {
                ...
                String zipItemName = entry.getName();
                InputStream zippedInputStream = new BufferedInputStream(zipFile.getInputStream(entry));
                ...
                if (totalEntryArchive > thresholdEntries) {
                    throw new SpagoBIServiceException(getActionName(), "The uploaded file contains too many entries");
                }
                ...
                while ((nBytes = zippedInputStream.read(buffer)) > 0) { // Compliant
                    tempFileItemOutStream.write(buffer, 0, nBytes);
                    ...
                    if (compressionRatio > thresholdRatio || totalSizeArchive > thresholdSize) {
                        ...
                        throw new SpagoBIServiceException(getActionName(), "The uploaded file is a zip bomb attack");
                    }
                }
