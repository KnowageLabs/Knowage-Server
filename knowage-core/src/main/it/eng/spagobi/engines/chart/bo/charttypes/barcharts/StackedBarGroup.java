...
public class StackedBarGroup extends BarCharts {// implements ILinkableChart {
		...
		for (Iterator iterator = listAtts.iterator(); iterator.hasNext();) {
			...
			for (Iterator iterator3 = myseries.keySet().iterator(); iterator3.hasNext();) {
				...
				if (!hiddenSeries.contains(nameS)) {
					...
					// logger.debug("dataset.addValue("+Double.valueOf(valueS).doubleValue()+ ", '"+subcat+"'"+",'"+catValue+"');");
	...
	public void configureChart(SourceBean content) {
		...
		/*
		 * SourceBean drillSB = (SourceBean)content.getAttribute("CONF.DRILL"); if(drillSB!=null){ String lab=(String)drillSB.getAttribute("document");
		 * if(lab!=null) drillLabel=lab; else{ logger.error("Drill label not found"); }
		 * 
		 * List parameters =drillSB.getAttributeAsList("PARAM"); if(parameters!=null){ drillParameter=new HashMap();
		 * 
		 * for (Iterator iterator = parameters.iterator(); iterator.hasNext();) { SourceBean att = (SourceBean) iterator.next(); String
		 * name=(String)att.getAttribute("name"); String type=(String)att.getAttribute("type"); String value=(String)att.getAttribute("value");
		 * 
		 * if(type!=null && type.equalsIgnoreCase("RELATIVE")){ // Case relative if(value.equalsIgnoreCase("serie"))serieUrlname=name;
		 * if(value.equalsIgnoreCase("category"))categoryUrlName=name; } else{ // Case absolute drillParameter.put(name, value); } } } }
		 */
	...
	public JFreeChart createChart(DatasetMap datasets) {
		...
		/*
		 * MyCategoryUrlGenerator mycatUrl=new MyCategoryUrlGenerator(rootUrl); mycatUrl.setDocument_composition(document_composition);
		 * mycatUrl.setCategoryUrlLabel(categoryUrlName); mycatUrl.setSerieUrlLabel(serieUrlname);
		 * 
		...
		if (additionalLabels) {
			...
			renderer.setBaseItemLabelsVisible(true);
			// vertical labels
			renderer.setBasePositiveItemLabelPosition(new ItemLabelPosition(ItemLabelAnchor.CENTER, TextAnchor.CENTER, TextAnchor.CENTER, orient));
			renderer.setBaseNegativeItemLabelPosition(new ItemLabelPosition(ItemLabelAnchor.CENTER, TextAnchor.CENTER, TextAnchor.CENTER, orient));
