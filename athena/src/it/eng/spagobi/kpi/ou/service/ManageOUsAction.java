...
public class ManageOUsAction extends AbstractSpagoBIAction {
	...
	public void doService() {
		...
		try {
			...
			if (serviceType != null && serviceType.equalsIgnoreCase(GRANT_DEF)) {
				...
				String parentId = getAttributeAsString("modelInstId");
				...
				try {
					...
					throw new SpagoBIServiceException(SERVICE_NAME, "Exception occurred while retrieving model tree", e);
				}
	 ...
	 * @param grantId
	 *            the id of the grant because we return OrganizationalUnitNodeWithGrant. If null the grant object in the OrganizationalUnitNodeWithGrant will be
	 *            null
	 */
	 ...
	 * @param grantNodesJSON
	 *            the JSON representation of the nodes
	 */
	...
	private List<OrganizationalUnitGrantNode> buildGrantNodesForAll(OrganizationalUnitNode ouNode, List<ModelInstanceNode> miNodes,
			OrganizationalUnitGrant grant) throws EMFUserError {
	 ...
	 * 
	 * @param jsonGrant
	 *            the JSON representation of the OrganizationalUnitGrant object
	...
	// * @throws Exception
	// */
	// private List<OrganizationalUnitGrantNode> deserializeOrganizationalUnitGrantNodes(JSONArray JSONGrantNodes, OrganizationalUnitGrant grant) throws
	// Exception{
	// List<OrganizationalUnitGrantNode> nodes = new ArrayList<OrganizationalUnitGrantNode>();
	...
	/**
	 * Deserialize a list of OrganizationalUnitGrantNode objects. If the nodes have not been expanded by the user, this methods load the subtree rooted in all
	 * the JSONGrantNodes and build one grant node for each node of the subtree
	 * 
	 * @param JSONGrantNodes
	 *            the JSON representation of the list of OrganizationalUnitGrantNode
	 * @param grant
	 *            the grant of that list
	 * @return the deserialized object
	 * @throws Exception
	 */
	private List<OrganizationalUnitGrantNode> deserializeOrganizationalUnitGrantNodesAndUpdateChilds(JSONArray JSONGrantNodes, OrganizationalUnitGrant grant)
			throws Exception {
						...
						if (-1 == modelInstanceId) {
							...
							for (int o = 0; o < children.size(); o++) {
								...
								for (int y = 0; y < JSONGrantNodesFiltered.size(); y++) {
									if (JSONGrantNode.getInt("hierarchyId") == JSONGrantNodesFiltered.get(y).getInt("hierarchyId")
											&& JSONGrantNode.getString("ouPath").equals(JSONGrantNodesFiltered.get(y).getString("ouPath"))
											&& (children.get(o) == JSONGrantNodesFiltered.get(y).getInt("modelinstance") || JSONGrantNodesFiltered.get(y)
													.getInt("modelinstance") == -1)
	...
	/**
	 * Deserialize a OrganizationalUnitGrantNode object. If the node has not been expanded by the user, this methods load the subtree rooted in the
	 * JSONGrantNodes and build one grant node for each node of the subtree
	 * 
	 * @param JSONGrantNodes
	 *            the JSON representation of the OrganizationalUnitGrantNode
	 * @param grant
	 *            the grant of that object
	 * @return the deserialized object
	 * @throws Exception
	 */
	private List<OrganizationalUnitGrantNode> deserializeOrganizationalUnitGrantNodeAndUpdateChilds(JSONObject JSONGrantNode, OrganizationalUnitGrant grant)
			throws Exception {
		OrganizationalUnitGrantNode node = new OrganizationalUnitGrantNode();
		List<OrganizationalUnitGrantNode> nodes = new ArrayList<OrganizationalUnitGrantNode>();
		int hierarchyId = JSONGrantNode.getInt("hierarchyId");
	...
	/**
	 * For each child of the OrganizationalUnitNode ouNode, build a OrganizationalUnitGrantNode with grant grant and model Instance Node modelInstanceNode
	 * 
	 * @param ouNode
	 ...
	 * @return
	 */
	private List<OrganizationalUnitGrantNode> buildGrantForChilds(OrganizationalUnitNode ouNode, ModelInstanceNode modelInstanceNode,
			OrganizationalUnitGrant grant) {
		List<OrganizationalUnitGrantNode> nodes = new ArrayList<OrganizationalUnitGrantNode>();
		OrganizationalUnitGrantNode childNode;
		// logger.debug(ouNode.getNodeId());
	...
	/**
	 * For each child of the ModelInstanceNode modelInstanceNode, build a OrganizationalUnitGrantNode with grant grant and model Instance Node modelInstanceNode
	 * 
	 * @param ouNode
	 * @param modelInstanceNode
	 * @param grant
	 ...
	 * @throws EMFUserError
	 */
	private List<OrganizationalUnitGrantNode> buildGrantForModelInstChildren(OrganizationalUnitNode ouNode, ModelInstanceNode modelInstanceNode,
			OrganizationalUnitGrant grant) throws EMFUserError {
		List<OrganizationalUnitGrantNode> nodes = new ArrayList<OrganizationalUnitGrantNode>();
		OrganizationalUnitGrantNode childNode;
		List<Integer> childOus = modelInstanceNode.getChildrenIds();
		for (int i = 0; i < childOus.size(); i++) {
			ModelInstanceNode miChildNode = DAOFactory.getModelInstanceDAO().loadModelInstanceById(childOus.get(i), null);
			childNode = new OrganizationalUnitGrantNode();
			childNode.setGrant(grant);
