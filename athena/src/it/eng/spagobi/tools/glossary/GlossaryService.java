...
public class GlossaryService {
	...
	public String newWord(@Context HttpServletRequest req) {
		try {
			...
			Integer id = dao.insertWord(word, objLink, objAttr, attr);
	...
	public String listAttribute(@Context HttpServletRequest req) {
		try {
			...
			JSONArray jarr = new JSONArray();
			if (lst != null) {
				for (Iterator<SbiGlAttribute> iterator = lst.iterator(); iterator.hasNext();) {
	...
	private static JSONObject fromAttrLight(SbiGlAttribute SbiGlAttribute) throws JSONException {
		JSONObject jobj = new JSONObject();
