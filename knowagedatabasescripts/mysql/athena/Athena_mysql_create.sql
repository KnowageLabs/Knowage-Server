...
CREATE TABLE `sbi_cross_navigation_par` (
  ...
  `ORGANIZATION` varchar(20) DEFAULT NULL,
  PRIMARY KEY (`ID`),
  KEY `CN_ID_REF_idx` (`CN_ID`),
  CONSTRAINT `CN_ID_REF` FOREIGN KEY (`CN_ID`) REFERENCES `sbi_cross_navigation` (`ID`) ON DELETE NO ACTION ON UPDATE NO ACTION
) ENGINE=InnoDB DEFAULT CHARSET=utf8;

/* OUTPUT PARAMETER*/
CREATE TABLE `spagobi`.`sbi_output_parameter` (
  `ID` INT(11) NOT NULL,
  `BIOBJ_ID` INT(11) NOT NULL,
  `LABEL` VARCHAR(40) NOT NULL,
  ...
  PRIMARY KEY (`ID`),
  INDEX `BIOBJ_ID_idx` (`BIOBJ_ID` ASC),
  INDEX `REF_DOMAINS_idx` (`PAR_TYPE_ID` ASC),
  CONSTRAINT `fk_sbi_output_parameter_1`
    FOREIGN KEY (`BIOBJ_ID`)
    REFERENCES `spagobi`.`sbi_objects` (`BIOBJ_ID`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_sbi_output_parameter_2`
    FOREIGN KEY (`PAR_TYPE_ID`)
    REFERENCES `spagobi`.`sbi_domains` (`VALUE_ID`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION);
