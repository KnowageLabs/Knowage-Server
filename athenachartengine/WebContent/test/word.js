function renderWordCloud(chartConf){
	...
	(function() {
		...
		/*var resultArrayMap={};
		for (var i = 0; i<jsono.rows.length; i++){
			var row = jsono.rows[i];
			for(property in row){

				var prop = row[property];

				if(resultArrayMap[prop]){
					var value = resultArrayMap[prop];
					resultArrayMap[prop] = resultArrayMap[prop]+1;
				}else{
					resultArrayMap[prop] = 1;
				}
			}
		}
		...
		/*for(property in resultArrayMap){
			var item = {};
			item.word=property;
			item.count=resultArrayMap[property];
