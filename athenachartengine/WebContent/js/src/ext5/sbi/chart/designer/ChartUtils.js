	...
	statics : {
		...
		convertJsonAxisObjToAxisData : function (axis) {
			...
			if (axis.MAJORGRID) {
				...
				result['majorgridStyleTypeline'] = majorgridStyleAsMap.typeline && majorgridStyleAsMap.typeline != '' ?
					majorgridStyleAsMap.typeline : '';
				result['majorgridStyleColor'] = majorgridStyleAsMap.color && majorgridStyleAsMap.color != '' ?
					majorgridStyleAsMap.color : '';
			}
			if (axis.MINORGRID) {
				var minorgridStyleAsMap = ChartUtils.jsonizeStyle(axis.MINORGRID.style);
				result['minorgridInterval'] = axis.MINORGRID.interval && axis.MINORGRID.interval != '' ?
					axis.MINORGRID.interval : '';
				result['minorgridStyleTypeline'] = minorgridStyleAsMap.typeline && minorgridStyleAsMap.typeline != '' ?
					minorgridStyleAsMap.typeline : '';
				result['minorgridStyleColor'] = minorgridStyleAsMap.color && minorgridStyleAsMap.color != '' ?
					minorgridStyleAsMap.color : '';
			}
			...
			if (axis.TITLE) {
				...
				result['titleStyleAlign'] = titlegridStyleAsMap.align && titlegridStyleAsMap.align != '' ?
					titlegridStyleAsMap.align : '';
				result['titleStyleColor'] = titlegridStyleAsMap.color && titlegridStyleAsMap.color != '' ?
					titlegridStyleAsMap.color : '';
				result['titleStyleFont'] = titlegridStyleAsMap.font && titlegridStyleAsMap.font != '' ?
					titlegridStyleAsMap.font : '';
				result['titleStyleFontWeigh'] = titlegridStyleAsMap.fontWeight && titlegridStyleAsMap.fontWeight != '' ?
					titlegridStyleAsMap.fontWeight : '';
				result['titleStyleFontSize'] = titlegridStyleAsMap.fontSize && titlegridStyleAsMap.fontSize != '' ?
					titlegridStyleAsMap.fontSize : '';
			}
...
//					console.log("YYY 1");
		...
		createEmptyAxisData : function (isCategory, isLeftSerie) {
			...
			result['position'] = isCategory ? 'bottom' : isLeftSerie ? 'left' : 'right';
		...
		exportAsJson : function (chartModel) {
			...
			if (Sbi.chart.designer.Designer.chartTypeSelector.getChartType() == "PARALLEL") {
				...
				axesList += 'axisColNamePadd:'
					+ ((chartModel.get('axisColNamePadd')) ?
							chartModel.get('axisColNamePadd') : '') + ';';
				axesList += 'brushWidth:'
					+ ((chartModel.get('brushWidth')) ?
							chartModel.get('brushWidth') : '') + ';';
				axesList += 'axisColor:'
					+ ((chartModel.get('axisColor') != undefined && chartModel.get('axisColor') != '') ? 
							'#' + chartModel.get('axisColor') : '') + ';';
				axesList += 'brushColor:'
					+ ((chartModel.get('brushColor') != undefined && chartModel.get('brushColor') != '') ?
			...
			for (storeIndex in serieStores) {
				...
				for (var rowIndex = 0; rowIndex < storeSerieDataLength; rowIndex++) {
					...
					if (chartType.toUpperCase() != "SUNBURST"
						...
						var TOOLTIP = {};
						TOOLTIP['backgroundColor'] = serieAsMap.get('serieTooltipBackgroundColor') != undefined ? serieAsMap.get('serieTooltipBackgroundColor')
							 : '';
						TOOLTIP['templateHtml'] = serieAsMap.get('serieTooltipTemplateHtml') != undefined ? serieAsMap.get('serieTooltipTemplateHtml')
							 : '';
		...
		getCategoriesDataAsOriginalJson : function () {
			...
			if (chartType.toUpperCase() == "SUNBURST"
				...
				for (var i = 0; i < categoriesStore.data.length; i++) {
					...
					category['name'] = mainCategory.get('axisName') != undefined ? 
							mainCategory.get('axisName') : mainCategory.get('categoryColumn');
					category['column'] = mainCategory.get('categoryColumn') != undefined ? 
							mainCategory.get('categoryColumn') : '';
					category['orderColumn'] = mainCategory.get('categoryOrderColumn') != undefined ? 
							mainCategory.get('categoryOrderColumn') : '';
					category['orderType'] = mainCategory.get('categoryOrderType') != undefined ? 
							mainCategory.get('categoryOrderType') : '';
					category['stackedType'] = mainCategory.get('categoryOrderType') != undefined ? 
							mainCategory.get('categoryOrderType') : '';
					category['stacked'] = mainCategory.get('categoryStacked') != undefined ? 
							mainCategory.get('categoryStacked') : '';
					
//					category['categoryDataType'] = mainCategory.get('colType') != undefined ? mainCategory.get('colType') : '';
...
//					console.log(mainCategory.get('categoryDataType'));
		...
		getChartDataAsOriginaJson : function (chartModel) {
			...
			var chartStyle = '';
			chartStyle += 'fontFamily:' + ((chartModel.get('font') != undefined) ? 
					chartModel.get('font') : '') + ';';
...
//				chartStyle += 'maxWords:'
//						+ ((chartModel.get('maxWords')) ? chartModel.get('maxWords') : '') + ';';
//				chartStyle += 'maxAngle:'
//						+ ((chartModel.get('maxAngle')) ? chartModel.get('maxAngle') : '') + ';';
//				chartStyle += 'minAngle:'
//						+ ((chartModel.get('minAngle')) ? chartModel.get('minAngle') : '') + ';';
//				chartStyle += 'maxFontSize:'
//						+ ((chartModel.get('maxFontSize')) ? chartModel.get('maxFontSize') : '') + ';';
//				chartStyle += 'wordPadding:'
//						+ ((chartModel.get('wordPadding')) ? chartModel.get('wordPadding') : '') + ';';
			}
			...
			emptymessageStyle += 'fontSize:' + ((chartModel.get('nodataDimension') != undefined) ? 
			...
			if (chartType.toUpperCase() == "PARALLEL") {
				 ...
				 * TITLE
				 */
				legendTitleStyle += 'fontFamily:' + ((chartModel.get('parallelLegendTitleFontFamily') != undefined) ? 
						chartModel.get('parallelLegendTitleFontFamily') : '') + ';';

				legendTitleStyle += 'fontSize:' + ((chartModel.get('parallelLegendTitleFontSize') != undefined) ? 
						 chartModel.get('parallelLegendTitleFontSize') : '') + ';';

				legendTitleStyle += 'fontWeight:' + ((chartModel.get('parallelLegendTitleFontWeight') != undefined) ? 
						 chartModel.get('parallelLegendTitleFontWeight') : '') + ';';

				/**
				 * ELEMENT
				 */
				legendElementStyle += 'fontFamily:' + ((chartModel.get('parallelLegendElementFontFamily') != undefined) ? 
						 chartModel.get('parallelLegendElementFontFamily') : '') + ';';

				legendElementStyle += 'fontSize:' + ((chartModel.get('parallelLegendElementFontSize') != undefined) ? 
						 chartModel.get('parallelLegendElementFontSize') : '') + ';';

				legendElementStyle += 'fontWeight:' + ((chartModel.get('parallelLegendElementFontWeight') != undefined) ? 
						 chartModel.get('parallelLegendElementFontWeight') : '') + ';';
				...
				var legendStyle = '';
				legendStyle += 'align:' + ((chartModel.get('legendAlign') != undefined) ? 
						chartModel.get('legendAlign') : '') + ';';
				legendStyle += 'fontFamily:' + ((chartModel.get('legendFont') != undefined) ? 
						chartModel.get('legendFont') : '') + ';';
				legendStyle += 'fontSize:' + ((chartModel.get('legendDimension') != undefined) ? 
						chartModel.get('legendDimension') : '') + ';';
				legendStyle += 'fontWeight:' + ((chartModel.get('legendStyle') != undefined) ?
						chartModel.get('legendStyle') : '') + ';';
				legendStyle += 'borderWidth:' + ((chartModel.get('legendBorderWidth') != undefined) ? 
						chartModel.get('legendBorderWidth') : '') + ';';
				 ...
				 */
				legendStyle += 'symbolWidth:' + ((chartModel.get('symbolWidth') != undefined) ? chartModel.get('symbolWidth') : '') + ';';
			...
			if (chartType.toUpperCase() == "SUNBURST") {
				...
				toolbarStyle += 'position:' + ((chartModel.get('toolbarPosition')) ? 
						 chartModel.get('toolbarPosition') : '') + ';';
				toolbarStyle += 'height:' + ((chartModel.get('toolbarHeight')) ? 
						 chartModel.get('toolbarHeight') : '') + ';';
				toolbarStyle += 'width:' + ((chartModel.get('toolbarWidth')) ? 
						 chartModel.get('toolbarWidth') : '') + ';';
				toolbarStyle += 'spacing:' + ((chartModel.get('toolbarSpacing')) ? 
						 chartModel.get('toolbarSpacing') : '') + ';';
				toolbarStyle += 'tail:' + ((chartModel.get('toolbarTail')) ? 
						 chartModel.get('toolbarTail') : '') + ';';
				// toolbarStyle += 'padding:' +
				// ((chartModel.get('serieTooltipFontSize') !=
				// undefined)?
				// chartModel.get('serieTooltipFontSize'): '') +
				// ';';
				toolbarStyle += 'percFontColor:' + ((chartModel.get('toolbarPercFontColor') != undefined 
						&& chartModel.get('toolbarPercFontColor') != '') ? 
						'#' + chartModel.get('toolbarPercFontColor') : '') + ';';
				toolbarStyle += 'fontFamily:' + ((chartModel.get('toolbarFontFamily')) ? 
						chartModel.get('toolbarFontFamily') : '') + ';';
				toolbarStyle += 'fontWeight:' + ((chartModel.get('toolbarFontWeight')) ? 
						chartModel.get('toolbarFontWeight') : '') + ';';
				toolbarStyle += 'fontSize:' + ((chartModel.get('toolbarFontSize')) ? 
						chartModel.get('toolbarFontSize') : '') + ';';
			...
			if (chartType.toUpperCase() == "PARALLEL") {
				...
				limitStyle += 'maxNumberOfLines:' + ((chartModel.get('maxNumberOfLines')) ? 
						chartModel.get('maxNumberOfLines') : '') + ';';
				limitStyle += 'serieFilterColumn:' + ((chartModel.get('serieFilterColumn')) ? 
						chartModel.get('serieFilterColumn') : '') + ';';
				limitStyle += 'orderTopMinBottomMax:'+ ((chartModel.get('orderTopMinBottomMax')) ? 
						chartModel.get('orderTopMinBottomMax') : '') + ';';
				...
				parallelTooltipStype += 'fontFamily:' + ((chartModel.get('parallelTooltipFontFamily')) ? 
						chartModel.get('parallelTooltipFontFamily') : '') + ';';
				parallelTooltipStype += 'fontSize:' + ((chartModel.get('parallelTooltipFontSize')) ? 
						chartModel.get('parallelTooltipFontSize') : '') + ';';
				parallelTooltipStype += 'minWidth:' + ((chartModel.get('parallelTooltipMinWidth')) ? 
						chartModel.get('parallelTooltipMinWidth') : '') + ';';
				parallelTooltipStype += 'maxWidth:' + ((chartModel.get('parallelTooltipMaxWidth')) ? 
						chartModel.get('parallelTooltipMaxWidth') : '') + ';';
				parallelTooltipStype += 'minHeight:' + ((chartModel.get('parallelTooltipMinHeight')) ? 
						chartModel.get('parallelTooltipMinHeight') : '') + ';';
				parallelTooltipStype += 'maxHeight:' + ((chartModel.get('parallelTooltipMaxHeight')) ? 
						chartModel.get('parallelTooltipMaxHeight') : '') + ';';
				parallelTooltipStype += 'padding:' + ((chartModel.get('parallelTooltipPadding')) ? 
						chartModel.get('parallelTooltipPadding') : '') + ';';
				parallelTooltipStype += 'border:' + ((chartModel.get('parallelTooltipBorder')) ? 
						chartModel.get('parallelTooltipBorder') : '') + ';';
				parallelTooltipStype += 'borderRadius:' + ((chartModel.get('parallelTooltipBorderRadius')) ? 
						chartModel.get('parallelTooltipBorderRadius') : '') + ';';
		...
		enablePalette : function () {
			return Sbi.chart.designer.Designer.chartTypeSelector.getChartType() != 'SUNBURST'
			 ...
			 && Sbi.chart.designer.Designer.chartTypeSelector.getChartType() != 'SCATTER'
		...
		disableShowLegendCheck : function () {
			return Sbi.chart.designer.Designer.chartTypeSelector.getChartType() == 'SUNBURST'
			 ...
			 || Sbi.chart.designer.Designer.chartTypeSelector.getChartType() == 'HEATMAP'
			 || Sbi.chart.designer.Designer.chartTypeSelector.getChartType() == 'GAUGE';
		},
		...
		createChartConfigurationModelFromJson : function (
			...
			if (Sbi.chart.designer.Designer.chartTypeSelector.getChartType() == 'SUNBURST') {
				jsonTipStyle = jsonTemplate.CHART.TIP ? Sbi.chart.designer.ChartUtils.jsonizeStyle(jsonTemplate.CHART.TIP.style) : '';
			} else if (Sbi.chart.designer.Designer.chartTypeSelector.getChartType() == 'HEATMAP') {
				jsonTipStyle = jsonTemplate.CHART.TOOLTIP ? Sbi.chart.designer.ChartUtils.jsonizeStyle(jsonTemplate.CHART.TOOLTIP.style) : '';
			}
			/* END */
			...
			var jsonParallelLegendTitle = jsonTemplate.CHART.LEGEND.TITLE ? Sbi.chart.designer.ChartUtils.jsonizeStyle(jsonTemplate.CHART.LEGEND.TITLE.style) : "";

			var jsonParallelLegendElement = jsonTemplate.CHART.LEGEND.ELEMENT ? Sbi.chart.designer.ChartUtils.jsonizeStyle(jsonTemplate.CHART.LEGEND.ELEMENT.style) : "";
		...
		convertTreeFormatToJson : function (data, isWrapper) {
			...
			if (data.leaf) {
				...
				for (i in data.children) {
					...
					if (result[datum.key] != undefined) { // If a node with same name is yet present
