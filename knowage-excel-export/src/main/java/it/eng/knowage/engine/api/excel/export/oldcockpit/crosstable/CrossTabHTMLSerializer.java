...
public class CrossTabHTMLSerializer {
	...
	private SourceBean serializeRowsMembers(CrossTab crossTab) throws SourceBeanException, JSONException {
		...
		for (int i = 0; i < levels; i++) {
			...
			for (int j = 0; j < levelNodes.size(); j++) {
				...
				List<CrosstabDefinition.Row> rowsDef = crossTab.getCrosstabDefinition().getRows();
				if (rowsDef != null && rowsDef.size() > 0) {
					CrosstabDefinition.Row row = rowsDef.get(i);
		...
		for (int i = 0; i < levels; i++) {
			...
			for (int j = 0; j < levelNodes.size(); j++) {
				...
				if (crossTab.getCrosstabDefinition().isMeasuresOnColumns() && i + 1 == levels) {
					...
					List<CrosstabDefinition.Column> columns = crossTab.getCrosstabDefinition().getColumns();
					for (int c = 0; c < columns.size(); c++) {
						CrosstabDefinition.Column col = columns.get(c);
						if (col.getAlias().equals(text)) {
				...
				if (isLevel) {
					...
					if (parentIsLevel) {
						if (!text.equalsIgnoreCase(labelTotal) && !text.equalsIgnoreCase(labelSubTotal)) {
							CrosstabDefinition.Column columnObj = ((CrosstabDefinition.Column) getCategoryConfByLabel(crossTab, crossTab.getColumnsRoot().getLevel(i).get(0).getValue(), "columns"));
	...
	private String getMeasureWidthStyle(CrossTab crossTab, String text) throws JSONException {
		...
		for (int m = 0; m < measures.size(); m++) {
			...
			if (text == null || mis.getAlias().equals(text)) {
				...
				String width = getConfiguratedElementStyle(null, CrossTab.CellType.DATA, measureConfig, crossTab, "width");
				if (!width.equals("")) {
					if (width.indexOf("%") >= 0)
	...
	private SourceBean serializeData(CrossTab crossTab) throws SourceBeanException, JSONException {
		...
		List<CrossTab.MeasureInfo> measuresInfo = crossTab.getMeasures();
		...
		if (columnsSpecification.size() > 0) {
			...
			if (crossTab.isMeasureOnRow()) {
				List<CrossTab.CellType> columnsTypes = crossTab.getCelltypeOfColumns();
		...
		if (crossTab.isMeasureOnRow()) {
			...
			for (CrossTab.MeasureInfo measureInfo : crossTab.getMeasures()) {
		...
		for (int i = 0; i < data.length; i++) {
			...
			for (int j = 0; j < values.length; j++) {
				...
				SourceBean aColumn = new SourceBean(COLUMN_TAG);
				CrossTab.CellType cellType = crossTab.getCellType(i, j);
	...
	private String getConfiguratedElementStyle(Double value, CrossTab.CellType cellType, JSONObject config, CrossTab crossTab, String prop) throws JSONException {
		String toReturn = "";
		String dataStyle = "";
	...
	private String getConfiguratedElementStyle(Double value, CrossTab.CellType cellType, JSONObject config, CrossTab crossTab) throws JSONException {
		String dataStyle = "";
...
//	}
	...
	private Double calculatePercent(double value, int i, int j, String percentOn, CrossTab crossTab) {
		...
		List<CrossTab.MeasureInfo> measures = crossTab.getMeasures();
	...
	private SourceBean serializeRowsHeaders(CrossTab crossTab) throws SourceBeanException, JSONException {
		List<CrosstabDefinition.Row> rows = crossTab.getCrosstabDefinition().getRows();
		SourceBean table = new SourceBean(TABLE_TAG);
		SourceBean aRow = new SourceBean(ROW_TAG);
		...
		for (int i = 0; i < rows.size(); i++) {
			addRow = true;
			CrosstabDefinition.Row aRowDef = rows.get(i);
			SourceBean aColumn = new SourceBean(COLUMN_TAG);
	...
	private Object getCategoryConfByLabel(CrossTab crossTab, String text, String type) throws JSONException {
		if (type.equalsIgnoreCase("rows")) {
			for (int e = 0; e < crossTab.getCrosstabDefinition().getRows().size(); e++) {
				CrosstabDefinition.Row ris = crossTab.getCrosstabDefinition().getRows().get(e);
			...
			for (int e = 0; e < crossTab.getCrosstabDefinition().getColumns().size(); e++) {
				CrosstabDefinition.Column cis = crossTab.getCrosstabDefinition().getColumns().get(e);
