...
public class JsonChartTemplateService extends AbstractChartEngineResource {
	...
	public String getJSONChartTemplate(@FormParam("jsonTemplate") String jsonTemplate,
			@FormParam("exportWebApp") String exportWebApp, @FormParam("datasetLabel") String datasetLabel,
			@FormParam("driverParams") String driverParams, @FormParam("jsonData") String jsonData,
		...
		try {
			...
			/*
			 * https://production.eng.it/jira/browse/KNOWAGE-581 if the dataset is null and we have datasetlabel valorized, probabily we are calling this REST service from
			 * the cockpit. So, we need to get the dataset from his label
			 */
			...
			Map profileAttributes = UserProfileUtils
					.getProfileAttributes((UserProfile) this.getEnv().get(EngineConstants.ENV_USER_PROFILE));

			if (StringUtils.isEmpty(jsonData)) {
				jsonData = ChartEngineDataUtil.loadJsonData(jsonTemplate, dataSet, analyticalDrivers, profileAttributes,
						getLocale());
			}
			logger.debug("jsonData :" + jsonData);
			VelocityContext velocityContext = ChartEngineUtil.loadVelocityContext(jsonTemplate, jsonData,
					Boolean.parseBoolean(exportWebApp), engineInstance.getDocumentLabel(),
					getEngineInstance().getUserProfile());
			...
			logger.error(
					"An unexpected error occured while executing service: JsonChartTemplateService.getJSONChartTemplate",
					t);
	...
	@SuppressWarnings("rawtypes")
	@UserConstraint(functionalities = { CommunityFunctionalityConstants.CREATE_COCKPIT_FUNCTIONALITY })
	public String getJSONChartTemplateForCockpit(@FormParam("jsonTemplate") String jsonTemplate,
			@FormParam("exportWebApp") String exportWebApp, @FormParam("jsonData") String jsonData,
			@Context HttpServletResponse servletResponse) {
		logger.debug("IN");
		...
		try {
			...
			logger.debug("jsonData :" + jsonData);
			VelocityContext velocityContext = ChartEngineUtil.loadVelocityContext(jsonTemplate, jsonData,
					Boolean.parseBoolean(exportWebApp), null, getIOManager().getUserProfile());
			String chartType = ChartEngineUtil.extractChartType(jsonTemplate, velocityContext);
			Template velocityTemplate = ve.getTemplate(ChartEngineUtil.getVelocityModelPath(chartType), UTF_8.name());
			String jsonChartTemplate = ChartEngineUtil.applyTemplate(velocityTemplate, velocityContext);
			...
			logger.error(
					"An unexpected error occured while executing service: JsonChartTemplateService.getJSONChartTemplateForCockpit",
					t);
			throw new SpagoBIServiceException(this.request.getPathInfo(),
					"An unexpected error occured while executing service: JsonChartTemplateService.getJSONChartTemplateForCockpit",
					t);
	...
	private void refreshDriverParams(Map analyticalDrivers, String driverParams) throws JSONException {
		if (driverParams != null && !driverParams.trim().isEmpty() && analyticalDrivers != null
				&& !analyticalDrivers.isEmpty()) {
	...
	public String drilldownHighchart(@FormParam("breadcrumb") String breadcrumb) {
		...
		try {
			...
			return ChartEngineDataUtil.drilldown(jsonTemplate, breadcrumb, dataSet, analyticalDrivers,
					profileAttributes, getLocale(), getEngineInstance().getDocumentLabel(),
					getEngineInstance().getUserProfile(), "", "", "", null);
		} catch (Throwable t) {
			logger.error(
					"An unexpected error occured while executing service: JsonChartTemplateService.drilldownHighchart",
					t);
			throw new SpagoBIServiceException(this.request.getPathInfo(),
					"An unexpected error occured while executing service: JsonChartTemplateService.drilldownHighchart",
					t);
	...
	@SuppressWarnings("rawtypes")
	@UserConstraint(functionalities = { CommunityFunctionalityConstants.CREATE_COCKPIT_FUNCTIONALITY })
	public String drilldownHighchartForCockpit(@FormParam("breadcrumb") String breadcrumb,
			@FormParam("widgetData") String widgetData, @FormParam("selections") String selections,
			@FormParam("parameters") String parameters, @FormParam("aggregations") String aggregations,
		...
		try {
			...
			return ChartEngineDataUtil.drilldown(jsonTemplate, breadcrumb, dataSet, analyticalDrivers,
					profileAttributes, getIOManager().getLocale(), null, userProfile, selections, aggregations,
					parameters, queryParams);

		} catch (JSONException e) {
			logger.error("Error creating json object from query param widgetData");
			throw new SpagoBIServiceException(this.request.getPathInfo(),
					"An unexpected error occured while executing service: Error creating json object from query param widgetData",
					e);
	...
	public String getDatasetMetadata() {
		...
		try {
			...
			logger.error(
					"An unexpected error occured while executing service: JsonChartTemplateService.getDatasetMetadata",
					t);
			throw new SpagoBIServiceException(this.request.getPathInfo(),
					"An unexpected error occured while executing service: JsonChartTemplateService.getDatasetMetadata",
					t);
	...
	public String getDatasetMetadataForCockpit(@PathParam("datasetId") String datasetId) {
		...
		try {
			if (datasetId != null) {
				...
				throw new SpagoBIServiceException(this.request.getPathInfo(),
						"JsonChartTemplateService.fieldsMetadataforCockpit : datasetID is null");
			}
		} catch (Throwable t) {
			logger.error(
					"An unexpected error occured while executing service: JsonChartTemplateService.fieldsMetadataforCockpit",
					t);
			throw new SpagoBIServiceException(this.request.getPathInfo(),
					"An unexpected error occured while executing service: JsonChartTemplateService.fieldsMetadataforCockpit",
					t);
	...
	public String isDatasetRealTime() {
		...
		try {
			...
			logger.error(
					"An unexpected error occured while executing service: JsonChartTemplateService.isDatasetRealTime",
					t);
			throw new SpagoBIServiceException(this.request.getPathInfo(),
					"An unexpected error occured while executing service: JsonChartTemplateService.isDatasetRealTime",
					t);
	...
	public String isDatasetForCockpitRealTime(@PathParam("datasetId") String datasetId) {
		...
		try {
			if (datasetId != null) {
				...
				throw new SpagoBIServiceException(this.request.getPathInfo(),
						"JsonChartTemplateService.isDatasetForCockpitRealTime : datasetID is null");
			}
		} catch (Throwable t) {
			logger.error(
					"An unexpected error occured while executing service: JsonChartTemplateService.isDatasetForCockpitRealTime",
					t);
			throw new SpagoBIServiceException(this.request.getPathInfo(),
					"An unexpected error occured while executing service: JsonChartTemplateService.isDatasetForCockpitRealTime",
					t);
	...
	@SuppressWarnings("rawtypes")
	@UserConstraint(functionalities = { CommunityFunctionalityConstants.CREATE_COCKPIT_FUNCTIONALITY })
	public String getDataAndConf(@PathParam("label") String label, String body,
			@Context HttpServletResponse servletResponse, @DefaultValue("-1") @QueryParam("offset") int offset,
			@DefaultValue("-1") @QueryParam("limit") int limit, @DefaultValue("-1") @QueryParam("size") int fetchSize,
			@QueryParam("nearRealtime") boolean isNearRealtime) {
		logger.debug("IN");
		...
		try {
			...
			logger.error("An unexpected error occured while executing service: JsonChartTemplateService.getDataAndConf",
					t);
			throw new SpagoBIServiceException(this.request.getPathInfo(),
