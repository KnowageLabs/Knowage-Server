...
(function () {
  ...
  cockpitApp.config(function ($mdThemingProvider, $mdGestureProvider, $compileProvider, $mdInkRippleProvider, $mdAriaProvider, $mdDateLocaleProvider) {
    ...
    moment.locale(_CURRENTLANGUAGE);
  ...
  function cockpitMasterControllerFunction(
    $scope,
    cockpitModule_widgetServices,
    cockpitModule_template,
    cockpitModule_backwardCompatibility,
    ...
    $scope.variablesInit = function () {
      ...
      return Promise.all(
        cockpitModule_template.configuration.variables.map(function (variable) {
    ...
    cockpitModule_datasetServices.loadDatasetsFromTemplate().then(
      function () {
        ...
        if (dsNotInCache.length > 0) {
          cockpitModule_datasetServices.addDatasetInCache(dsNotInCache).then(function () {
