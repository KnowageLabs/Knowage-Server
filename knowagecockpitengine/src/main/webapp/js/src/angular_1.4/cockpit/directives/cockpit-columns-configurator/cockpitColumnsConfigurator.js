;(function () {
    angular.module('cockpitModule').directive('cockpitColumnsConfigurator', function (cockpitModule_widgetServices, $mdDialog, $mdSidenav) {
        return {
    ...
    function cockpitColumnsConfiguratorControllerFunction(
        $scope,
        $mdDialog,
        ...
        $scope.columnsGrid = {
            ...
            columnDefs: [
                ...
                { headerName: $scope.translate.load('sbi.cockpit.widgets.table.column.type'), field: 'fieldType', editable: true, cellRenderer: editableCell, cellClass: 'editableCell', cellEditor: 'agSelectCellEditor', cellEditorParams: { values: ['ATTRIBUTE', 'MEASURE'] } },
                {
                    ...
                    cellEditorParams: { values: $scope.availableAggregations }
                },
        ...
        function orderRenderer(params) {
            ...
            return '<div layout="row">' + upButton + params.data.order + downButton + '</div>'
        }
        ...
        function buttonRenderer(params) {
            ...
            if (params.data.isCalculated) {
                calculator =
                    '<md-button class="md-icon-button" ng-click="addNewCalculatedField(\'' +
                    params.rowIndex +
                    '\')">' +
            ...
            if (params.data.isFunction) {
                calculator =
                    '<md-button class="md-icon-button" ng-click="addNewCatalogFunction(\'' +
                    params.rowIndex +
                    '\')">' +
            ...
            return (
                calculator +
                '<md-button class="md-icon-button noMargin" ng-click="draw(\'' +
                params.data.name +
                "')\" ng-style=\"{'background-color':model.content.columnSelectedOfDataset[" +
                params.rowIndex +
                "].style['background-color']}\">" +
                '   <md-tooltip md-delay="500">{{::translate.load("sbi.cockpit.widgets.table.columnstyle.icon")}}</md-tooltip>' +
                "	<md-icon ng-style=\"{'color':model.content.columnSelectedOfDataset[" +
                params.rowIndex +
                '].style.color}" md-font-icon="fa fa-paint-brush" aria-label="Paint brush"></md-icon>' +
                '</md-button>' +
                '<md-button class="md-icon-button" ng-click="deleteColumn(\'' +
                params.data.name +
                '\',$event)"><md-icon md-font-icon="fa fa-trash"></md-icon><md-tooltip md-delay="500">{{::translate.load("sbi.cockpit.widgets.table.column.delete")}}</md-tooltip></md-button>'
            )
        }
        ...
        $scope.moveUp = function (evt, index) {
            ...
            for (var k in $scope.model.content.columnSelectedOfDataset) {
                if ($scope.model.content.columnSelectedOfDataset[k].order == index) {
                    $scope.model.content.columnSelectedOfDataset[k].order--
                    continue
                }
                if ($scope.model.content.columnSelectedOfDataset[k].order == index - 1) {
                    $scope.model.content.columnSelectedOfDataset[k].order++
                    continue
                }
            }
            ...
            ($scope.deleteColumn = function (rowName, event) {
                ...
                if (!item.isFunction) {
                    ...
                    for (var i = 0; i < $scope.newModel.content.columnSelectedOfDataset.length; i++) {
                        ...
                        if (col.isFunction && col.boundFunction.id == id) colsToRemove.push(col)
                    }
        ...
        $scope.functionsCockpitColumn = {
            ...
            typeList: [
                { code: 'java.lang.String', name: 'String' },
                { code: 'java.lang.Integer', name: 'Number' },
                { code: 'java.math.BigDecimal', name: 'Number' }
            ],
...
function controllerCockpitColumnsConfigurator($scope, sbiModule_translate, $mdDialog, model, getMetadata, cockpitModule_datasetServices, cockpitModule_generalOptions) {
    ...
    $scope.columnsGridOptions = {
        ...
        columnDefs: [
            { headerName: 'Column', field: 'alias', headerCheckboxSelection: true, checkboxSelection: true },
...
function cockpitStyleColumnFunction(
    ...
    $scope.thresholdsList = [
        { priority: 0, icon: "<md-icon style='color:red'  md-font-icon='fa fa-exclamation-circle' ng-init='scopeFunctions.condition[0].iconColor=\"red\";	scopeFunctions.condition[0].icon=\"fa fa-exclamation-circle\"'></md-icon>", condition: conditionString0, value: valueString0 },
        { priority: 1, icon: "<md-icon style='color:red'	md-font-icon='fa fa-times-circle' ng-init='scopeFunctions.condition[1].iconColor=\"red\"; scopeFunctions.condition[1].icon=\"fa fa-times-circle\"'></md-icon>", condition: conditionString1, value: valueString1 },
        { priority: 2, icon: "<md-icon style='color:yellow'  md-font-icon='fa fa-exclamation-triangle' ng-init='scopeFunctions.condition[2].iconColor=\"yellow\"; scopeFunctions.condition[2].icon=\"fa fa-exclamation-triangle\"'></md-icon>", condition: conditionString2, value: valueString2 },
        { priority: 3, icon: "<md-icon style='color:green'  md-font-icon='fa fa-check-circle' ng-init='scopeFunctions.condition[3].iconColor=\"green\";	scopeFunctions.condition[3].icon=\"fa fa-check-circle\"'></md-icon>", condition: conditionString3, value: valueString3 }
    ]
    $scope.tableColumns = [
        { label: 'Icon', name: 'icon', hideTooltip: true },
        { label: 'Condition', name: 'condition', hideTooltip: true },
    ...
    $scope.cellColorThresholdsList = [
        { priority: 0, color: '<md-input-container class="md-block">  <color-picker  options="{format:\'rgb\'}" ng-model="scopeFunctions.colorCondition[0].value "></color-picker>  </md-input-container>', condition: condString0, value: valString0 },
        { priority: 1, color: '<md-input-container class="md-block"> <color-picker  options="{format:\'rgb\'}" ng-model="scopeFunctions.colorCondition[1].value "></color-picker></md-input-container>', condition: condString1, value: valString1 },
        { priority: 2, color: '<md-input-container class="md-block"> <color-picker  options="{format:\'rgb\'}" ng-model="scopeFunctions.colorCondition[2].value "></color-picker></md-input-container>', condition: condString2, value: valString2 }
    ]
    $scope.cellColorTableColumns = [
        { label: 'Color', name: 'color', hideTooltip: true },
        { label: 'Condition', name: 'condition', hideTooltip: true },
        { label: 'Value', name: 'value', hideTooltip: true }
    ]
    ...
    $scope.saveColumnStyleConfiguration = function () {
        if ($scope.selectedColumn.visType == 'Chart' || $scope.selectedColumn.visType == 'Text & Chart') {
            if ($scope.selectedColumn.barchart && $scope.selectedColumn.barchart.enabled && (typeof $scope.selectedColumn.barchart.minValue == 'undefined' || typeof $scope.selectedColumn.barchart.maxValue == 'undefined')) {
                var toast = $mdToast.simple().content($scope.translate.load('kn.table.missingrequiredfields')).action('OK').highlightAction(false).hideDelay(5000).position('top')
                $mdToast.show(toast)
                return
            }
        }
