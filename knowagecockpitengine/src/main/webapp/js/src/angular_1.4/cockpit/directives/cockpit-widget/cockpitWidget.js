            ...
            return {
                ...
                link: function (scope, ele, attrs) {
                    ele.on('load', function () {
                        if (scope.callbackFunction) scope.callbackFunction()
                    })
                }
            }
        })
        .directive('actionButtonPositionHandler', function ($compile, $timeout) {
            return {
                ...
                link: function (scope, ele, attrs) {
                    ...
                    scope.$watch(
                        function () {
                            return scope.gridsterItem.isMoving()
                        },
                            ...
                            post: function postLink(scope, element, attrs, ctrl, transclud) {
                                ...
                                element.ready(function () {
                                    ...
                                    if (scope.ngModel.type.toLowerCase() == 'chart' && scope.ngModel.drillable == undefined) {
                                        if (scope.ngModel.cliccable) {
                                            ...
                                            if (scope.highcharts) scope.ngModel.drillable = true
                                        }
                                    }
                                ...
                                scope.initializeWidgetProperty = function (directive) {
                                    scope.initWidget = function () {
                                        ...
                                        if (initOnFinish) {
                                            scope.doEditWidget(initOnFinish).then(
                                                function () {
    ...
    function cockpitWidgetControllerFunction(
        ...
        $rootScope.$on('WIDGET_EVENT', function (conf, eventType, config) {
            switch (eventType) {
                ...
                case 'PARAMETER_CHANGE':
                    ...
                    break
                default:
                    console.error('event ' + eventType + ' not found')
            }
        })
                ...
                if (previewSettings.parameters[p].bindType == 'selection') {
                    if (cockpitModule_template.configuration.filters && cockpitModule_template.configuration.filters[previewSettings.parameters[p].dataset]) {
                        ...
                        for (var k in cockpitModule_template.configuration.aggregations) {
                            if (cockpitModule_template.configuration.aggregations[k].selection && cockpitModule_template.configuration.aggregations[k].selection[previewSettings.parameters[p].dataset + '.' + previewSettings.parameters[p].column]) {
        ...
        $scope.doSelection = function (columnName, columnValue, modalColumn, modalValue, row, skipRefresh, dsId, disableAssociativeLogic, directInteraction) {
            ...
            if (!directInteraction || directInteraction == 'cross') {
                if (previewSettings && previewSettings.enable) {
                    if (!previewSettings.previewType || previewSettings.previewType == 'allRow' || (previewSettings.previewType == 'singleColumn' && previewSettings.column == originalColumnName) || (previewSettings.previewType == 'icon' && (!columnName || columnName == ''))) {
                        ...
                        if (previewDataset.parameters && previewDataset.parameters.length > 0) config.parameters = $scope.checkPreviewParameters(previewSettings, previewDataset, columnName, modalColumn, row, columnValue)
                    ...
                    if (!Array.isArray(columnName)) {
                        ...
                        if ($scope.ngModel.content.crosstabDefinition) {
                            ...
                            if (originalColumnName == undefined || originalColumnName == '') {
                                for (var i = 0; i < $scope.ngModel.content.crosstabDefinition.columns.length; i++) {
                                    if (
                                        ($scope.ngModel.content.crosstabDefinition.columns[i].alias && $scope.ngModel.content.crosstabDefinition.columns[i].alias.toUpperCase() === columnName.toUpperCase()) ||
                            ...
                            if (originalColumnName == undefined || originalColumnName == '') {
                                for (var i = 0; i < $scope.ngModel.content.crosstabDefinition.rows.length; i++) {
                                    if (
                                        ($scope.ngModel.content.crosstabDefinition.rows[i].alias && $scope.ngModel.content.crosstabDefinition.rows[i].alias.toUpperCase() === columnName.toUpperCase()) ||
                        ...
                        if ($scope.ngModel.content.crosstabDefinition) {
                            ...
                            for (var k = 0; k < columnName.length; k++) {
                                ...
                                for (var i = 0; i < $scope.ngModel.content.crosstabDefinition.columns.length; i++) {
                                    if (
                                        ($scope.ngModel.content.crosstabDefinition.columns[i].alias && $scope.ngModel.content.crosstabDefinition.columns[i].alias.toUpperCase() === singleColumnName.toUpperCase()) ||
                                ...
                                if (!foundInColumns) {
                                    for (var i = 0; i < $scope.ngModel.content.crosstabDefinition.rows.length; i++) {
                                        if (
                                            ($scope.ngModel.content.crosstabDefinition.rows[i].alias && $scope.ngModel.content.crosstabDefinition.rows[i].alias.toUpperCase() === singleColumnName.toUpperCase()) ||
        ...
        $scope.captureScreenShot = function (ev, model) {
            ...
            if (model.type == 'chart' && isIE && $scope.enterpriseEdition) {
                ...
                canvg(document.getElementById('canvas_' + model.id), xml, {
