...
(function () {
  ...
  function cockpitStaticPivotTableWidgetControllerFunction($scope, cockpitModule_widgetConfigurator, $q, $mdPanel, sbiModule_restServices, $compile, cockpitModule_generalOptions, $mdDialog, sbiModule_device, sbiModule_i18n, $timeout, cockpitModule_properties, cockpitModule_defaultTheme) {
    ...
    $scope.refresh = function (element, width, height, datasetRecords, nature) {
      ...
      if (
        dataToSend.crosstabDefinition == undefined ||
        dataToSend.crosstabDefinition.measures == undefined ||
        dataToSend.crosstabDefinition.measures.length == 0 ||
        ((dataToSend.crosstabDefinition.rows == undefined || dataToSend.crosstabDefinition.rows.length == 0) && (dataToSend.crosstabDefinition.columns == undefined || dataToSend.crosstabDefinition.columns.length == 0))
      ) {
    ...
    $scope.getCorrectRows = function (allElements, parent) {
      ...
      if (Object.keys(parent).length > 0) {
        //filter only rows related to current parent, don't hide rows of other parents
        for (var i = 0; i < allElements.length; i++) {
          var valid = false;
          for (var key in parent) {
            var val = parent[key];
    ...
    $scope.collapse = function (e, column, value, parent) {
      ...
      rowsToHide.forEach(function (row, index) {
        ...
        if (index > 0) {
          ...
          // first row
          if (row.children[0] == e.currentTarget.parentElement) {
            // hide first cell to show hidden one
            row.style.display = "none";
          } else {
            var finished = false;
            e.currentTarget.parentElement.setAttribute("firstrow", true);
            for (var c = 0; c < row.children.length && !finished; c++) {
              // loop on all row elements
              if (row.children[c] == e.currentTarget.parentElement) {
                ...
                // parent elements in the same row
      ...
      if (parent) {
        ...
        for (var p in orderedParents) {
          ...
          for (var k in correctParentRows[0].querySelectorAll("td")) {
            // loop on all parent cells
            var currentParentEl = correctParentRows[0].querySelectorAll("td")[k];
            if (currentParentEl.id == parent[p]) {
              // apply rowspan normalization only to current cell
              var rowspan = currentParentEl.getAttribute("rowspan");

              if (currentParentEl.getAttribute("normalizedRowspan")) {
              ...
              } else if (e.currentTarget.parentElement.getAttribute("firstrow")) {
                // first record has empty row
                currentParentEl.setAttribute("rowspan", parseInt(rowspan) - (rowsToHide.length - hiddenRows) + 2);
    ...
    $scope.expand = function (e, column, value, parent) {
      ...
      if (parent && Object.keys(parent).length > 0) {
        ...
        for (var p in orderedParents) {
          ...
          for (var k in correctParentRows[0].querySelectorAll("td")) {
            // loop on all cells
            if (correctParentRows[0].querySelectorAll("td")[k].id == parent[p]) {
              // if current cell
    ...
    $scope.getSortOptions = function (columns, rows) {
      ...
      if (columnsSortKeys.length > 0) toReturn.columnsSortKeys = columnsSortKeys;
      ...
      if (rowsSortKeys.length > 0) toReturn.rowsSortKeys = rowsSortKeys;
    ...
    $scope.manageSortCompatibility = function (oldSortOptions) {
      ...
      if (oldSortOptions.measuresSortKeys && !Array.isArray(oldSortOptions.measuresSortKeys)) {
        ...
        if (measuresSortKeys.length > 0) toReturn.measuresSortKeys = measuresSortKeys;
      }

      if (JSON.stringify(toReturn) === JSON.stringify({})) toReturn = oldSortOptions; //syntax is already correct
    ...
    $scope.selectMeasure = function (rowHeaders, rowsValues, columnsHeaders, columnValues, measureRef) {
      ...
      if (lstHeaders.length > 0) $scope.doSelection(lstHeaders, lstValues, measureId); //call selection method passing all headers and values (unique time)
    };
    ...
    $scope.addPivotTableStyle = function () {
      if ($scope.ngModel.content.style != undefined) {
        ...
        if ($scope.ngModel.content.style.showGrid || ($scope.ngModel.content.style.showAlternateRows && $scope.ngModel.content.style.measuresRow != undefined && Object.keys($scope.ngModel.content.style.measuresRow).length > 0)) {
          ...
          angular.forEach(rowList, function (row, index) {
            ...
            if (dataColumnList.length > 0) {
              ...
              if ($scope.ngModel.content.style.showAlternateRows && angular.element(dataColumnList).css("background-color") == "") {
        ...
        if ($scope.ngModel.content.style.measures != undefined && Object.keys($scope.ngModel.content.style.measures).length > 0) {
          ...
          for (var prop in $scope.ngModel.content.style.measures) {
            ...
            } else if ($scope.ngModel.content.style.measures[prop] != "") dataItem.css(prop, $scope.ngModel.content.style.measures[prop]);
          }
        }
        ...
        if ($scope.ngModel.content.style.measuresHeaders != undefined && Object.keys($scope.ngModel.content.style.measuresHeaders).length > 0) {
          ...
          for (var prop in $scope.ngModel.content.style.measuresHeaders) {
            if (angular.equals("background-color", prop) && $scope.ngModel.content.style.measuresHeaders[prop] != "") {
              if ($scope.ngModel.content.crosstabDefinition.config.measureson == "columns") measureHeaderItem.parent().parent().css(prop, $scope.ngModel.content.style.measuresHeaders[prop]);
              else measureHeaderItem.css(prop, $scope.ngModel.content.style.measuresHeaders[prop]);
            } else if ($scope.ngModel.content.style.measuresHeaders[prop] != "") measureHeaderItem.css(prop, $scope.ngModel.content.style.measuresHeaders[prop]);
          }
        }
        ...
        if ($scope.ngModel.content.style.totals != undefined && Object.keys($scope.ngModel.content.style.totals).length > 0) {
          ...
          for (var prop in $scope.ngModel.content.style.totals) {
            if ($scope.ngModel.content.style.totals[prop] != "") totalsItem.css(prop, $scope.ngModel.content.style.totals[prop]);
          }
        }
        //subTotals
        if ($scope.ngModel.content.style.subTotals != undefined && Object.keys($scope.ngModel.content.style.subTotals).length > 0) {
          ...
          for (var prop in $scope.ngModel.content.style.subTotals) {
            if ($scope.ngModel.content.style.subTotals[prop] != "") subtotalsItem.css(prop, $scope.ngModel.content.style.subTotals[prop]);
          }
        }
      } else {
      ...
      var config = {
        ...
        controller: function ($scope, finishEdit, sbiModule_translate, model, fnOrder, mdPanelRef, cockpitModule_datasetServices, cockpitModule_template, cockpitModule_generalOptions, $mdDialog, $mdToast, sbiModule_device) {
          ...
          $scope.getDatasetAdditionalInfo = function (dsId) {
            ...
            sbiModule_restServices.promiseGet("2.0/datasets", "availableFunctions/" + dsId, "useCache=" + $scope.localDataset.useCache).then(
              function (response) {
          ...
          $scope.deleteCatalogFunction = function (localModel) {
            ...
            for (var i = 0; i < $scope.localModel.content.crosstabDefinition.columns.length; i++) {
              ...
              if (col.isFunction) colsToRemove.push(col);
            }
            ...
            for (var i = 0; i < $scope.localModel.content.crosstabDefinition.rows.length; i++) {
              ...
              if (row.isFunction) rowsToRemove.push(row);
            }
            ...
            for (var i = 0; i < $scope.localModel.content.crosstabDefinition.measures.length; i++) {
              ...
              if (meas.isFunction) measToRemove.push(meas);
            }
          ...
          $scope.saveConfiguration = function () {
            ...
            if ($scope.localModel.content.crosstabDefinition.measures.length == 0 || ($scope.localModel.content.crosstabDefinition.rows.length == 0 && $scope.localModel.content.crosstabDefinition.columns.length == 0)) {
          ...
          isAliasUnique = function (alias) {
            if (alias == $scope.localModel.content.crosstabDefinition.config.columnsubtotalLabel) return false;
            if (alias == $scope.localModel.content.crosstabDefinition.config.columntotalLabel) return false;
            if (alias == $scope.localModel.content.crosstabDefinition.config.rowsubtotalLabel) return false;
            if (alias == $scope.localModel.content.crosstabDefinition.config.rowtotalLabel) return false;
            return true;
          };

          $scope.showAction = function (text) {
            var toast = $mdToast.simple().content(text).action("OK").highlightAction(false).hideDelay(3000).position("top");
    ...
    function cockpitStyleColumnFunction($scope, sbiModule_translate, $mdDialog, model, selectedColumn, cockpitModule_datasetServices, cockpitModule_generalOptions, cockpitModule_properties, $mdToast, sbiModule_messaging, knModule_fontIconsService, cockpitModule_generalServices) {
      ...
      $scope.colorPickerProperty = { placeholder: sbiModule_translate.load("sbi.cockpit.color.select"), format: "rgb" };
      $scope.visTypes = [
        { label: $scope.translate.load("kn.generic.visualization.text"), value: "Text" },
      ...
      if ($scope.selectedColumn.containerType && $scope.selectedColumn.containerType == "MEASURE-PT") {
        if (model.content.crosstabDefinition.measures.length == 1) $scope.selectedColumn.disableShowHeader = false;
        else {
    ...
    $scope.getItemDirection = function (axisConf, idx) {
      for (c in axisConf) {
        if (axisConf[c].column == idx) return axisConf[c].direction;
      }
      return null;
    };
    ...
    $scope.setItemDirection = function (axisConf, idx, direction) {
      for (c in axisConf) {
        if (axisConf[c].column == idx) axisConf[c].direction = direction;
      }
    };
  }
