    ...
    .factory("buildParametersForExecution", function (sbiModule_user, sbiModule_config) {
      var formAction = function (service) {
        return {
          url:
            "/" +
            sbiModule_config.chartEngineContextName +
            "/api/1.0/chart/pages/" +
            service + // changed by Dragan was
            // /api/1.0/pages/
            "&SBI_LANGUAGE=" +
            sbiModule_config.curr_language +
            "&SBI_COUNTRY=" +
            sbiModule_config.curr_country +
            "SBI_SCRIPT=" +
            sbiModule_config.curr_script +
            "&user_id=" +
            sbiModule_user.userId,
          testUrl: "/" + sbiModule_config.chartEngineContextName + "/api/1.0/chart/pages/executeTest",
        }; // changed by Dragan was
  ...
  function cockpitChartWidgetControllerFunction(
    ...
    $scope.refresh = function (element, width, height, data, nature, undefined, changedChartType, dataAndChartConf) {
      ...
      if ($scope.ngModel.dataset) {
        if (dataAndChartConf && $scope.ngModel.content.chartTemplate.CHART.categoriesCheck && $scope.ngModel.content.chartTemplate.CHART.categoriesCheck.min) {
      ...
      var config = {
        ...
        controller: function ($scope, sbiModule_translate, model, mdPanelRef, doRefresh, sbiModule_user, cockpitModule_generalOptions, cockpitModule_datasetServices, sbiModule_util) {
          ...
          $scope.getDatasetAdditionalInfo = function (dsId) {
            ...
            sbiModule_restServices.promiseGet("2.0/datasets", "availableFunctions/" + dsId, "useCache=" + $scope.tempDataset.useCache).then(
              function (response) {
          ...
          function buttonRenderer(params) {
            if (params.data.isCalculated) {
              return (
                '<calculated-field ng-model="localModel"  callback-update-grid="updateGrid()" callback-update-alias="updateAliasOnSerie(newAlias, oldAlias)" selected-item="' +
                params.rowIndex +
                '" additional-info="datasetAdditionalInfos"></calculated-field>' +
            ...
            if (params.data.isFunction) {
              return (
                '<catalog-function ng-model="localModel"  callback-update-grid="updateGrid()" callback-update-alias="updateAliasOnSerie(newAlias, oldAlias)" selected-item="' +
                params.rowIndex +
                '" additional-info="datasetAdditionalInfos"></catalog-function>' +
                '<md-button class="md-icon-button" ng-click="deleteColumn(\'' +
                params.data.alias +
                '\',$event)"><md-icon md-font-icon="fa fa-trash"></md-icon><md-tooltip md-delay="500">{{::translate.load("sbi.cockpit.widgets.table.column.delete")}}</md-tooltip></md-button>'
              );
