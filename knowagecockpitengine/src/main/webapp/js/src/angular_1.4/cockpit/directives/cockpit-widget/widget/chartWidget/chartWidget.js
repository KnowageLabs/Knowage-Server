...
function cockpitChartWidgetControllerFunction(
	...
	$scope.refresh=function(element,width,height,data,nature, undefined, changedChartType,dataAndChartConf){
		...
		if ($scope.ngModel.dataset){
			if($scope.ngModel.content.chartTemplate.categoriesCheck && $scope.ngModel.content.chartTemplate.categoriesCheck.min){
				...
				if(stat && stat.cardinality < $scope.ngModel.content.chartTemplate.categoriesCheck.min){
					$scope.hideChart = dataAndChartConf.chartConf.categoriesCheck.text;
					return
				}
			}
