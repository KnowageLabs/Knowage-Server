...
(function () {
  ...
  angular.module("cockpitModule").directive("cockpitToolbar", function (sbiModule_config) {
    return {
  ...
  function cockpitToolbarControllerFunction(
    $scope,
    $timeout,
    $q,
    $location,
    ...
    if ($location.search()["scheduledexport"]) $scope.isScheduledExcelExport = true;
    else $scope.isScheduledExcelExport = false;
    ...
    $scope.saveCockpit = function (event) {
      ...
      if (haveSel) {
        var confirm = $mdDialog.confirm().title(sbiModule_translate.load("sbi.cockpit.widgets.save.keepselections")).textContent("").ariaLabel("save cockpit").ok(sbiModule_translate.load("sbi.qbe.messagewin.yes")).cancel(sbiModule_translate.load("sbi.qbe.messagewin.no"));
      ...
      $mdDialog.show({
        ...
        controller: function ($scope, sbiModule_translate, cockpitModule_template, cockpitModule_properties, cockpitModule_widgetServices) {
          ...
          $scope.loadWidgetTypes = function () {
            sbiModule_restServices.get("1.0/engine", "widget").then(
              function (response) {
              ...
              function (error) {
                $mdToast.show($mdToast.simple().content(error.data.ERROR).position("top").action("OK").highlightAction(false).hideDelay(5000));
                $scope.widgetSpinner = false;
              },
            );
          };
    ...
    window.addEventListener("message", (event) => {
      if (event.data.type && event.data.type === "export") {
        if (event.data.format && event.data.format === "pdf") {
          $scope.exportPdf().then(
            function () {},
            function (error) {
              $mdDialog.show($mdDialog.alert().parent(angular.element(document.body)).clickOutsideToClose(true).title("Error during export").textContent(error).ok("Close"));
            },
          );
        }
        if (event.data.format && (event.data.format === "xls" || event.data.format === "xlsx")) {
          $scope.exportExcel("xlsxExport").then(
            function () {},
            function (error) {
              $mdDialog.show($mdDialog.alert().parent(angular.element(document.body)).clickOutsideToClose(true).title("Error during export").textContent(error).ok("Close"));
            },
    ...
    handler.handleMessage = function (message) {
      ...
      if (message == "xlsExport" || message == "xlsxExport") {
        $scope.exportExcel(message).then(
          function () {},
          function (error) {
            $mdDialog.show($mdDialog.alert().parent(angular.element(document.body)).clickOutsideToClose(true).title("Error during export").textContent(error).ok("Close"));
          },
    ...
    $scope.exportExcel = function (message) {
      ...
      return $q(function (resolve, reject) {
        ...
        sbiModule_restServices.promisePost("1.0/cockpit/export", "excel", requestUrl, config).then(
          function (response) {
            $mdDialog.hide();
    ...
    var replaceStringVariables = function (obj) {
      ...
      objString = objString.replace(/\$V\{([a-zA-Z0-9\-\_]{1,255})(?:.([a-zA-Z0-9\-\_]{1,255}))?\}/g, function (match, p1, p2) {
      ...
      return $q(function (resolve, reject) {
        ...
        function getScreenshot(sheet) {
          ...
          function replaceIframe(widget) {
            if (widget.type == "python" && (widget.pythonOutputType == "html" || widget.pythonOutputType == "bokeh")) {
              ...
              var element = document
                .querySelector("#w" + widget.id + " iframe")
          ...
          if ($scope.sheetsWidgets.length == $scope.cockpitModule_widgetServices.getAllWidgets().length) {
            if (isIE11) {
              //if is IE11 and using higcharts
        ...
        for (var s in cockpitModule_template.sheets) {
          if (cockpitModule_template.sheets[s].index == 0) {
            ...
            if (!jsPdfConstructor) {
              console.error("jsPDF is not available in the current page context.");
              return;
            }
  ...
  function cockpitSelectionControllerFunction($scope, cockpitModule_template, cockpitModule_datasetServices, $mdDialog, sbiModule_translate, $q, sbiModule_messaging, cockpitModule_documentServices, cockpitModule_widgetSelection, cockpitModule_properties) {
    ...
    $scope.selectionsGrid = {
      ...
      columnDefs: [
        { headerName: $scope.translate.load("sbi.cockpit.dataset"), field: "ds" },
