...
(function () {
  ...
  function cockpitSelectorWidgetControllerFunction(
    ...
    $scope.selectDate = function () {
      ...
      if ($scope.ngModel.settings.modalityValue == "multiValue") {
        ...
        var values = $scope.datasetRecords && $scope.datasetRecords.rows && $scope.datasetRecords.rows.length > 0 ? $scope.datasetRecords.rows : $scope.ngModel.activeValues || [];
        for (var k in values) {
    ...
    $scope.refresh = function (element, width, height, datasetRecords, nature) {
      ...
      if (datasetRecords.activeValues && !Array.isArray(datasetRecords.activeValues)) {
        datasetRecords.activeValues.then(
  ...
  function selectorWidgetEditControllerFunction($scope, finishEdit, sbiModule_translate, $mdDialog, originalModel, mdPanelRef, getMetadata, scopeFather, $mdToast, cockpitModule_generalOptions) {
    $scope.translate = sbiModule_translate;
