;(function () {
    angular.module('cockpitModule').service('cockpitModule_mapServices', CockpitModuleMapServiceController)
    function CockpitModuleMapServiceController(sbiModule_translate, sbiModule_messaging, cockpitModule_template, cockpitModule_mapThematizerServices, $q, $mdPanel, $rootScope, $location) {
                        ...
                        try {
                            ...
                            if (geoFieldConfig.properties.coordType == 'json') {
                                ...
                                if (geoFieldConfig.properties.coordType == 'string' && IsJsonString(geoFieldValue)) {
                                    ...
                                    sbiModule_messaging.showInfoMessage(
                                        sbiModule_translate
                                            .load('sbi.cockpit.map.stringInvalid')
                                            .replace('{0}', geoColumn)
                                            .replace('{1}', geoFieldValue.substring(0, 20) + '...'),
                                        'Title',
                                        0
                                    )
                                    return null
        ...
        ms.getSimpleCoordinates = function (geocol, config, value) {
            var coord
            if (Array.isArray(value)) coord = value
            else {
                if (value.indexOf(' ') > 0) {
                    coord = value.split(' ')
                    ...
                    sbiModule_messaging.showInfoMessage(
                        sbiModule_translate
                            .load('sbi.cockpit.map.lonLatError')
                            .replace('{0}', geocol)
                            .replace('{1}', value.substring(0, 20) + '...'),
                        'Title',
                        0
                    )
        ...
        ms.updateCoordinatesAndZoom = function (model, map, l, setValues) {
             ...
             */
            if (!(model.content.currentView.center instanceof Array) || (model.content.currentView.center instanceof Array && isNaN(model.content.currentView.center[0]))) {
        ...
        ms.getColumnName = function (key, values) {
            ...
            for (var v = 0; v < values.length; v++) {
                if (values[v].header === toReturn) return values[v].name
            }
