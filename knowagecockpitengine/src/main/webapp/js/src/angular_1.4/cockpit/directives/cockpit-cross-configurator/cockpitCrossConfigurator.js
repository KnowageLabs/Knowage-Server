...
(function () {
  ...
  function cockpitCrossConfiguratorControllerFunction($scope, sbiModule_translate, cockpitModule_template, cockpitModule_generalServices, knModule_fontIconsService, cockpitModule_generalOptions, cockpitModule_datasetServices, cockpitModule_properties, cockpitModule_documentServices, cockpitModule_crossServices, $filter, cockpitModule_analyticalDriversUrls) {
    ...
    $scope.$watchCollection("ngModel.preview.dataset", function (newValue, oldValue) {
      ...
      if ($scope.allCockpitDatasetsColumns[newPreviewingDSLabel] == undefined) {
        ...
        if (foundDs.length > 0) $scope.allCockpitDatasetsColumns[newPreviewingDSLabel] = foundDs[0].metadata.fieldsMeta;
      }
    ...
    $scope.initModel = function () {
      ...
      if ($scope.ngModel && $scope.ngModel.cross) {
        ...
        for (var k in $scope.ngModel.cross.outputParametersList) {
          if ($scope.ngModel.cross.outputParametersList[k].type === "static") {
            if ($scope.ngModel.cross.outputParametersList[k].value) {
              if (!Array.isArray($scope.ngModel.cross.outputParametersList[k].value)) $scope.ngModel.cross.outputParametersList[k].value = [$scope.ngModel.cross.outputParametersList[k].value];
            } else $scope.ngModel.cross.outputParametersList[k].value = [];
          }
