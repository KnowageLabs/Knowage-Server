...
//	}
	...
	}else if($scope.crossAdvancedTable){
		if($scope.$parent.newModel && $scope.$parent.newModel.dataset!=undefined && $scope.$parent.newModel.dataset.dsId != undefined){
			   angular.copy(cockpitModule_datasetServices.getDatasetById($scope.$parent.newModel.dataset.dsId), $scope.localDataset);
		   }else{
			   if($scope.$parent.newModel) $scope.$parent.newModel.dataset= {};
			   else $scope.$parent.newModel = {dataset:{}};
		   }
	}else{
