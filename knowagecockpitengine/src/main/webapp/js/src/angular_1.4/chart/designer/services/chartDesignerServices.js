...
.service("StructureTabService", function(sbiModule_restServices,sbiModule_messaging,sbiModule_translate,sbiModule_config){
	...
	this.getMinMaxSerieCategorie= function(chartType,chartLibNamesConfig) {
		var minMaxCategories = {};
		var minMaxSeries = {};
		switch(chartType){
		case 'parallel':
			minMaxCategories.min = 1;
			minMaxSeries.min = 2;
			break;
		case 'sunburst':
			minMaxCategories.min = 2;
			minMaxSeries.min = 1;
			break;
		case 'scatter':
			minMaxCategories.min = 1;
			minMaxCategories.max = 1;
			minMaxSeries.min = 1;
			break;
		case 'treemap':
			minMaxCategories.min = 2;
		...
		case 'bar':
			...
			break;
		case 'pie':
			if(chartLibNamesConfig.pie=="chartJs") {
				minMaxCategories.min = 1;
				minMaxCategories.max = 1;
				minMaxSeries.min = 1;
				minMaxSeries.max = 1; 
		...
		return {"category":minMaxCategories, "serie":minMaxSeries}
	}	
...
.service("ChartUpdateService", function(sbiModule_restServices,StructureTabService,sbiModule_messaging,sbiModule_translate,sbiModule_config, $http, $q, $timeout){
	this.getTemplate = function (originalTemplate){
		...
		var categories = checkCategories(originalTemplate);
		var maxCategoriesSeries = limitSerieCateg ();
		if(maxCategoriesSeries.serie[type]<originalTemplate.CHART.VALUES.SERIE.length){
			originalTemplate.CHART.VALUES.SERIE.length = maxCategoriesSeries.serie[type];
		}
		if(maxCategoriesSeries.categ[type]<categories.length){
			categories.length = maxCategoriesSeries.categ[type];
		}
		...
		delete originalTemplate.CHART.VALUES.CATEGORY;
		if(type=="scatter" || type=="radar" ||  type=="pie"){
			originalTemplate.CHART.VALUES.CATEGORY = categories[0];
			
		} else {

			originalTemplate.CHART.VALUES.CATEGORY = categories;
		}
