	...
	private IDataStore queryStandardCachedDataset(List<GroupCriteria> groups, List<FilterCriteria> filters, List<ProjectionCriteria> projections,
		...
		try {
			if (getMetadata().containsCacheItem(resultsetSignature)) {
				...
				/**
				 * orderColumn - the column of the table through which the table will be ordered
				 * for the first category.
				 * 
				 * appendColumnForCategories - the string that will be used for the ORDER BY clause
				 * of the query that we are going to construct. The ordering by potential column and
				 * the category for it comes after the ordering by the series item(s).
				 * 
				 * arrayCategoriesForOrdering - an array list that contains category, its potential
				 * ordering column and their ordering type attached to them. When the clause for the
				 * order by is going to be created, we will take all items from this array list in
				 * order to create a final order by SQL string.
				 * 
				 * keepCategoryForOrdering - the part of the ORDER BY clause that is associated to 
				 * the first category of the document.
				 * 
				 * columnAndCategoryAreTheSame - indicator if the column that is set as an ordering 
				 * one for the first category is of the same value (name) as the category. In that
				 * case we will skip duplicating of columns of the table that the query should 
				 * specify in the SELECT, GROUP BY and ORDER BY clause.
				 * 
				...
				if (projections != null) {
					...
					for (ProjectionCriteria projection : projections) {
						...
						/**
						 * Aggregation function is possible only for series, hence this is the part for 
						 * handling them and their ordering criteria for the final query.
						...
						else
						{
							...
							/**
							 * If the order type is not defined for current item, consider it as it is
							 * of an empty value (empty string).
							 */
							if (orderType == null)
							{
								orderType = "";
							...
							/**
							 * If the order column is defined and is not an empty string the column (attribute)
							 * through which the first category should be ordered is set.
							 */
							...
							/**
							 * Keep the ordering for the first category so it can be appended to the end
							 * of the ORDER BY clause when it is needed.
							 */
					...
					/**
					 * Only in the case when the category name and the name of the column through which
					 * it should be ordered are not the same, append the part for ordering that category 
					 * to the end of the ORDER BY clause.					 
					 * @author Danilo Ristovski (danristo, danilo.ristovski@mht.net)
					 */
					...
					/**
					 * Append ordering by categories (columns, attributes) at the end of the array of
					 * table columns through which the ordering of particular ordering type should be
					 * performed. This is the way in which the query is constructed inside the Chart
					 * Engine, so we will keep the same approach.
					 * @author Danilo Ristovski (danristo, danilo.ristovski@mht.net)
					 */
				...
				if (groups != null) {
					for (GroupCriteria group : groups) {
						...
						/**
						 * If there is an ordering columns set to the first category and if the one is not
						 * the same value as the category, append its name to the GROUP BY clause, just
						 * before the first category name.
						 * @author Danilo Ristovski (danristo, danilo.ristovski@mht.net)
						 */
						else
						{						
							/**
							 * The last expression serves for skipping the duplication of the same column 
