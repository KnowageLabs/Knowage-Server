...
public class InvalidCharsValidator implements ConstraintValidator<NotInvalidCharacters, String> {

	public static final Character[] INVALID_WINDOWS_SPECIFIC_CHARS = { '"', '*', ':', '<', '>', '?', '\\', '|', 0x7F };
	public static final Character[] INVALID_UNIX_SPECIFIC_CHARS = { '\000' };

	@Override
	public void initialize(NotInvalidCharacters arg0) {
	}

	@Override
	public boolean isValid(String toValidate, ConstraintValidatorContext constraintContext) {
		if (!validateStringFilenameUsingContains(toValidate)) {
			return false;
		}
