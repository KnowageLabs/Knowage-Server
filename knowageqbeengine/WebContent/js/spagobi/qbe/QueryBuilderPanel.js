...
Sbi.qbe.QueryBuilderPanel = function(config) {
	...
	this.services['loadCalendarList'] = this.services['loadCalendarList'] || Sbi.config.serviceRegistry.getServiceUrl({
		serviceName: 'LOAD_CALENDAR_LIST_ACTION'
			, baseParams: params
	});
	/*
...
, getQuery: function(asObject) {
	...
	if(asObject) {
		...
		query.calendar=this.selectedCalendar || {};
...
//-- handler methods ---------------------------------------------------------------
,onSelectCalendar : function(node){
...
//		fieldLabel : 'Type',
		...
		listeners:{
			...
			'select': function(element){
				this.onCalendarSelect(node,element.lastSelectionText);
	...
	var dialogBox = new Ext.Window({
		title : LN('sbi.generic.selectCalendar'),
		modal : true,
		width : '50%',
	...
	Ext.Ajax.request({
		url: this.services['loadCalendarList'],
//		params:'',
		params: params,
		success: function(response, options) {
			var dataresp=JSON.parse(response.responseText);
			mystore.loadData(dataresp.rows);
			if(this.selectedCalendar!=undefined && this.selectedCalendar.hasOwnProperty("filters") && this.selectedCalendar.filters.hasOwnProperty("rightOperandValue")  ){
				cb.setValue(this.selectedCalendar.filters.rightOperandValue[0]);
			}else if(mystore.getTotalCount()>0){
				//TODO set default
				cb.setValue(mystore.getAt(0).data["column_1"]);
				this.onCalendarSelect(node,mystore.getAt(0).data["column_1"]);
			}
		},
		scope: this,
		failure: Sbi.exception.ExceptionHandler.handleFailure		
...
, onCalendarSelect : function (node,calendarName) {
	console.log("selezionato calendario",node,calendarName);
	var fields = {
			id: node.id,
			alias: node.attributes.field,
			type:"datamartField",
	...
	var filters = {
			...
			rightOperandDescription: calendarName,
			rightOperandLongDescription:calendarName,
			...
			if(node.parentNode.attributes.attributes.isdefault)	{
				...
				while (node.parentNode.childNodes[i]!=node) { 
					...
					var field = {
							...
							field: n.text,
							alias: n.text,
							longDescription: n.parentNode.text + ' : ' + node.text
					};
					this.selectGridPanel.addField(field);
					i++;
				}
