...
/**
 * This is a table that can be put at an absolute position but can also be added to the document as the class <CODE>Table</CODE>. In the last case when crossing
 * pages the table always break at full rows; if a row is bigger than the page it is dropped silently to avoid infinite loops.
 * <P>
 * A PdfPTableEvent can be associated to the table to do custom drawing when the table is rendered.
 *
 * NOTE: THIS CLASS SHOULD OVERRIDE THE SAME CLASS OF THE ITEX 2.1.7 LIB.. LOOK THE ROW 871
 *
...
public class PdfPTable implements LargeElement {
	 ...
	 *
	 * @param firsttime if true, the heights of the rows will be recalculated. This takes time; normally the heights of the rows are already calcultated, so in most
	 *                  cases, it's save to use false as parameter.
	 * @return the total height of the table. Note that it will be 0 if you didn't specify the width of the table with setTotalWidth().
	...
	/**
	 * Gets the default <CODE>PdfPCell</CODE> that will be used as reference for all the <CODE>addCell</CODE> methods except <CODE>addCell(PdfPCell)</CODE>.
	 *
	...
	/**
	 * Writes the selected rows to the document. <CODE>canvases</CODE> is obtained from <CODE>beginWritingRows()</CODE>.
	 *
	 ...
	 * @param canvases an array of 4 <CODE>PdfContentByte</CODE> obtained from <CODE>beginWrittingRows()</CODE>
	...
	/**
	 * Writes the selected rows and columns to the document. This method does not clip the columns; this is only important if there are columns with colspan at
	 * boundaries. <CODE>canvases</CODE> is obtained from <CODE>beginWritingRows()</CODE>. The table event is only fired for complete rows.
	 *
	 ...
	 * @param xPos     the x write coordinate
	 * @param yPos     the y write coordinate
	 * @param canvases an array of 4 <CODE>PdfContentByte</CODE> obtained from <CODE>beginWritingRows()</CODE>
	 * @return the y coordinate position of the bottom of the last row
	 * @see #beginWritingRows(com.lowagie.text.pdf.PdfContentByte)
	 */
	...
	/**
	 * Writes the selected rows and columns to the document. This method clips the columns; this is only important if there are columns with colspan at boundaries.
	...
	/**
	 * Gets and initializes the 4 layers where the table is written to. The text or graphics are added to one of the 4 <CODE>PdfContentByte</CODE> returned with the
	 * following order:
	 * <p>
	 * <ul>
	 * <li><CODE>PdfPtable.BASECANVAS</CODE> - the original <CODE>PdfContentByte</CODE>. Anything placed here will be under the table.
	 ...
	 * <li><CODE>PdfPtable.TEXTCANVAS</CODE> - the layer where the text go to. Anything placed here will be over the table.
	 * </ul>
	 * <p>
	...
	public float getRowHeight(int idx, boolean firsttime) {
		...
		for (int i = 0; i < relativeWidths.length; i++) {
			...
			/**
			 * MODIFICATION: Without cell!=null, an error occurs while a pdfpTable is added.. NullPointerException because cell==null
			 */
	...
	/**
	 * Gets the height of the rows that constitute the header as defined by <CODE>setHeaderRows()</CODE>.
	 *
	...
	/**
	 * Gets the height of the rows that constitute the footer as defined by <CODE>setFooterRows()</CODE>.
	 *
	...
	/**
	 * Sets the number of the top rows that constitute the header. This header has only meaning if the table is added to <CODE>Document</CODE> and the table crosses
	 * pages.
	 *
			...
			while (rowSpanAbove(rowIndex--, colIndex)) {
				...
				if (row != null) {
					...
					if (replaceCell != null) {
						...
						float diff = getRowspanHeight(rowIndex, colIndex) - getRowHeight(start) - extra;
	 ...
	 *
	 * @param runDirection One of the following values: PdfWriter.RUN_DIRECTION_DEFAULT, PdfWriter.RUN_DIRECTION_NO_BIDI, PdfWriter.RUN_DIRECTION_LTR or
	 ...
	 *
	 * @return One of the following values: PdfWriter.RUN_DIRECTION_DEFAULT, PdfWriter.RUN_DIRECTION_NO_BIDI, PdfWriter.RUN_DIRECTION_LTR or
	 *         PdfWriter.RUN_DIRECTION_RTL.
	 */
	...
	/**
	 * When set the rows that won't fit in the page will be split. Note that it takes at least twice the memory to handle a split table row than a normal table.
	...
	/**
	 * If true the table will be kept on one page if it fits, by forcing a new page if it doesn't fit on the current page. The default is to split the table over
	 * multiple pages.
	 *
	...
	/**
	 * Sets the number of rows to be used for the footer. The number of footer rows are subtracted from the header rows. For example, for a table with two header
	 * rows and one footer row the code would be:
	 *
	 * <pre>
	...
	/**
	 * Completes the current row with the default cell. An incomplete row will be dropped but calling this method will make sure that it will be present in the
	 * table.
	 */
