...
public class SQLFieldsReader {
    ...
    private static final Logger LOGGER = Logger.getLogger(SQLFieldsReader.class);
	 ...
	 */
	public List<Field> readFields() throws Exception {
			...
			try (PreparedStatement ps = connection.prepareStatement(query)) {
				...
				try (ResultSet rs = ps.executeQuery()) {
					...
					for (int i = 1; i <= rsmd.getColumnCount(); ++i) {
						Field field = new Field(rsmd.getColumnLabel(i), getJdbcTypeClass(rsmd, i),
