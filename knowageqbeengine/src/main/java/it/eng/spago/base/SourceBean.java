...
**/
/**
 * La classe <code>SourceBean</code> implementa un contenitore di oggetti. Ogni
 * oggetto memorizzato nel contenitore &egrave; associato ad una chiave che ne consente
 * il recupero. Pi&ugrave; oggetti possono essere memorizzati con la stessa chiave. Al
 * contenitore &egrave; associato un nome.
 * <p>
 * Il contenitore &egrave; in grado di ritornare una sua rappresentazione XML e di
 * costruirsi a partire da uno stream XML (File, String, ...).
 * <p>
 ...
 * </pre></blockquote>
 * <p>
 * Per recuperare il/i valori di un attributo il servizio da invocare &egrave; del
 * tipo:
 * <blockquote><pre>
 * String value2 = (String)outer.getAttribute("inner.param2");
 * </pre></blockquote>
 * <p>
 * La chiave di un attributo pu&ograve; sempre essere espressa con una dot-notation:
 * <blockquote><pre>
...
//  Flag that specify if apply to elements the function toUpperCase().
     ...
     * @exception SourceBeanException viene lanciata se il parametro di input
     * <em>name</em> non &egrave; una nome valido
    ...
    public SourceBean(SourceBean sourceBean) throws SourceBeanException {
        ...
        for (int i = 0; i < sourceBean._attributes.size(); i++) {
            SourceBeanAttribute attribute =
                (SourceBeanAttribute) (sourceBean._attributes.get(i));
    ...
    /**
     * Elimina tutto il contenuto del <code>SourceBean</code>
    ...
    /**
     * Copia tutto il contenuto del parametro <code>SourceBean</code>
     * nel proprio stato.
     * <p>
    ...
    /**
     * Copia il contenuto del parametro <code>SourceBean</code>
     * nel proprio stato a partire dall'attributo con chiave <em>key</em>.
     * <p>
    ...
    /**
     * Ritorna tutti gli oggetti di tipo <code>SourceBeanAttribute</code> il
     * cui campo chiave vale <em>key</em>.
     * <p>
     * @param key chiave dell'attributo in dot-notation
     * @return
     * <ul>
     * <li>
     * <em>null</em> se l'attributo non esiste
     * <li>
     * il <code>SourceBeanAttribute</code> corrispondente alla chiave se
     * l'attributo &egrave; single-value
     * <li>
     * <em>List</em> arraylist di <code>SourceBeanAttribute</code> corrispondenti
     * alla chiave se l'attributo &egrave; multi-value
     * </ul>
    ...
    public Object getAttributeItem(String key) {
        ...
        for (int i = 0; i < _attributes.size(); i++) {
            SourceBeanAttribute attribute =
                (SourceBeanAttribute) (_attributes.get(i));
        ...
        if (values.size() == 1) {
            if (deepSearch) {
                if (!((((SourceBeanAttribute) (values.get(0)))
                    .getValue())
                    instanceof SourceBean)) {
                    TracerSingleton.log(
                        Constants.NOME_MODULO,
                        TracerSingleton.WARNING,
                        "SourceBean::getAttributeItem: attributo ["
                ...
                return (
                    (SourceBean)
                        (
                            ((SourceBeanAttribute) (values.get(0)))
                                .getValue()))
                                .getAttributeItem(
                    childKey);
            } // if (deepSearch)
     ...
     * <em>null</em> se l'attributo non esiste
     * <li>
     * l'oggetto corrispondente alla chiave se l'attributo &egrave; single-value
     * <li>
     * <em>List</em> arraylist degli oggetti corrispondenti alla chiave
     * se l'attributo &egrave; multi-value
     * </ul>
    ...
    public Object getAttribute(String key) {
        ...
        if (attributeItem instanceof SourceBeanAttribute) {
            ...
            SourceBeanAttribute sourceBeanAttribute =
                (SourceBeanAttribute) attributeItem;
        ...
        for (int i = 0; i < sourceBeanAttributes.size(); i++) {
            SourceBeanAttribute sourceBeanAttribute =
                (SourceBeanAttribute) (sourceBeanAttributes.get(i));
     ...
     * <p>
     * @param key chiave dell'attributo in dot-notation
     * @return <em>List</em> arraylist degli oggetti corrispondenti alla chiave,
     * di dimensione nulla se nessun attributo viene trovato
     * @see SourceBean#setAttribute(String, Object)
     * @see SourceBean#setAttribute(SourceBean)
     * @see SourceBean#updAttribute(String, Object)
     * @see SourceBean#delAttribute(String)
    ...
    /**
     * Aggiunge al <code>SourceBean</code> un nuovo attributo con chiave <em>key</em>
     * e valore <em>value</em>. Se il valore dell'attributo &egrave; un <code>SourceBean</code>
     * il servizio &egrave; equivalente a:
     * <blockquote><pre>
     ...
     * @exception SourceBeanException viene lanciata in tutti i casi in cui la chiave
     * espressa in dot-notation non sia corretta
     * @see SourceBean#getAttribute(String)
     * @see SourceBean#setAttribute(SourceBean)
     * @see SourceBean#updAttribute(String, Object)
     * @see SourceBean#delAttribute(String)
    ...
    /**
     * Aggiunge al <code>SourceBean</code> un nuovo attributo il cui valore &egrave;
     * un <code>SourceBean</code>. La chiave con cui il secondo <code>SourceBean</code>
     * viene aggiunto &egrave; pari al nome del contenitore stesso.
     * <p>
     * @param value <code>SourceBean</code> da aggiungere
     * @exception SourceBeanException viene lanciata se <em>value</em> &egrave;
     * <em>null</em>
     * @see SourceBean#getAttribute(String)
     * @see SourceBean#setAttribute(String, Object)
     * @see SourceBean#updAttribute(String, Object)
    ...
    /**
     * Sostituisce il valore dell'attributo con chiave <em>key</em> con il nuovo
     * valore <em>value</em>. Se l'attributo non esiste viene aggiunto. Nel caso
     * in cui alla chiave corrisponda un attributo multi-value viene lanciata
        ...
        if (key.indexOf('.') != -1) {
            ...
            TracerSingleton.log(
                ...
                "SourceBean::updAttribute: attributo ["
                    ...
                    + "] non 衵n SourceBean");
            throw new SourceBeanException(
                "Attributo [" + searchKey + "] non 衵n SourceBean");
        } // if (key.indexOf('.') != -1)
        Object attributeItem = getAttributeItem(key);
        if (attributeItem == null) {
    ...
    public void delAttribute(String key)
        ...
        if (key.indexOf('.') != -1) {
            ...
            if (values.size() == 1) {
                ...
                ((SourceBean) (values.get(0))).delAttribute(
                    key.substring(key.indexOf('.') + 1, key.length()));
                return;
            } // if (values.size() == 1)
            TracerSingleton.log(
    ...
    /**
     * Ritorna il testo contenuto nel <code>SourceBean</code> corrispondente
     * all'attributo di chiave <em>key</em>.
     * <p>
     * @param key chiave dell'attributo in dot-notation
     * @return il testo contenuto
    ...
    /**
     * Sostituisce il testo contenuto nel <code>SourceBean</code> corrispondente
     * all'attributo di chiave <em>key</em> con quello del parametro
     * <em>characters</em>.
     * <p>
     * @param key chiave dell'attributo in dot-notation
     * @param characters il nuovo testo
    ...
    /**
     * Elimina il testo contenuto nel <code>SourceBean</code> corrispondente
     * all'attributo di chiave <em>key</em>.
     * <p>
     * @param key chiave dell'attributo in dot-notation
     * @see SourceBean#getCharacters()
    ...
    /**
     * Ritorna un List contenente tutti i valori dell'attributo con chiave <em>key</em> che sono
     * di tipo <code>SourceBean</code> e contengono un attributo single-value
     * con chiave <em>paramName</em> e valore <em>paramValue</em>.
     * <p>
     * @param key chiave dell'attributo in dot-notation
     * @param paramName nome del parametro di filtro
     * @param paramValue valore del parametro di filtro
     * @return
     * <em>List</em> arraylist di <code>SourceBean</code> se pi&ugrave; valori
     * vengono trovati
     * </ul>
     * @see SourceBean#getAttribute(String)
     */
    public List getFilteredSourceBeanAttributeAsList(
        String key,
        String paramName,
        String paramValue) {
        ...
        for (int i = 0; i < attributeValues.size(); i++) {
            ...
            if (!(attributeValue instanceof SourceBean))
                continue;
            Object localParamValue =
                ((SourceBean) attributeValue).getAttribute(paramName);
            if (((localParamValue == null) && (paramValue != null))
                || ((localParamValue != null) && (paramValue == null)))
                continue;
            if (((localParamValue == null) && (paramValue == null))
                || ((localParamValue instanceof String)
    ...
    /**
     * Ritorna tutti i valori dell'attributo con chiave <em>key</em> che sono
     * di tipo <code>SourceBean</code> e contengono un attributo single-value
     * con chiave <em>paramName</em> e valore <em>paramValue</em>.
     * <p>
     * @param key chiave dell'attributo in dot-notation
     ...
     * <ul>
     * <li>
     * <em>null</em> se nessun valore viene trovato
     * <li>
     * <code>SourceBean</code> se un solo valore viene trovato
     * <li>
     * <em>List</em> arraylist di <code>SourceBean</code> se pi&ugrave; valori
     * vengono trovati
     * </ul>
     * @see SourceBean#getAttribute(String)
     */
    public Object getFilteredSourceBeanAttribute(
        String key,
        String paramName,
        String paramValue) {
    	List sourceBeanAttributeValues =
            getFilteredSourceBeanAttributeAsList(key, paramName, paramValue);
    ...
    /**
     * Ritorna tutti gli oggetti di tipo <code>SourceBeanAttribute</code> contenuti
     * nel <code>SourceBean</code> corrispondente all'attributo di chiave <em>key</em>.
     * <p>
     * @param key chiave dell'attributo in dot-notation
     * @return <code>List</code> list di <code>SourceBeanAttribute</code>
     * contenuti
    ...
    /**
     * Sostituisce tutti gli oggetti di tipo <code>SourceBeanAttribute</code> contenuti
     * con quelli del vettore <em>attributes</em>.
     * <p>
     * @param attributes <code>List</code> list
     * <code>SourceBeanAttribute</code> dei nuovi attributi
     * @see SourceBean#getContainedAttributes()
     * @see SourceBean#getContainedAttributes(String)
     * @see SourceBean#setContainedAttributes(String, List)
     * @see SourceBean#delContainedAttributes()
     ...
     * con quelli del vettore <em>attributes</em>.
     * <p>
     * @param attributes <code>List</code> list di
     * <code>SourceBeanAttribute</code> dei nuovi attributi
     * @see SourceBean#getContainedAttributes()
     * @see SourceBean#getContainedAttributes(String)
     * @see SourceBean#setContainedAttributes(List)
     ...
     * @see SourceBean#delContainedAttributes(String)
     */
    public void setContainedAttributes(
        String key,
        List attributes)
        throws SourceBeanException {
        Object attribute = getAttribute(key);
    ...
    public void updContainedAttributes(List attributes)
        ...
        for (int i = 0; i < attributes.size(); i++) {
            SourceBeanAttribute attribute =
                (SourceBeanAttribute) attributes.get(i);
    ...
    public void updContainedAttributes(
        String key,
    ...
    /**
     * Ritorna gli oggetti di tipo <code>SourceBeanAttribute</code> contenuti
     ...
     * Ritorna gli oggetti di tipo <code>SourceBeanAttribute</code> contenuti
     * nel <code>SourceBean</code> corrispondente all'attributo di chiave <em>key</em>
     * ed il cui valore associato &egrave; di tipo <code>SourceBean</code>.
     * <p>
     * @param key chiave dell'attributo in dot-notation
    ...
    /**
     * Ritorna gli oggetti di tipo <code>SourceBeanAttribute</code> contenuti
     * il cui valore associato &egrave; di tipo <code>XMLObject</code>.
     * <p>
     * @return <code>List</code> arraylist di <code>SourceBeanAttribute</code>
    ...
    /**
     * Ritorna gli oggetti di tipo <code>SourceBeanAttribute</code> contenuti
     * il cui valore associato non &egrave; di tipo <code>XMLObject</code>.
     * <p>
     * @return <code>List</code> arraylist di <code>SourceBeanAttribute</code>
    ...
    /**
     * Ritorna il vettore di chiavi in dot-notation degli attributi a cui &egrave;
     * associata la chiave <em>key</em>.
     * <p>
    ...
    /**
     * Ritorna il <code>SourceBean</code> ottenuto dal parsing
     * dell'<code>InputSource</code> <em>stream</em>.
     * <p>
    ...
    /**
     * Ritorna il <code>SourceBean</code> ottenuto dal parsing
     * della stringa <em>xmlSourceBean</em>.
     * <p>
    ...
    public static SourceBean fromXMLString(String xmlSourceBean, boolean trimCharacters, boolean upperCase)
        ...
        if (!xmlSourceBean.startsWith("<")) {
            xmlSourceBean =
                XMLUtil.XML_HEADER_PREFIX
                    + " version=\""
                    + XMLUtil.XML_HEADER_DEFAULT_VERSION
                    + "\" "
                    + "encoding=\""
...
//                    + "\">\n"
                    ...
                    + xmlSourceBean
                    + "\n</XMLSOURCEBEAN>";
            SourceBean xmlRequestBean =
                fromXMLStream(new InputSource(new StringReader(xmlSourceBean)));
...
//                    + "\">\n"
    ...
    /**
     * Ritorna il <code>SourceBean</code> ottenuto dal parsing
     * del file <em>xmlSourceBean</em>.
     * <p>
     * @param xmlSourceBean nome del file che contiene la rappresentazione XML
     * del <code>SourceBean</code>
     * @return il <code>SourceBean</code> corrispondente allo stream XML
     * @see SourceBean#fromXMLStream(InputSource)
     * @see SourceBean#fromXMLString(String)
     */
    ...
    private void notXMLObjectAttributesToElement(Document document, Element element, XMLSerializer serializer) {
        ...
        for (int i = 0; i < notXMLObjectAttributes.size(); i++) {
            SourceBeanAttribute sourceBeanAttribute =
                (SourceBeanAttribute) (notXMLObjectAttributes.get(i));
            try {
...
//                serializer.serialize(document, element, sourceBeanAttribute.getKey(), sourceBeanAttribute.getValue());
    ...
    private void XMLObjectAttributesToElement(Document document, Element element) {
        ...
        for (int i = 0; i < xmlObjectAttributes.size(); i++) {
            SourceBeanAttribute sourceBeanAttribute =
                (SourceBeanAttribute) (xmlObjectAttributes.get(i));
            XMLObject xmlObject =
                (XMLObject) (sourceBeanAttribute.getValue());
