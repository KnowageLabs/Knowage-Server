...
public class FormState extends EngineAnalysisState {
	...
	public void load(byte[] rowData) throws SpagoBIEngineException {
		...
		try {
			...
			if (encodingFormatVersion.equalsIgnoreCase(CURRENT_VERSION)) {
				...
				formStateJSON = formViewerStateLoader.load(str);
	...
	/**
	 * Used for save the form values: For keep the values of the form if the form state changes (id est: if the fields of the form change ) we need to find the
	 * relation between the new fields and the ones saved in the sub object. This relation is the name of the field. This method maps the id of the field and
	...
	private void fillMaps(JSONArray filters, Map<String, String> idNameMap, Map<String, String> nameIdMap, String idRap, String nameRap, String prefix)
	...
	/**
	 * Get the form state of the subobject and changes the names of the fields of the form with the ids
	 * 
	 * @param formState
	...
	private JSONObject parseValues(JSONObject formState) {
		...
		if (formState != null) {
			...
			if (nameIdMap == null) {
				logger.debug("The nameIdMap is null.. THE STORE STATE VALUES WILL NOT BE PARSED.. ");
				return formState;
			}
	...
	/**
	 * Filter the values of the subobject. It removes the selected field value if the form is changed and the values are not consistent with the new
	 * configuration.
	 * 
	 * @param valuesJSON
	 *            the selected values of the subobject
	 * @param admissibleJSON
		...
		for (int y = 0; y < admissibleJSON.length(); y++) {
			...
			while (keyIter.hasNext()) {
				...
				if (key.equals(prefix + admissibleJSON.getJSONObject(y).getString("title"))) {
					if (!filterAdmissibleValues(valuesJSON.getJSONObject(key).getString("field"),
							admissibleJSON.getJSONObject(y).getJSONArray("admissibleFields"))) {
	...
	/**
	 * Filter the group fields of the subobject. It removes the selected field value if the form is changed and the values are not consistent with the new
	 * configuration.
	 * 
	 * @param valuesJSON
	 *            the selected values of the subobject
